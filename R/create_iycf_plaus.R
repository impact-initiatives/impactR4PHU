...
create_iycf_plaus <- function(
  ...
  if (!is.null(exp_prevalence_mad)) {
    if (!is.numeric(exp_prevalence_mad) & length(exp_prevalence_mad) == 1) {
      stop(
        "Invalid input for expected prevalence for Minimum Acceptable Diet (MAD). Please input exp_prevalence_mad as a decimal between 0 and 1...or leave blank to assume a default of 30%"
      )
    }
    if (exp_prevalence_mad > 1) {
      stop("Please input exp_prevalence_mad as a decimal between 0 and 1.")
    }
  ...
  if (!is.null(exp_ratio_under6m_6to23m)) {
    if (
      !is.numeric(exp_ratio_under6m_6to23m) &
        length(exp_ratio_under6m_6to23m) == 1
    ) {
      stop(
        "Invalid input for exp_ratio_under6m_6to23m. Please put a single numeric value for the expected age ratio of children <6 month to children 2-<5years in the population...or leave blank to assume a 0-<2 years to 6 - 23 month ratio of 1/4 (that is, 25% of children are <6 months of age out of under-2 children.)."
      )
    }
  ...
  if (age_months %in% colnames(df_iycf)) {
    df2 <- df_iycf %>%
      dplyr::mutate(
        is_under6m = ifelse(
          ...
          NA,
          ifelse(!!rlang::sym(age_months) < 6, 1, NA)
        ),
        is_6to23m = ifelse(
          is.na(!!rlang::sym(age_months)),
          NA,
          ifelse(
            !!rlang::sym(age_months) > 5 & !!rlang::sym(age_months) < 24,
            1,
            NA
          )
        )
      ...
      dplyr::summarize(
        age_ratio_under6m_6to23m = sum(!is.na(is_under6m)) /
          sum(!is.na(is_6to23m)),
        age_ratio_under6m_6to23m.pvalue = ifelse(
          is.nan(age_ratio_under6m_6to23m),
          NA,
          as.numeric(stats::chisq.test(
            x = c(sum(!is.na(is_under6m)), sum(!is.na(is_6to23m))),
            p = age_under6m_23m_ratio
          )[3])
        )
      )
    ...
    if (!exists("results")) {
      ...
      results <- merge(results, df2)
    }
  }

  if (!is.null(mad_ratio)) {
    if (is.null(exp_prevalence_mad)) {
      exp_prevalence_mad <- (mad_ratio[1] / mad_ratio[2])
    }
    ...
    df2 <- df_iycf %>%
      dplyr::mutate(
        is_mad = ifelse(is.na(iycf_mad), NA, ifelse(iycf_mad == 1, 1, NA)),
        is_not_mad = ifelse(is.na(iycf_mad), NA, ifelse(iycf_mad == 0, 1, NA))
      ) %>%
      dplyr::group_by(group) %>%
      dplyr::summarize(
        prop_mad_exp = exp_prevalence_mad,
        ...
        mad_ratio.pvalue = ifelse(
          is.nan(mad_ratio),
          NA,
          as.numeric(stats::chisq.test(
            x = c(sum_is_mad, sum_is_not_mad),
            p = c(left, right)
          )[3])
        )
      )
  ...
  if (c("iycf_mdd_score") %in% colnames(df_iycf)) {
    df2 <- df_iycf %>%
      dplyr::group_by(group) %>%
      dplyr::summarise(
        mean_mdd = mean(iycf_mdd_score, na.rm = TRUE),
  ...
  if (c("flag_high_mdd_low_mmf") %in% colnames(df_iycf)) {
    df2 <- df_iycf %>%
      dplyr::group_by(group) %>%
      dplyr::summarise(
        prop_flag_high_mdd_low_mmf = sum(flag_high_mdd_low_mmf, na.rm = TRUE) /
          sum(!is.na(flag_high_mdd_low_mmf))
      )
  ...
  if (length(setdiff(length(iycf_8), 1)) == 0) {
    df2 <- df_iycf %>%
      dplyr::group_by(group) %>%
      dplyr::summarise(
        mean_mmf = mean(!!rlang::sym(iycf_8), na.rm = TRUE),
  ...
  if (c("flag_no_foods") %in% colnames(df_iycf)) {
    df2 <- df_iycf %>%
      dplyr::group_by(group) %>%
      dplyr::summarise(
        prop_flag_no_foods = sum(flag_no_foods, na.rm = TRUE) /
          sum(!is.na(flag_no_foods))
      )
  ...
  if (c("flag_yes_foods") %in% colnames(df_iycf)) {
    df2 <- df_iycf %>%
      dplyr::group_by(group) %>%
      dplyr::summarise(
        prop_flag_yes_foods = sum(flag_yes_foods, na.rm = TRUE) /
          sum(!is.na(flag_yes_foods))
      )
  ...
  if (c("flag_all_foods_no_meal") %in% colnames(df_iycf)) {
    df2 <- df_iycf %>%
      dplyr::group_by(group) %>%
      dplyr::summarise(
        prop_flag_all_foods_no_meal = sum(
          flag_all_foods_no_meal,
          na.rm = TRUE
        ) /
          sum(!is.na(flag_all_foods_no_meal))
      )
  ...
  if (c("flag_some_foods_no_meal") %in% colnames(df_iycf)) {
    df2 <- df_iycf %>%
      dplyr::group_by(group) %>%
      dplyr::summarise(
        prop_flag_some_foods_no_meal = sum(
          flag_some_foods_no_meal,
          na.rm = TRUE
        ) /
          sum(!is.na(flag_some_foods_no_meal))
      )
  ...
  if (c("flag_yes_liquids") %in% colnames(df_iycf)) {
    df2 <- df_iycf %>%
      dplyr::group_by(group) %>%
      dplyr::summarise(
        prop_flag_yes_liquids = sum(flag_yes_liquids, na.rm = TRUE) /
          sum(!is.na(flag_yes_liquids))
      )
  ...
  if (c("flag_no_anything") %in% colnames(df_iycf)) {
    df2 <- df_iycf %>%
      dplyr::group_by(group) %>%
      dplyr::summarise(
        prop_flag_no_anything = sum(flag_no_anything, na.rm = TRUE) /
          sum(!is.na(flag_no_anything))
      )
  ...
  if (length(setdiff(length(iycf_caregiver), 1)) == 0) {
    df2 <- df_iycf %>%
      dplyr::mutate(
        total_iycf_caregiver = ifelse(
          !is.na(!!rlang::sym(iycf_caregiver)),
          1,
          0
        ),
        !!rlang::sym(iycf_caregiver) := dplyr::if_else(
          is.na(!!rlang::sym(iycf_caregiver)),
          NA,
          dplyr::if_else(
            !!rlang::sym(iycf_caregiver) == yes_value_caregiver,
            1,
            dplyr::if_else(
              !!rlang::sym(iycf_caregiver) == no_value_caregiver,
              0,
              NA
            )
          )
      ...
      dplyr::group_by(group) %>%
      dplyr::summarise(
        prop_iycf_caregiver = sum(!!rlang::sym(iycf_caregiver), na.rm = TRUE) /
          sum(total_iycf_caregiver)
      )
  ...
  a <- c(
    ...
    "prop_mad_obs",
    "mad_ratio.pvalue",
    "age_ratio_under6m_6to23m",
    "sex_ratio",
    "sex_ratio.pvalue",
    "age_ratio_under6m_6to23m.pvalue",
    "prop_iycf_caregiver",
    "prop_flag_high_mdd_low_mmf",
    "iycf_plaus_score",
    "iycf_plaus_cat"
  )
