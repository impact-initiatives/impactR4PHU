  ...
  if(age_months %in% colnames(df_iycf)) {
    ...
    df2 <- df_iycf %>%
      ...
      dplyr::summarize(age_ratio_under6m_6to23m = sum(!is.na(is_under6m)) / sum(!is.na(is_6to23m)),
                       age_ratio_under6m_6to23m.pvalue = ifelse(is.nan(age_ratio_under6m_6to23m),NA,as.numeric(stats::chisq.test(x = c(sum(!is.na(is_under6m)), sum(!is.na(is_6to23m))), p = age_under6m_23m_ratio)[3])))
  ...
  if(!is.null(mad_ratio)) {
    ...
    df2 <- df_iycf %>%
      ...
      dplyr::summarize(prop_mad_exp = exp_prevalence_mad,
                       ...
                       mad_ratio.pvalue = ifelse(is.nan(mad_ratio),NA,as.numeric(stats::chisq.test(x = c(sum_is_mad, sum_is_not_mad), p = c(left, right))[3])))
