...
check_fsl_flags <- function(
    ...
    .dataset <- .dataset %>%
      ...
      dplyr::mutate(
        flag_meat_cereal_ratio = ifelse(
          ...
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_cereal) < !!rlang::sym(fsl_fcs_meat),
            1,
            0
          )
        ),
        flag_low_cereal = ifelse(
          is.na(!!rlang::sym(fsl_fcs_cereal)),
          NA,
          ifelse(!!rlang::sym(fsl_fcs_cereal) < 4, 1, 0)
        ),
        flag_low_fcs = ifelse(
          is.na(!!rlang::sym(fsl_fcs_score)),
          NA,
          ifelse(!!rlang::sym(fsl_fcs_score) <= 14, 1, 0)
        ),
        flag_high_fcs = ifelse(
          is.na(!!rlang::sym(fsl_fcs_score)),
          NA,
          ifelse(!!rlang::sym(fsl_fcs_score) >= 100, 1, 0)
        ),
        flag_full_fcs = ifelse(
          is.na(!!rlang::sym(fsl_fcs_score)),
          NA,
          ifelse(!!rlang::sym(fsl_fcs_score) == 112, 1, 0)
        ),
        flag_fcs_zero = ifelse(
          is.na(!!rlang::sym(fsl_fcs_score)),
          NA,
          ifelse(!!rlang::sym(fsl_fcs_score) == 0, 1, 0)
        ),
        flag_low_oil = ifelse(
          is.na(!!rlang::sym(fsl_fcs_cereal)),
          NA,
          ifelse(!!rlang::sym(fsl_fcs_oil) < 3, 1, 0)
        ),
        flag_high_protein = ifelse(
          is.na(!!rlang::sym(fsl_fcs_cereal)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_meat) >= 5 | !!rlang::sym(fsl_fcs_dairy) >= 5,
            1,
            0
          )
        )
      ...
      dplyr::mutate(
        sd_foods = stats::sd(
          c(
            !!rlang::sym(fsl_fcs_cereal),
            !!rlang::sym(fsl_fcs_legumes),
            !!rlang::sym(fsl_fcs_dairy),
            !!rlang::sym(fsl_fcs_meat),
            !!rlang::sym(fsl_fcs_veg),
            !!rlang::sym(fsl_fcs_fruit),
            !!rlang::sym(fsl_fcs_oil),
            !!rlang::sym(fsl_fcs_sugar)
          ),
          na.rm = TRUE
        ),
  ...
  if (length(setdiff(length(required_columns), 10)) != 0) {
    warning(
      "Missing rcsi or fsl_fcs_cereal/fsl_fcs_dairy/fsl_fcs_meat/fsl_fcs_score columns"
    )
  } else {
    .dataset <- .dataset %>%
      ...
      dplyr::mutate(
        flag_protein_rcsi = ifelse(
          is.na(!!rlang::sym(fsl_rcsi_score)),
          NA,
          ifelse(
            is.na(!!rlang::sym(fsl_fcs_cereal)),
            NA,
            ifelse(
              !!rlang::sym(fsl_rcsi_score) >= 19 &
                (!!rlang::sym(fsl_fcs_dairy) >= 5 |
                  !!rlang::sym(fsl_fcs_meat) >= 5),
              1,
              0
            )
          )
        ...
        flag_fcs_rcsi = ifelse(
          ...
          ifelse(
            ...
            NA,
            ifelse(
              !!rlang::sym(fsl_fcs_score) < 35 &
                !!rlang::sym(fsl_rcsi_score) <= 3,
              1,
              0
            )
          )
    ...
    .dataset <- .dataset %>%
      dplyr::mutate(
        flag_fcsrcsi_box = dplyr::case_when(
          ...
          as.numeric(!!rlang::sym(fsl_rcsi_score)) > 18 &
            as.numeric(!!rlang::sym(fsl_fcs_score)) > 56 ~
            1,
          TRUE ~ 0
        )
      ) %>%
      dplyr::rowwise() %>%
      dplyr::mutate(
        sd_rcsicoping = stats::sd(
          c(
            !!rlang::sym(fsl_rcsi_lessquality),
            !!rlang::sym(fsl_rcsi_borrow),
            !!rlang::sym(fsl_rcsi_mealsize),
            !!rlang::sym(fsl_rcsi_mealadult),
            !!rlang::sym(fsl_rcsi_mealnb)
          ),
          na.rm = TRUE
        )
      ) %>%
  ...
  if (length(setdiff(length(lcs_flag_columns), 16)) != 0) {
    ...
    .dataset <- .dataset %>%
      dplyr::mutate(
        flag_lcsi_coherence = ifelse(
          is.na(!!rlang::sym(fsl_lcsi_emergency)),
          NA,
          ifelse(
            !!rlang::sym(fsl_lcsi_emergency) == 1 &
              !!rlang::sym(fsl_lcsi_stress) == 0 |
              !!rlang::sym(fsl_lcsi_emergency) == 1 &
                !!rlang::sym(fsl_lcsi_crisis) == 0 |
              !!rlang::sym(fsl_lcsi_crisis) == 1 &
                !!rlang::sym(fsl_lcsi_stress) == 0,
            1,
            0
          )
        ...
        flag_lcsi_severity = dplyr::case_when(
          is.na(!!rlang::sym(fsl_lcsi_emergency)) ~ NA,
    ...
    .dataset$lcsi.count.na <- apply(.dataset[c(lcs_variables)], 1, function(x) {
  ...
  if (length(setdiff(length(hdds_flag_columns), 14)) != 0) {
    ...
    .dataset <- .dataset %>%
      dplyr::mutate(
        flag_low_hdds = ifelse(
          is.na(!!rlang::sym(fsl_hdds_score)),
          NA,
          ifelse(!!rlang::sym(fsl_hdds_score) == 0, 1, 0)
        ),
        flag_high_hdds = ifelse(
          is.na(!!rlang::sym(fsl_hdds_score)),
          NA,
          ifelse(!!rlang::sym(fsl_hdds_score) == 12, 1, 0)
        )
      )
  }
  ...
  if (length(setdiff(length(fcs_hdds_flag_columns), 14)) != 0) {
    ...
    .dataset <- .dataset %>%
      dplyr::mutate(
        chk_fcs_hdds_cereal1 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_cereal)) |
            is.na(!!rlang::sym(fsl_hdds_cereals)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_cereal) == 7 &
              !!rlang::sym(fsl_hdds_cereals) != fsl_hdds_yes_value,
            1,
            0
          )
        ),
        chk_fcs_hdds_cereal2 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_cereal)) |
            is.na(!!rlang::sym(fsl_hdds_cereals)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_cereal) == 0 &
              !!rlang::sym(fsl_hdds_cereals) == fsl_hdds_yes_value,
            1,
            0
          )
        ...
        chk_fcs_hdds_legumes1 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_legumes)) |
            is.na(!!rlang::sym(fsl_hdds_legumes)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_legumes) == 7 &
              !!rlang::sym(fsl_hdds_legumes) != fsl_hdds_yes_value,
            1,
            0
          )
        ),
        chk_fcs_hdds_legumes2 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_legumes)) |
            is.na(!!rlang::sym(fsl_hdds_legumes)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_legumes) == 0 &
              !!rlang::sym(fsl_hdds_legumes) == fsl_hdds_yes_value,
            1,
            0
          )
        ...
        chk_fcs_hdds_dairy1 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_dairy)) |
            is.na(!!rlang::sym(fsl_hdds_dairy)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_dairy) == 7 &
              !!rlang::sym(fsl_hdds_dairy) != fsl_hdds_yes_value,
            1,
            0
          )
        ),
        chk_fcs_hdds_dairy2 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_dairy)) |
            is.na(!!rlang::sym(fsl_hdds_dairy)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_dairy) == 0 &
              !!rlang::sym(fsl_hdds_dairy) == fsl_hdds_yes_value,
            1,
            0
          )
        ...
        chk_fcs_hdds_meat1 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_meat)) |
            is.na(!!rlang::sym(fsl_hdds_meat)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_meat) == 7 &
              !!rlang::sym(fsl_hdds_meat) != fsl_hdds_yes_value,
            1,
            0
          )
        ),
        chk_fcs_hdds_meat2 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_meat)) |
            is.na(!!rlang::sym(fsl_hdds_meat)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_meat) == 0 &
              !!rlang::sym(fsl_hdds_meat) == fsl_hdds_yes_value,
            1,
            0
          )
        ...
        chk_fcs_hdds_veg1 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_veg)) | is.na(!!rlang::sym(fsl_hdds_veg)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_veg) == 7 &
              !!rlang::sym(fsl_hdds_veg) != fsl_hdds_yes_value,
            1,
            0
          )
        ),
        chk_fcs_hdds_veg2 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_veg)) | is.na(!!rlang::sym(fsl_hdds_veg)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_veg) == 0 &
              !!rlang::sym(fsl_hdds_veg) == fsl_hdds_yes_value,
            1,
            0
          )
        ...
        chk_fcs_hdds_fruit1 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_fruit)) |
            is.na(!!rlang::sym(fsl_hdds_fruit)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_fruit) == 7 &
              !!rlang::sym(fsl_hdds_fruit) != fsl_hdds_yes_value,
            1,
            0
          )
        ),
        chk_fcs_hdds_fruit2 = ifelse(
          is.na(!!rlang::sym(fsl_fcs_fruit)) |
            is.na(!!rlang::sym(fsl_hdds_fruit)),
          NA,
          ifelse(
            !!rlang::sym(fsl_fcs_fruit) == 0 &
              !!rlang::sym(fsl_hdds_fruit) == fsl_hdds_yes_value,
            1,
            0
          )
      ...
      dplyr::rowwise() %>%
      dplyr::mutate(
        sum_fcs_hdds_chks1 = sum(
          chk_fcs_hdds_cereal1,
          chk_fcs_hdds_legumes1,
          chk_fcs_hdds_dairy1,
          chk_fcs_hdds_meat1,
          chk_fcs_hdds_veg1,
          chk_fcs_hdds_fruit1,
        ...
        sum_fcs_hdds_chks2 = sum(
          chk_fcs_hdds_cereal2,
          chk_fcs_hdds_legumes2,
          chk_fcs_hdds_dairy2,
          chk_fcs_hdds_meat2,
          chk_fcs_hdds_veg2,
          chk_fcs_hdds_fruit2,
      ...
      dplyr::ungroup() %>%
      dplyr::mutate(
        flag_hdds_fcs_1 = ifelse(sum_fcs_hdds_chks1 > 0, 1, 0),
