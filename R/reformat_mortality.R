...
reformat_mortality <- function(df) {
  if ("sex" %in% names(df)) {
    ...
    if (length(setdiff(sex_codes, ideal_codes)) == 0) {
      ...
      for (i in 1:length(sex_codes)) {
        a <- svDialogs::dlg_input(
          message = paste0(
            "\n RE-FORMATTING VARIABLE : SEX \n How is '",
            sex_codes[[i]],
            "' coded? Please input either\n '1' for male, \n '2' for 'female' or \n 'NA' for missing. "
          )
        )$res
        ...
        if (!is.na(sex_codes[[i]])) {
          if (a == "NA") {
            df <- df %>%
              dplyr::mutate(sex = ifelse(sex == sex_codes[[i]], NA, sex))
          } else {
            df <- df %>%
              dplyr::mutate(sex = ifelse(sex == sex_codes[[i]], a, sex))
          }
        }
      }
    }
  ...
  if (c("date_dc") %in% names(df)) {
    ...
    if (is.character(df$date_dc)) {
      df <- df %>% dplyr::mutate(date_dc = ifelse(date_dc == "", NA, date_dc))
    }

    df <- df %>%
      dplyr::mutate(
        date_dc_date = lubridate::parse_date_time(date_dc, orders = a)
      ) %>%
      dplyr::mutate(
  ...
  if (c("date_recall") %in% names(df)) {
    ...
    if (is.character(df$date_recall)) {
      df <- df %>%
    ...
    df <- df %>%
      dplyr::mutate(
        date_recall_date = lubridate::parse_date_time(date_recall, orders = a)
      ) %>%
      dplyr::mutate(
  ...
  if (c("date_join") %in% names(df)) {
    ...
    df <- df %>%
      dplyr::mutate(
        date_join_date = lubridate::parse_date_time(date_join, orders = a)
      ) %>%
      dplyr::mutate(
  ...
  if (c("date_left") %in% names(df)) {
    ...
    df <- df %>%
      dplyr::mutate(
        date_left_date = lubridate::parse_date_time(date_left, orders = a)
      ) %>%
      dplyr::mutate(
  ...
  if (c("date_birth") %in% names(df)) {
    ...
    df <- df %>%
      dplyr::mutate(
        date_birth_date = lubridate::parse_date_time(date_birth, orders = a)
      ) %>%
      dplyr::mutate(
  ...
  if (c("date_death") %in% names(df)) {
    ...
    df <- df %>%
      dplyr::mutate(
        date_death_date = lubridate::parse_date_time(date_death, orders = a)
      ) %>%
      dplyr::mutate(
        ...
        if (a == "NA") {
          df <- df %>%
            dplyr::mutate(dplyr::across(
              list_to_check,
              ~ (ifelse(. == list_to_check_codes[[i]], NA, .))
            ))
        } else {
          df <- df %>%
            dplyr::mutate(dplyr::across(
              list_to_check,
              ~ (ifelse(. == list_to_check_codes[[i]], a, .))
            ))
        }
      }
    }
          ...
          df <- df %>%
            dplyr::mutate(
              death_location_smart = ifelse(
                death_location == location_codes[[i]],
                NA,
