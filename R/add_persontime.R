  ...
  if (smart == TRUE) {
    df_mortality <- df_mortality %>% dplyr::mutate(age_years = as.numeric(age_years),
                                                   ...
                                                   person_time = ifelse(is.na(date_join_date), person_time,
                                                                        ifelse(!is.na(date_death_date) & !is.na(death) & !is.na(join), as.numeric(lubridate::days(date_death_date - date_join_date))/86400,
                                                                               ifelse(!is.na(date_left_date) & !is.na(left) & !is.na(join), as.numeric(lubridate::days(date_left_date - date_join_date))/86400,
                                                                                      ifelse(!is.na(join), as.numeric(lubridate::days(date_dc_date - date_join_date))/86400, person_time)))),
                                                   ...
                                                   person_time = ifelse(is.na(date_left_date), person_time,
                                                                        ifelse(!is.na(date_join_date) & !is.na(join), person_time,
                                                                               ifelse(!is.na(left), as.numeric(lubridate::days(date_left_date - date_recall_date))/86400, person_time))),
                                                   ...
                                                   person_time = ifelse(is.na(date_birth_date), person_time,
                                                                        ifelse( date_birth_date < date_recall_date, person_time,
                                                                                ifelse(!is.na(date_death_date)  & !is.na(death) & !is.na(birth), as.numeric(lubridate::days(date_death_date - date_birth_date))/86400,
                                                                                       ifelse(!is.na(date_left_date) & !is.na(left) & !is.na(birth), as.numeric(lubridate::days(date_left_date - date_birth_date))/86400,
                                                                                              ifelse(!is.na(birth), as.numeric(lubridate::days(date_dc_date - date_birth_date))/86400, person_time))))),
                                                   #
                                                   ...
                                                   # person_time = case_when((!is.na(date_join_date) & date_join_date >= date_recall_date) & is.na(date_left_date) & is.na(date_death_date) ~ as.numeric(lubridate::as_date(date_dc_date) - lubridate::as_date(date_join_date)),
                                                   ...
                                                   #                         (!is.na(date_left_date) & date_left_date < date_dc_date) & is.na(date_death_date) ~ as.numeric(lubridate::as_date(date_left_date) - lubridate::as_date(date_recall_date)),
