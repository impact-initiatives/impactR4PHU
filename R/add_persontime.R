...
add_persontime <- function(df_mortality, smart = FALSE) {
    ...
    df_mortality <- df_mortality %>%
      dplyr::mutate(
        ...
        person_time = ifelse(
          ...
          person_time,
          ifelse(
            !is.na(date_death_date) & !is.na(death) & !is.na(join),
            as.numeric(lubridate::days(date_death_date - date_join_date)) /
              86400,
            ifelse(
              !is.na(date_left_date) & !is.na(left) & !is.na(join),
              as.numeric(lubridate::days(date_left_date - date_join_date)) /
                86400,
              ifelse(
                !is.na(join),
                as.numeric(lubridate::days(date_dc_date - date_join_date)) /
                  86400,
                person_time
              )
            )
        ...
        person_time = ifelse(
          ...
          person_time,
          ifelse(
            !is.na(date_join_date) & !is.na(join),
            person_time,
            ifelse(
              !is.na(left),
              as.numeric(lubridate::days(date_left_date - date_recall_date)) /
                86400,
              person_time
            )
          )
        ...
        person_time = ifelse(
          ...
          person_time,
          ifelse(
            date_birth_date < date_recall_date,
            person_time,
            ifelse(
              !is.na(date_death_date) & !is.na(death) & !is.na(birth),
              as.numeric(lubridate::days(date_death_date - date_birth_date)) /
                86400,
              ifelse(
                !is.na(date_left_date) & !is.na(left) & !is.na(birth),
                as.numeric(lubridate::days(date_left_date - date_birth_date)) /
                  86400,
                ifelse(
                  !is.na(birth),
                  as.numeric(lubridate::days(date_dc_date - date_birth_date)) /
                    86400,
                  person_time
                )
              )
        ...
        person_time = ifelse(
          ...
          ifelse(
            ...
            person_time,
            ifelse(
              !is.na(date_birth_date) & !is.na(birth),
              person_time,
              ifelse(
                !is.na(death),
                as.numeric(lubridate::days(
                  date_death_date - date_recall_date
                )) /
                  86400,
                person_time
              )
    ...
    df_mortality <- df_mortality %>%
      dplyr::mutate(
        under_5 = ifelse(
          is.na(age_years),
          NA,
          ifelse(as.numeric(age_years) < 5, 1, NA)
        ),
        under_5_pt = ifelse(
          is.na(under_5),
          NA,
  ...
  if (smart == FALSE) {
    df_mortality <- df_mortality %>%
      dplyr::mutate(
        ...
        person_time_in = ifelse(
          !is.na(join) | !is.na(birth),
          person_time *
            0.5,
          0
        ),
        person_time_out = ifelse(
          !is.na(death) | !is.na(left),
          person_time *
            0.5,
          0
        ),
  ...
  df_mortality <- df_mortality %>%
    dplyr::mutate(
      ...
      age_0to2 = ifelse(
        is.na(age_years),
        NA,
        ifelse(age_years >= 0 & age_years < 2, 1, NA)
      ),
      age_2to5 = ifelse(
        is.na(age_years),
        NA,
        ifelse(age_years >= 2 & age_years < 5, 1, NA)
      ),
      age_5to10 = ifelse(
        is.na(age_years),
        NA,
        ifelse(age_years >= 5 & age_years < 10, 1, NA)
      ),
      age_0to5 = ifelse(
        is.na(age_years),
        NA,
        ifelse(age_years >= 0 & age_years < 5, 1, NA)
      ),
      age_5plus = ifelse(
        is.na(age_years),
        NA,
        ifelse(age_years >= 5 & age_years < 200, 1, NA)
      ),
    )
  df_mortality$age_group <- cut(
    as.numeric(df_mortality$age_years),
    breaks = c(
      -1,
      4,
      9,
  ...
  df_mortality <- df_mortality %>%
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(
      id = paste0(uuid, "_", individual_id),
      individual_id = NULL
    ) %>%
