...
create_fsl_plaus <- function(
  ...
  if ("fsl_fcs_score" %in% names(.dataset)) {
    results2 <- .dataset %>%
      ...
      dplyr::summarise(
        ...
        mean_days_cereals = round(
          mean(!!rlang::sym(fsl_fcs_cereal), na.rm = TRUE),
          2
        ),
        sd_days_cereals = round(
          stats::sd(!!rlang::sym(fsl_fcs_cereal), na.rm = TRUE),
          2
        ),
        mean_days_legumes = round(
          mean(!!rlang::sym(fsl_fcs_legumes), na.rm = TRUE),
          2
        ),
        sd_days_legumes = round(
          stats::sd(!!rlang::sym(fsl_fcs_legumes), na.rm = TRUE),
          2
        ),
        mean_days_dairy = round(
          mean(!!rlang::sym(fsl_fcs_dairy), na.rm = TRUE),
          2
        ),
        sd_days_dairy = round(
          stats::sd(!!rlang::sym(fsl_fcs_dairy), na.rm = TRUE),
          2
        ),
        mean_days_meat = round(
          mean(!!rlang::sym(fsl_fcs_meat), na.rm = TRUE),
          2
        ),
        sd_days_meat = round(
          stats::sd(!!rlang::sym(fsl_fcs_meat), na.rm = TRUE),
          2
        ),
        mean_days_veg = round(mean(!!rlang::sym(fsl_fcs_veg), na.rm = TRUE), 2),
        sd_days_veg = round(
          stats::sd(!!rlang::sym(fsl_fcs_veg), na.rm = TRUE),
          2
        ),
        mean_days_fruit = round(
          mean(!!rlang::sym(fsl_fcs_fruit), na.rm = TRUE),
          2
        ),
        sd_days_fruit = round(
          stats::sd(!!rlang::sym(fsl_fcs_fruit), na.rm = TRUE),
          2
        ),
        mean_days_oils = round(
          mean(!!rlang::sym(fsl_fcs_oil), na.rm = TRUE),
          2
        ),
        sd_days_oils = round(
          stats::sd(!!rlang::sym(fsl_fcs_oil), na.rm = TRUE),
          2
        ),
        mean_days_sugar = round(
          mean(!!rlang::sym(fsl_fcs_sugar), na.rm = TRUE),
          2
        ),
        sd_days_sugar = round(
          stats::sd(!!rlang::sym(fsl_fcs_sugar), na.rm = TRUE),
          2
        )
      )
    if (!exists("results")) {
    ...
    results2 <- .dataset %>%
      ...
      dplyr::summarise(
        ...
        mean_rcsi_lessquality = round(
          mean(!!rlang::sym(fsl_rcsi_lessquality), na.rm = TRUE),
          2
        ),
        sd_rcsi_lessquality = round(
          stats::sd(!!rlang::sym(fsl_rcsi_lessquality), na.rm = TRUE),
          2
        ),
        mean_rcsi_borrow = round(
          mean(!!rlang::sym(fsl_rcsi_borrow), na.rm = TRUE),
          2
        ),
        sd_rcsi_borrow = round(
          stats::sd(!!rlang::sym(fsl_rcsi_borrow), na.rm = TRUE),
          2
        ),
        mean_rcsi_mealsize = round(
          mean(!!rlang::sym(fsl_rcsi_mealsize), na.rm = TRUE),
          2
        ),
        sd_rcsi_mealsize = round(
          stats::sd(!!rlang::sym(fsl_rcsi_mealsize), na.rm = TRUE),
          2
        ),
        mean_rcsi_mealadult = round(
          mean(!!rlang::sym(fsl_rcsi_mealadult), na.rm = TRUE),
          2
        ),
        sd_rcsi_mealadult = round(
          stats::sd(!!rlang::sym(fsl_rcsi_mealadult), na.rm = TRUE),
          2
        ),
        mean_rcsi_mealnb = round(
          mean(!!rlang::sym(fsl_rcsi_mealnb), na.rm = TRUE),
          2
        ),
        sd_rcsi_mealnb = round(
          stats::sd(!!rlang::sym(fsl_rcsi_mealnb), na.rm = TRUE),
          2
        )
      )
    if (!exists("results")) {
  ...
  if (all(c("fsl_fcs_score", "fsl_rcsi_score") %in% names(.dataset))) {
    ...
    filtered_group <- .dataset %>%
      dplyr::mutate(
        fsl_fcs_score_notna = ifelse(!is.na(!!rlang::sym(fsl_fcs_score)), 1, 0),
        fsl_rcsi_score_notna = ifelse(
      ...
      dplyr::group_by(group) %>%
      dplyr::summarise(
        fsl_fcs_score_notna = sum(fsl_fcs_score_notna, na.rm = T),
  ...
  if (
    ...
    nms <- .dataset %>% dplyr::select(dplyr::starts_with("flag")) %>% names()
    results2 <- .dataset %>%
      dplyr::group_by(group) %>%
    ...
    results2 <- .dataset %>%
      ...
      dplyr::summarise(
        prop_fc_flags = sum(flag_fc_cell, na.rm = TRUE) /
          sum(!is.na(fsl_fc_cell), na.rm = TRUE),
