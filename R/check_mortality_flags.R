...
check_mortality_flags <- function(
  ...
  if (!"uuid" %in% names(df)) {
    stop("uuid not available in the df Mortality")
  }
  if (!"enumerator" %in% names(df)) {
    stop("enumerator not available in the df Mortality")
  }

  hh_summary <- df %>%
    dplyr::mutate(
      flag_cause_death = ifelse(
        sex == 1 & death_cause %in% cause_death_f,
        1,
        0
      ),
    ...
    dplyr::summarize(
      flag_multiple_death = ifelse(sum(!is.na(death)) > 1, 1, 0),
