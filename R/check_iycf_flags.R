...
check_iycf_flags <- function(
  ...
  if (length(setdiff(length(required_columns), 28)) != 0) {
    ...
    .dataset <- .dataset %>%
      dplyr::mutate(
        flag_yes_foods = ifelse(
          is.na(count_foods),
          NA,
          ifelse(count_foods == 18, 1, 0)
        ),
        flag_yes_liquids = ifelse(
          is.na(count_liquids),
          NA,
          ifelse(count_liquids == 10, 1, 0)
        ),
        flag_no_foods = ifelse(
          is.na(count_foods),
          NA,
          ifelse(count_foods == 0, 1, 0)
        ),
        flag_no_liquids = ifelse(
          is.na(count_liquids),
          NA,
  ...
  if (length(setdiff(length(required_columns), 29)) != 0) {
    warning(
      "Your dataset appears not to have either all the foods/liquids or the IYCF 4 from the standard IYCF 2021 question sequence.\nIt is advised you ask about all recommended foods/liquids or there is a risk of overestimating EBF."
    )
  } else {
    .dataset <- .dataset %>%
      dplyr::mutate(
        flag_no_anything = ifelse(
  ...
  if (length(setdiff(length(required_columns), 29)) != 0) {
    warning(
      "Your dataset appears not to have either all the foods/liquids or the IYCF 8 from the standard IYCF 2021 question sequence.\nIt is advised you ask about all recommended foods/liquids or there is a risk of overestimating EBF."
    )
  } else {
    .dataset <- .dataset %>%
      dplyr::mutate(
        flag_no_foods_at_all = ifelse(
          flag_no_foods == 1 &
        ...
        flag_all_foods_no_meal = ifelse(
          flag_yes_foods == 1 &
            as.numeric(!!rlang::sym(iycf_8)) == 0,
          1,
          0
        ),
        flag_some_foods_no_meal = ifelse(
          count_foods > 0 &
            count_foods < 18 &
            as.numeric(!!rlang::sym(iycf_8)) == 0 &
            as.numeric(!!rlang::sym(age_months)) > 5,
  ...
  if (length(setdiff(length(required_columns), 2)) != 0) {
    warning(
      "Your dataset appears not to have either the IYCF 8 or the iycf_mdd_score columns .\nMake sure that you have the add_iycf() before this function."
    )
  } else {
    .dataset <- .dataset %>%
      dplyr::mutate(
        flag_high_mdd_low_mmf = ifelse(
          is.na(iycf_mdd_score),
          NA,
          ifelse(
            is.na(!!rlang::sym(iycf_8)),
            NA,
            ifelse(
              iycf_mdd_score >= 6 & as.numeric(!!rlang::sym(iycf_8)) <= 1,
              1,
              0
            )
          )
  ...
  if (length(setdiff(length(required_columns), 4)) != 0) {
    warning(
      "Your dataset appears not to have either the IYCF 4 or iycf_6b/6c/6d_num columns .\nMake sure that you run the add_iycf() before this function."
    )
  } else {
    .dataset <- .dataset %>%
      dplyr::mutate(
        flag_under6_nobf_nomilk = ifelse(
          is.na(!!rlang::sym(age_months)) |
            is.na(!!rlang::sym(iycf_4)) |
            is.na(!!rlang::sym(iycf_6b_num)) |
            is.na(!!rlang::sym(iycf_6c_num)) |
            is.na(!!rlang::sym(iycf_6d_num)),
          NA,
          ifelse(
  ...
  if (length(setdiff(length(required_columns), 7)) != 0) {
    warning(
      "Your dataset appears not to have either the iycf_7b/iycf_7d/iycf_7i/iycf_7j/iycf_7k/iycf_7l/iycf_7m columns.\nIt is advised you ask about all recommended foods/liquids or there is a risk of overestimating EBF.\nMake sure also to run the add_iycf() before this function."
    )
  } else {
    .dataset <- .dataset %>%
      dplyr::mutate(
        flag_meats_nostaples = ifelse(
