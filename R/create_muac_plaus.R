...
create_muac_plaus <- function(df, grouping = NULL, file_path = NULL, short_report = NULL) {
  ...
  if(c("nut_muac_cm") %in% names(df)) {
    df2 <- df %>%
      ...
      dplyr::summarise(n_children_muac = sum(!is.na(nut_muac_cm), na.rm = TRUE),
                       ...
                       gam_muac_low = round(gam_muac_abs - (stats::qt(p=0.05/2, df = dplyr::n() - 1, lower.tail = F)*(stats::sd(gam_muac_noflag, na.rm = TRUE) / sqrt(dplyr::n()))*100),2),
                       gam_muac_upp = round(gam_muac_abs + (stats::qt(p=0.05/2, df = dplyr::n() - 1, lower.tail = F)*(stats::sd(gam_muac_noflag, na.rm = TRUE) / sqrt(dplyr::n()))*100),2),
                       mam_muac_abs = round(mean(mam_muac_noflag, na.rm = TRUE)*100,3),
                       mam_muac_low = round(mam_muac_abs - (stats::qt(p=0.05/2, df = dplyr::n() - 1, lower.tail = F)*(stats::sd(mam_muac_noflag, na.rm = TRUE) / sqrt(dplyr::n()))*100),2),
                       mam_muac_upp = round(mam_muac_abs + (stats::qt(p=0.05/2, df = dplyr::n() - 1, lower.tail = F)*(stats::sd(mam_muac_noflag, na.rm = TRUE) / sqrt(dplyr::n()))*100),2),
                       sam_muac_abs = round(mean(sam_muac_noflag, na.rm = TRUE)*100,3),
                       sam_muac_low = round(sam_muac_abs - (stats::qt(p=0.05/2, df = dplyr::n() - 1, lower.tail = F)*(stats::sd(sam_muac_noflag, na.rm = TRUE) / sqrt(dplyr::n()))*100),2),
                       sam_muac_upp = round(sam_muac_abs + (stats::qt(p=0.05/2, df = dplyr::n() - 1, lower.tail = F)*(stats::sd(sam_muac_noflag, na.rm = TRUE) / sqrt(dplyr::n()))*100),2)) %>%
