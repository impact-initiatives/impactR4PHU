...
#' }
create_mortality_plaus <- function(
  df_mortality,
  df_main = NULL,
  uuid_main = NULL,
  date_dc = NULL,
  ...
  if (is.null(grouping)) {
    ...
    df_mortality <- df_mortality %>%
      dplyr::mutate(group = !!rlang::sym(grouping))
  }

  if (!is.null(exp_sex_ratio)) {
    if (!is.numeric(exp_sex_ratio) & length(exp_sex_ratio) == 1) {
      stop(
        "Invalid input for exp_sex_ratio. Please put a single numeric value for the expected sex ratio in the population of male:female...or leave blank to assume a 1:1 male to female ratio (or 1)."
      )
    }
  ...
  if (!is.null(exp_ratio_0_4)) {
    if (!is.numeric(exp_ratio_0_4) & length(exp_ratio_0_4) == 1) {
      stop(
        "Invalid input for exp_ratio_0_4years Please put a single numeric value for the expected age ratio of <5years to >5years in the population...or leave blank to assume a 1:4 <5 years to >5 years ratio (that is, 20% of individuals are <5 years of age)."
      )
    }
  ...
  if (!is.null(exp_ratio_2_5)) {
    if (!is.numeric(exp_ratio_2_5) & length(exp_ratio_2_5) == 1) {
      stop(
        "Invalid input for exp_ratio_2_5 Please put a single numeric value for the expected age ratio of children 0-<2 years to children 2-<5years in the population...or leave blank to assume a 0-<2 years to 2-<5years ratio of 0.7 (that is, ~41% of individuals are <2 years of age out of under-5 children.)."
      )
    }
  ...
  if (!is.null(exp_ratio_5_10)) {
    if (!is.numeric(exp_ratio_5_10) & length(exp_ratio_5_10) == 1) {
      stop(
        "Invalid input for exp_ratio_5_10 Please put a single numeric value for the expected age ratio of children 0-<5 years to children 5-<10years in the population...or leave blank to assume a 0-<5 years to 5-<10years ratio of 1.1 (that is, ~52% of individuals are <5 years of age out of under-10 children)."
      )
    }
  ...
  if (!is.null(exp_hh_size)) {
    if (!is.numeric(exp_hh_size) & length(exp_hh_size) == 1) {
      stop(
        "Invalid input for exp_hh_size Please put a single numeric value for the expected hh size in the population...or leave blank to assume a 5 average HH size."
      )
    }
  ...
  if (c("age_years") %in% names(df_mortality)) {
    df_mortality <- df_mortality %>%
  ...
  if (!is.null(df_main)) {
    ...
    if (!is.null(num_join)) {
      if (!num_join %in% names(df_main)) {
        stop("num_join argument incorrect, or not available in the dataset")
      }
    }

    if (!is.null(num_left)) {
      if (!num_left %in% names(df_main)) {
        stop("num_left argument incorrect, or not available in the dataset")
      }
    }

    if (!is.null(enumerator)) {
      if (!enumerator %in% names(df_main)) {
        stop("enumerator argument incorrect, or not available in the dataset")
      }
    }
    if (is.null(grouping)) {
    ...
    df_main_join <- df_main %>%
      dplyr::mutate(
        num_days = as.numeric(
          lubridate::as_date(!!rlang::sym(date_dc)) -
            lubridate::as_date(!!rlang::sym(date_recall_event))
        ),
        pt_join = ifelse(
          is.na(num_join),
          0,
          as.numeric(num_days) * as.numeric(num_join) * 0.5
        ),
        pt_left = ifelse(
          is.na(num_left),
          0,
          as.numeric(num_days) * as.numeric(num_left) * 0.5
        )
      ) %>%
      dplyr::group_by(group) %>%
      dplyr::summarise(
        pt_join = sum(pt_join, na.rm = T),
  ...
  if (nrow(df_main_join) > 0) {
    df2 <- df_mortality %>%
      dplyr::group_by(group) %>%
      dplyr::summarize(
        total_people = sum(!is.na(person_time), na.rm = TRUE),
        ...
        age_ratio_5_10.pvalue = ifelse(
          is.na(age_ratio_5_10),
          NA,
          try(
            stats::chisq.test(
              x = c(sum(!is.na(under_5)), sum(!is.na(age_5to10))),
      ...
      dplyr::mutate(
        cdr = deaths / (total_persontime + pt_left - pt_join),
        cdr_se = sqrt(
          (cdr * (1 - cdr)) / (total_persontime + pt_left - pt_join)
        ),
        ...
        birth_rate_se = sqrt(
          (birth_rate * (1 - birth_rate)) / (total_persontime / 365)
        ),
        birth_rate_lower_ci = round(
          (birth_rate - 1.96 * birth_rate_se) * 1000,
          3
        ),
        birth_rate_lower_ci = ifelse(
          birth_rate_lower_ci < 0,
          0,
          birth_rate_lower_ci
        ),
        birth_rate_upper_ci = round(
          (birth_rate + 1.96 * birth_rate_se) * 1000,
          3
        ),
    ...
    df2 <- df_mortality %>%
      ...
      dplyr::summarize(
        total_people = sum(!is.na(person_time), na.rm = TRUE),
        total_persontime = sum(person_time, na.rm = TRUE),
        total_persontime_out = ifelse(
          "person_time_out" %in% names(df_mortality),
          sum(person_time_out, na.rm = T),
          0
        ),
        total_persontime_in = ifelse(
          "person_time_in" %in% names(df_mortality),
          sum(person_time_in, na.rm = T),
          0
        ),
        avg.persontime = mean(person_time, na.rm = TRUE),
        total_under5 = sum(under_5, na.rm = TRUE),
        ...
        sex_ratio.pvalue = as.numeric(nipnTK::sexRatioTest(
          sex,
          codes = c("1", "2"),
          pop = sx_ratio
        )[5]),
      ...
      dplyr::mutate(
        cdr = deaths /
          (total_persontime + total_persontime_out - total_persontime_in),
        cdr_se = sqrt(
          (cdr * (1 - cdr)) /
            (total_persontime + total_persontime_out - total_persontime_in)
        ),
  ...
  df3 <- df_mortality %>%
    ...
    dplyr::summarise(
      hh_size = sum(!is.na(sex), na.rm = TRUE),
    ...
    dplyr::mutate(
      is_hh = ifelse(is.na(uuid), NA, 1),
      is_hh_under5 = ifelse(is.na(uuid), NA, ifelse(total_under5 > 0, 1, 0)),
      is_hh_flag_deaths = ifelse(
        is.na(uuid),
        NA,
        ifelse(total_flag_deaths > 0, 1, 0)
      )
    )
  ...
  draft_df3 <- df3 %>%
    ...
    dplyr::mutate(
      prop_hh_under5 = (n_hh_under_5 / n_hh),
  ...
  df4 <- df4 %>%
    dplyr::select(c(
      ...
      total_under5,
      mean_num_under5,
      total_under5_persontime,
      n_hh,
      n_hh_under_5,
      sex_ratio,
      sex_ratio.pvalue,
      age_ratio_0_5,
      age_ratio_0_5.pvalue,
      age_ratio_2_5,
      age_ratio_2_5.pvalue,
      age_ratio_5_10,
      age_ratio_5_10.pvalue,
      joins,
      prop_join_people,
      lefts,
      prop_left_people,
  ...
  if (!is.null(file_path)) {
    writexl::write_xlsx(df4, file_path)
  }
  options(warn = 0)
