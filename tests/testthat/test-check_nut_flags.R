...
testthat::test_that("Check input type -- tool.survey", {
  testthat::expect_error(check_nut_flags(.dataset = "x"))
  testthat::expect_error(check_nut_flags(.dataset = 0))
  testthat::expect_error(check_nut_flags(.dataset = list()))
  testthat::expect_error(check_nut_flags(.dataset = 1.2))
})

testthat::test_that("Check dataframe empty", {
  test_df <- data.frame()
  testthat::expect_error(check_nut_flags(.dataset = test_df))
})

testthat::test_that("Check nut_muac_cm in the dataset", {
  load(testthat::test_path("testdata","test_df_nut.rda"))
  testthat::expect_error(check_nut_flags(
...
#   actual <- check_nut_flags(.dataset = df1)%>%
