...
pacman::p_load(tidyverse, readxl, writexl, openxlsx, randomcoloR, anytime, 
               cluster, DT, stringr, knitr, svDialogs, flextable)
...
if(length(enumerator_column) == 1){
  ...
  enumerator <- svDialogs::dlg_list(enumerator_column, title = "Enumerator Columns")$res
...
if(all(fcs_check_columns %in% names(raw.flag.fcs))) {
  ...
  (impactR4PHU::plot_ridge_distribution(raw.flag.fcs, numeric_cols = c("fsl_fcs_cereal", "fsl_fcs_dairy", "fsl_fcs_veg", "fsl_fcs_fruit", "fsl_fcs_legumes", "fsl_fcs_sugar", "fsl_fcs_oil"),
