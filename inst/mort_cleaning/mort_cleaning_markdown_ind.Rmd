...
label_colname <- tcltk::tk_select.list(names(readxl::read_excel(path.tool, sheet = "survey"))[grepl("label",names(readxl::read_excel(path.tool, sheet = "survey")))], title = "Label column to choose", multiple = F)
...
if(!file.exists("inputs/environment.Rdata")) {
 ...
 ## Detect Enumerator column
  ...
  if(length(uuid_roster) == 1){
    ...
    birth_roster <- names(raw.hh_roster)[grepl("born|birth",names(raw.hh_roster))]
    ...
    birthdate_roster <- names(raw.hh_roster)[grepl("dob|birth",names(raw.hh_roster))]
  ...
  if(collected_df_left){
    ...
    birth_left <- names(raw.left_member)[grepl("born|birth|dob",names(raw.left_member))]
...
df_mortality_long <- impactR4PHU::create_mortality_long_df(df_main = raw.main,
  ...
  if(collected_df_left){
    list_of_var <- c("date_dc","date_recall_event","enumerator","admin1","admin2","cluster",
                     ...
                     "sex_died","age_died","birth_died","birthdate_died","joined_died","smart",
                     ...
                     "label_colname","path.sheet.with.main","collect_num_join_left","num_join","num_left",
                     "path.sheet.with.roster","path.sheet.with.leavers","path.sheet.with.death", "dates_collected",
    ...
    list_of_var <- c("date_dc","date_recall_event","enumerator","admin1","admin2","cluster",
                     "uuid_main","uuid_roster","sex_roster","age_roster","joined_roster","joined_date_roster",
                     "birth_roster","birthdate_roster","uuid_died","smart",
                     "sex_died","age_died","birth_died","birthdate_died","joined_died",
                     "joined_date_died","date_death","death_cause","death_location","cause_death_f",
                     "exp_sex_ratio","exp_ratio_0_4","exp_ratio_2_5","exp_ratio_5_10","exp_hh_size",
                     "label_colname","path.sheet.with.main","collect_num_join_left","num_join","num_left",
                     "path.sheet.with.roster","path.sheet.with.death", "dates_collected",
                     "male","female","date_dc_reformat","date_recall_reformat","date_join_reformat",
...
# Check number 1
if("flag_multiple_death" %in% names(flag_mortality)){
...
if("flag_cause_death" %in% names(flag_mortality)){
  enum_df <- raw.main %>% 
    dplyr::rename(uuid = uuid_main) %>% 
    dplyr::select(uuid, enumerator)
  # Check number 2
  check_cause_death <- raw.died_member %>%
    dplyr::rename(uuid = uuid_died)%>% 
    dplyr::left_join(enum_df) %>% 
    dplyr::select(uuid, enumerator, sex_died, cause_death)%>%
    filter(sex_died == "m" & cause_death %in% cause_death_f) 
    

  if(nrow(check_cause_death)>0){
    checks_followups <- rbind(checks_followups,
                              make.logical.check.entry(check_cause_death, 2,
                                                       c("sex_died","cause_death"), # to provide all teh strategies
                                                       cols_to_keep = c(enumerator),"Respondent reported sex of dead person male and a cause of death related to female only.", F))
  }
}
#Check number 3


if("flag_negative_pt" %in% names(flag_mortality)) {
  check_negative_pt <- df_mortality_long %>%
    dplyr::select(uuid,enumerator,date_dc_date,date_recall_date,
           date_join_date,date_left_date,date_birth_date,
           date_death_date,person_time) %>%
    filter(person_time < 0)

  if(nrow(check_negative_pt)>0){
    checks_followups <- rbind(checks_followups,
                              make.logical.check.entry(check_negative_pt, 3,  c("date_dc_date","date_recall_date",
           "date_join_date","date_left_date","date_birth_date",
           "date_death_date"),
                                                       cols_to_keep = c(enumerator),"Respondent reported wrong dates leading to a negative person time", F))
  }
}
...
<h5>Number of detected entries with Respondent reported sex of dead person male and a cause of death related to female only:<strong> `r if(nrow(checks_followups)>0) {nrow(checks_followups %>% filter(check.id == 2)%>% distinct(uuid))} else {0}`</strong></h5>
<h5>Number of detected entries with Respondent reported wrong dates leading to a negative person time:<strong> `r if(nrow(checks_followups)>0) {nrow(checks_followups %>% filter(check.id == 3) %>% distinct(uuid))} else {0}`</strong></h5>
