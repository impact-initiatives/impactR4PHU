save.ph.integrated.tables <- function(
  df,
  df_cat,
  national_df,
  national_df_cat,
  ...
  if (use_template && mort) {
    ...
    wb <- openxlsx::loadWorkbook(
      "resources/ph_integrated_template_wo_mort.xlsx"
    )
  } else {
  ...
  if (mort) {
    openxlsx::conditionalFormatting(
      ...
      rule = '="Extremely high"',
      style = openxlsx::createStyle(bgFill = "#960000", fontColour = "white")
    )
    openxlsx::conditionalFormatting(
      wb,
      sheet = "Cat",
      ...
      rows = 3:(nrow(df_cat) + 4),
      rule = '="Very high"',
      style = openxlsx::createStyle(bgFill = "#FF0000", fontColour = "white")
    )
    openxlsx::conditionalFormatting(
      wb,
      sheet = "Cat",
      ...
      rows = 3:(nrow(df_cat) + 4),
      rule = '="High"',
      style = openxlsx::createStyle(bgFill = "#ED7D31", fontColour = "black")
    )
    openxlsx::conditionalFormatting(
      wb,
      sheet = "Cat",
      ...
      rows = 3:(nrow(df_cat) + 4),
      rule = '="Moderate"',
      style = openxlsx::createStyle(bgFill = "#FFE699", fontColour = "#833C0C")
    )
    openxlsx::conditionalFormatting(
      wb,
      sheet = "Cat",
      ...
      rows = 3:(nrow(df_cat) + 4),
      rule = '="Low"',
      style = openxlsx::createStyle(bgFill = "#C6E0B4", fontColour = "black")
    )
    openxlsx::conditionalFormatting(
      wb,
      sheet = "Cat",
    ...
    ## 20/15/10/5
    value <- quantile(df[[17]])[[2]] -
      (quantile(df[[17]])[[4]] - quantile(df[[17]])[[2]])
    if (value < 0) {
    ...
    cols <- c(14, 16)
    for (col in cols) {
      value <- quantile(df[[col]])[[2]] -
        (quantile(df[[col]])[[4]] - quantile(df[[col]])[[2]])
      if (value < 0) {
    ...
    for (i in 1:nrow(df)) {
      ...
      if (lower_bound >= 2) {
        style <- openxlsx::createStyle(fgFill = "#960000", fontColour = "white") # Red
      } else if (lower_bound > 1) {
        style <- openxlsx::createStyle(fgFill = "#FF0000", fontColour = "white") # Orange
      } else if (lower_bound > 0.75) {
        style <- openxlsx::createStyle(fgFill = "#ED7D31", fontColour = "black") # Yellow
        ...
        style <- openxlsx::createStyle(fgFill = "#C6E0B4", fontColour = "black") # Green
      }
    ...
    #national mort
    # Extract the lower bound of the confidence interval
    lower_bound_national <- as.numeric(stringr::str_extract(
      national_df$mort[1],
      "^[^ ]*"
    ))
    # Determine the style based on the value
    if (lower_bound_national >= 2) {
      style_national <- openxlsx::createStyle(
        fgFill = "#960000",
        fontColour = "white"
      ) # Red
