...
load.label_colname <- function(filename_tool, language = "English") {
  tool_colnames <- readxl::read_xlsx(
    filename_tool,
    sheet = "survey",
    col_types = "text"
  ) %>%
...
load.tool.survey <- function(filename_tool, keep_cols = F) {
  ...
  if (!keep_cols) {
    ...
    lang_code <- stringr::str_replace(
      stringr::str_replace(lang_code, "\\(", "\\\\("),
      "\\)",
      "\\\\)"
    )
    ...
    cols_to_keep <- cols[
      stringr::str_detect(
        cols,
        paste0("((label)|(hint)|(constraint_message))::", lang_code)
      ) |
  ...
  for (i in 1:nrow(tool.survey)) {
    ...
    if (stringr::str_detect(toolrow$type, "begin[ _]repeat")) {
      sheet_name <- toolrow$name
    } else if (stringr::str_detect(toolrow$type, "end[ _]repeat")) {
      # watch out for nested repeats (Why would you even want to do that?)
      sheet_name <- "main"
    } else if (
      stringr::str_detect(toolrow$type, "((end)|(begin))[ _]group", T)
    ) {
...
get.type <- function(variable) {
  ...
  res <- data.frame(name = variable) %>%
    left_join(
      select(tool.survey, name, q.type),
      by = "name",
      na_matches = "never"
    ) %>%
    mutate(
      q.type = ifelse(
        is.na(q.type) & stringr::str_detect(name, "/"),
        "select_multiple",
        q.type
      )
    )
  if (any(is.na(res$q.type))) {
    warning(paste(
      ...
      paste0(filter(res, is.na(q.type)) %>% pull(name), collapse = ", ")
    ))
  }
...
get.label <- function(variable) {
  ...
  res <- data.frame(name = variable) %>%
    left_join(
      select(tool.survey, name, !!sym(label_colname)),
      by = "name",
      na_matches = "never"
    )
...
get.choice.label <- function(choice, list, simplify = FALSE) {
  ...
  if (!list %in% tool.choices$list_name) {
    stop(paste("list", list, "not found in tool.choices!"))
  }

  res <- data.frame(name = unlist(choice)) %>%
    left_join(
      select(tool.choices, name, list_name, all_of(label_colname)) %>%
        filter(list_name == list),
      by = "name",
      na_matches = "never"
    )
  if (any(is.na(res[[label_colname]]))) {
    culprits <- paste0(
      filter(res, is.na(!!sym(label_colname))) %>%
        pull(name),
      collapse = ", "
    )
  ...
  if (simplify) {
    ...
    res_vec <- stringr::str_split(res_vec, e.g._pattern, n = 2, simplify = T)[,
      1
    ] %>%
      stringr::str_squish
  }
...
get.choice.list.from.name <- function(variable) {
  ...
  if (stringr::str_detect(variable, "/")) {
    variable <- stringr::str_split(variable, "/")[[1]][1]
  }
...
get.var.labels <- function() {
  ...
  var.labels <- var.labels %>%
    ...
    mutate(
      label.x = ifelse(is.na(label.x), name, label.x),
      label.full = ifelse(
        is.na(label.y),
        label.x,
        paste0(label.y, " / ", label.x)
      )
    ) %>%
...
get.select.db <- function() {
  ...
  select.questions <- tool.survey %>%
    ...
    mutate(
      q.type = as.character(lapply(type, split.q.type)),
...
get.other.db <- function() {
  ...
  df1 <- tool.survey %>%
    filter(stringr::str_ends(name, "_other"), type == "text") %>%
    rename(label = label_colname) %>%
...
get.trans.db <- function(include.all = c()) {
  ...
  df1 <- tool.survey %>%
    filter(
      type == "text" &
        (!(stringr::str_ends(name, "_other")) | name %in% include.all)
    ) %>%
    rename(label = label_colname) %>%
    select("name", "label")
  return(df1)
