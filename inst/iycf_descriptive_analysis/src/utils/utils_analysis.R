...
detect.outliers <- function(df, n.sd, method = "o1") {
  res <- data.frame()
  for (col in colnames(df)[2:length(colnames(df))]) {
    df.temp <- data.frame(id = df$id, value = as.numeric(df[[col]])) %>%
      filter(!is.na(value) & value > 0) %>%
      mutate(
        col.log = log10(value),
        is.outlier = case_when(
          method == "o1" ~
            ifelse(
              col.log >
                mean(col.log, na.rm = T) + n.sd * sd(col.log, na.rm = T) |
                col.log <
                  mean(col.log, na.rm = T) - n.sd * sd(col.log, na.rm = T),
              T,
              F
          ...
          method == "o2" ~
            ifelse(
              col.log > 1.5 * quantile(col.log, 0.95) |
                col.log < quantile(col.log, 0.05) / 1.5,
              T,
              F
            ),
          method == "o3" ~
          ...
          method == "o4" ~
            ifelse(
              col.log > median(col.log) + 3 * mad(col.log) |
                col.log < median(col.log) - 3 * mad(col.log),
              T,
              F
            )
...
generate.boxplot <- function(outliers) {
  ...
  f.colour <- function(x) {
    return(ifelse(x == 2, "#FF0000", ifelse(x == 1, "#FFCCCC", "#00FF00")))
  }
  g <- ggplot2::ggplot(data.boxplot) +
    ggplot2::geom_boxplot(aes(x = q_k7, y = log.value)) +
    ylab("Values (log10)") +
    ggplot2::geom_point(
      aes(x = q_k7, y = log.value),
...
choice.name2label <- function(list.name, name) {
  return(as.character(tool.choices[
    tool.choices$list_name == list.name &
      tool.choices$name == name,
    label_colname
  ]))
}
...
choice.label2name <- function(list.name, label) {
  return(as.character(tool.choices[
    tool.choices$list_name == list.name &
      tool.choices[label_colname] == label,
    "name"
  ]))
}
#---
...
load_entry <- function(daf_row) {
  ...
  entry$disaggregate.variables <- c(stringr::str_split(
    sub(" ", "", entry$disaggregations),
    ";",
    simplify = T
  ))
  # omit_na is True by default (calculation empty):
  entry$omit_na <- is.na(entry$calculation) ||
    stringr::str_detect(entry$calculation, "include[_-]na", negate = T)
  # add_total is False by default
  entry$add_total <- !is.na(entry$calculation) &&
    stringr::str_detect(entry$calculation, "add_total")
  ...
  if ((col %in% tool.survey$name)) {
    if (get.type(col) == "select_one") {
      choices <- tool.choices %>%
        filter(list_name == get.choice.list.from.name(col)) %>%
      ...
      if (omit_na) {
        ...
        return(factor(
          d$label,
          levels = append(choices$label, NA),
          exclude = NULL
        ))
      }
    } else if (get.type(col) == "integer" | get.type(col) == "decimal") {
      return(as.numeric(df[[col]]))
    } else if (get.type(col) == "date") {
      return(as.character(as.Date(convertToDateTime(as.numeric(df[[col]])))))
    } else {
      return(df[[col]])
    }
  ...
  for (r in 1:nrow(dap)) {
      ...
      if (stringr::str_detect(entry$calculation, "include_na")) {
        if (!is.na(q$relevant)) {
          stop(paste0(
            ...
            " (",
            col,
            "): flag include_na cannot be set if question has relevancy!"
          ))
        }
        if (entry$func == "mean") {
          stop(paste0(
            ...
            " (",
            col,
            "): flag include_na cannot be set if func == mean!"
          ))
        }
      }
    }
    ...
    if (!all(is.na(entry$disaggregate.variables))) {
      for (disagg.var in entry$disaggregate.variables) {
        ...
        if (!disagg.var %in% tool.survey$name) {
          warning(paste(
            "Disaggregation variable",
            disagg.var,
            "not found in tool.survey!\n"
          ))
        }
    ...
    if (entry$func == "select_multiple") {
      if (get.type(col) != "select_multiple") {
        stop(paste0(
          ...
          " (",
          col,
          "): func is 'select_multiple', but question type is ",
          get.type(col)
        ))
      }

      choice_cols <- colnames(df)[stringr::str_starts(
        colnames(df),
        paste0(col, "/")
      )]
    ...
    if (stringr::str_starts(get.type(col), "select_multiple")) {
      ...
      col.labels <- as.character(lapply(col, function(x) {
        if (!is.na(x)) {
          ...
          l.labels <- lapply(l, function(l_elem) {
            as.character(
              tool.choices[
                tool.choices$list_name == q.list_name &
                  tool.choices$name == l_elem,
                label_colname
              ]
            )
          })
...
name2label <- function(df) {
  ...
  for (i in 1:length(col.names)) {
    ...
    df.label[[colnames(df.label)[i]]] <- name2label.select_multiple.values(
      df,
      col.names[i]
    )
  }
