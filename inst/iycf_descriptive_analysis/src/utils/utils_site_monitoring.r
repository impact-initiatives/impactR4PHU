...
recode.new.site <- function(raw.data, x, f) {
  ...
  cl <- data.frame(
    uuid = x$uuid,
    variable = "site_pCode",
    issue = "Recoding an identified new site.",
    old.value = pull(raw.data %>% filter(uuid == x$uuid), site_pCode),
    new.value = f()
  )
...
create.gis.checks <- function(data, cols_to_keep) {
  gis.check.df <- data %>%
    filter(site_pCode != "new") %>%
    select(all_of(cols_to_keep)) %>%
    arrange(site_pCode) %>%
    relocate(
      all_of(colnames(data)[str_starts(
        colnames(data),
        "(site_Add?ress)|(admin)"
      )]),
      .after = last_col()
    ...
    mutate(
      "TRUE NEW site (provide a name)" = NA,
...
save.gis.checks <- function(df, wb_name, blue_cols = NULL) {
  ...
  style.col.green.bold <- createStyle(
    textDecoration = "bold",
    fgFill = "#E5FFCC",
    valign = "top",
    border = "TopBottomLeftRight",
  ...
  for (sheet_name in sheet_names) {
    ...
    writeData(
      wb = wb,
      x = df %>% filter(what.to.clean == sheet_name),
      sheet = sheet_name,
      startRow = 1
    )
  ...
  tryCatch(
    {
      new.sites.df <- data %>%
        ...
        relocate(comments_text, .after = last_col()) %>%
        mutate(
          "TRUE NEW site (provide a better name if necessary)" = NA,
          "EXISTING (paste the correct pCode for the site)" = NA,
          "INVALID (provide an explanation)" = NA
        ) %>%
        relocate(uuid)
      ...
      if (random_color == "") {
        ...
        for (i in 1:ncol(df)) {
          addStyle(
            wb,
            "Sheet1",
            style = createStyle(fgFill = random_color, wrapText = T),
            rows = r:(r + 1),
            cols = i
          )
        }
