  ...
  for (r in 1:nrow(dafp)) {
      ...
      for (disagg.var in entry$disaggregate.variables) {
        if (!disagg.var %in% colnames(data.list[[sheet]])) {
          ...
          if (disagg.var %in% colnames(data.list$main)) {
            ...
            join_attempt <- data.list()$main %>%
              dplyr::select(uuid, !!sym(disagg.var))
            data.list[[sheet]] <- data.list[[sheet]] %>%
              left_join(join_attempt, by = "uuid")
            # cat(" success!")
          } else {
            stop(paste(
              "Disaggregation variable",
        ...
        data.list[[sheet]][[disagg.var]] <- as.character(data.list[[sheet]][[
          disagg.var
    ...
    if (entry$func == "select_multiple") {
      if (strings['sel_mul_sep'] == "/") {
        ...
        choice_cols <- colnames(data.list[[sheet]])[
          colnames(data.list[[sheet]]) %>% stringr::str_starts(paste0(col, "/"))
        ]
        data.list[[sheet]] <- data.list[[sheet]] %>%
        ...
        # not converting to label here. instead just replace "/" with "___" and convert to numeric
        choice_cols <- colnames(data.list[[sheet]])[
          colnames(data.list[[sheet]]) %>% stringr::str_starts(paste0(col, "_"))
        ]
        data.list[[sheet]] <- data.list[[sheet]] %>%
          mutate(across(all_of(choice_cols), as.numeric))
        ...
        # not converting to label here. instead just replace "/" with "___" and convert to numeric
        choice_cols <- colnames(data.list[[sheet]])[
          colnames(data.list[[sheet]]) %>%
            stringr::str_starts(paste0(col, "__"))
        ]
        data.list[[sheet]] <- data.list[[sheet]] %>%
          mutate(across(all_of(choice_cols), as.numeric))
      }
      if (!entry$omit_na) {
        ...
        data.list[[sheet]][[na_colname]] <- is.na(data.list[[sheet]][[col]]) %>%
          as.numeric
        data.list[[sheet]] <- data.list[[sheet]] %>%
          relocate(all_of(na_colname), .after = !!sym(col))
      }
    } else {
      if (entry$func == "select_one") {
        # try to convert to label:
        choice_names <- tool.choices %>%
          filter(list_name == entry$list_name) %>%
          pull(name)
        not_in_choices <- data.list[[sheet]] %>%
          filter(!(!!sym(col) %in% choice_names) & !isna(!!sym(col))) %>%
          pull(!!sym(col)) %>%
          unique
        if (length(not_in_choices) > 0) {
          conv_vec <- data.list[[sheet]][[col]]
        } else {
          if (!entry$list_name %in% tool.choices$list_name) {
            stop(paste("list", entry$list_name, "not found in tool.choices!"))
          }

          res <- data.frame(name = unlist(data.list[[sheet]][[col]])) %>%
            left_join(
              dplyr::select(tool.choices, name, list_name, label_colname) %>%
                filter(list_name == entry$list_name),
              by = "name",
              na_matches = "never"
            )
          if (nrow(res) == 0) {
            stop("All choices not in the list!")
          }
        ...
        entry$func %in% c("mean", "median", "integer", "numeric", "decimal")
      ) {
        data.list[[sheet]][[col]] <- as.numeric(data.list[[sheet]][[col]])
      }
    }
    ## Deal with disaggregated change to Label
    if (is.na(entry$disaggregate.variables)) {} else {
      disag.col <- entry$disaggregate.variables
      type.disag.col <- tool.survey %>%
        filter(name == disag.col) %>%
        pull(q.type)
      list_name_disag <- tool.survey %>%
        filter(name == disag.col) %>%
        pull(list_name)
      if (type.disag.col == "select_multiple") {
        data.list[[sheet]][[disag.col]] <- xml2label_choices_multiple(
          tool.survey,
          tool.choices,
          label_colname,
          data.list[[sheet]],
        ...
        if (type.disag.col == "select_one") {
          # try to convert to label:
          choice_names <- tool.choices %>%
            filter(list_name == list_name_disag) %>%
            pull(name)
          not_in_choices <- data.list[[sheet]] %>%
            filter(!(!!sym(disag.col) %in% choice_names)) %>%
            pull(!!sym(disag.col)) %>%
            unique %>%
            na.omit()
          if (length(not_in_choices) > 0) {
            ...
            if (!list_name_disag %in% tool.choices$list_name) {
              stop(paste("list", list_name_disag, "not found in tool.choices!"))
            }

            res <- data.frame(
              name = unlist(data.list[[sheet]][[disag.col]])
            ) %>%
              left_join(
                dplyr::select(tool.choices, name, list_name, label_colname) %>%
                  filter(list_name == list_name_disag),
          ...
          type.disag.col %in%
            c("mean", "median", "integer", "numeric", "decimal")
        ) {
          data.list[[sheet]][[disag.col]] <- as.numeric(data.list[[sheet]][[
            disag.col
          ]])
        }
      }
