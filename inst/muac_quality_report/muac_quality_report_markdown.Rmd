...
```{r, results='asis'}
  ...
  cat(paste0(paste0(rep("\n",2), collapse=""), paste0(rep("#",4), collapse=""), " ", "MUAC SECTION"))
  cat('\nThis section includes:\n\n- Overall Plausibility Report / By Enumerator\n- All the flags related to MUAC (details shown for each flag in the section)\n- Plots showing the distribution of the data.')
...
```{r, results='asis'}
result_muac_pivot_enum <- result_muac_enum %>%
  dplyr::mutate_all(., as.character) %>%
  ...
  list_enum <- result_muac_pivot_enum[[enumerator]] %>% unique
  
  for (i in 1:length(list_enum)) {
    cat(paste0(paste0(rep("\n",2), collapse=""), paste0(rep("#",5), collapse=""), " ", "<strong>",i,"</strong>"))
    
    overall_muac_plaus_enum <- overall_muac_plaus %>%
      dplyr::left_join(result_muac_pivot_enum %>% 
                         dplyr::filter(!!rlang::sym(enumerator) == list_enum[[i]]), by = c("flag_name"="Criteria")) %>%
      dplyr::mutate_all(., ~ifelse(is.na(.),"NA",.)) %>%
