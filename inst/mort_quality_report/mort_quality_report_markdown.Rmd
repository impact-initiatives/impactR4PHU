...
if(!file.exists("inputs/environment.Rdata")) {
  ...
  raw.left_member <- raw.left_member %>%
    dplyr::mutate(!!rlang::sym(birthdate_left) := ifelse(is.na(!!rlang::sym(birthdate_left)), NA,
                  ...
                  !!rlang::sym(joined_date_left) := ifelse(is.na(!!rlang::sym(joined_date_left)), NA,
                  ...
                  !!rlang::sym(left_date_left) := ifelse(is.na(!!rlang::sym(left_date_left)), NA,
  ...
  raw.died_member <- raw.died_member %>%
    dplyr::mutate(!!rlang::sym(birthdate_died) := ifelse(is.na(!!rlang::sym(birthdate_died)), NA,
                  ...
                  !!rlang::sym(date_death) := ifelse(is.na(!!rlang::sym(date_death)), NA,
                  ...
                  !!rlang::sym(joined_date_died) := ifelse(is.na(!!rlang::sym(joined_date_died)), NA,
...
if(!file.exists("inputs/environment.Rdata")) {
  ...
  population_estimation <- as.numeric(svDialogs::dlg_input(message= "Enter the total population of the study area",10000)$res)
}
...
list_of_var <- c("date_dc","date_recall_event","enumerator","admin1","admin2","cluster",
                 ...
                 "path.sheet.with.roster","path.sheet.with.leavers","path.sheet.with.death","population_estimation")
...
results_mort_enum <- impactR4PHU::create_mortality_plaus(df_mortality = flag_mortality,
                                                         exp_sex_ratio = exp_sex_ratio,
                                                         exp_ratio_0_4 = exp_ratio_0_4,
                                                         exp_ratio_2_5 = exp_ratio_2_5,
                                                         exp_ratio_5_10 = exp_ratio_5_10,
...
raw.flag.mort <- flag_mortality %>% 
  ...
  dplyr::summarise(flag_cause_death = sum(flag_cause_death, na.rm = T),
                   flag_multiple_death = sum(flag_multiple_death, na.rm = T))
...
subch(table_djl_enum)
```
```{r}
if(as.numeric(population_estimation) > 10000){
  res <- srvyr::as_survey_design(df_mortality_long)
} else {
  res <- srvyr::as_survey_design(df_mortality_long %>% 
                                   dplyr::mutate(fpc = nrow(raw.hh_roster)/as.numeric(population_estimation)),
                                 fpc = fpc)
}
...
under5 <- res %>%
  srvyr::summarise(x = srvyr::survey_ratio(numerator = under_5 * 10000,
