...
if(!file.exists("inputs/environment.Rdata")) {
  ...
  raw.hh_roster <- raw.hh_roster %>% 
    dplyr::mutate(joined = ifelse(joined == "yes","no","yes"))
}
...
### TO CONTINUE HERE
min_filter <- svDialogs::dlg_input(message= paste0("Enter the start date (format: YYYY-MM-DD). Can't be less than: ", min_date),min_date)$res

max_filter <- svDialogs::dlg_input(message= paste0("Enter the end date (format: YYYY-MM-DD). Can't be more than: ", max_date),max_date)$res
...
while(!stringr::str_detect(min_filter,"^\\d{4}\\-(0?[1-9]|1[012])\\-(0?[1-9]|[12][0-9]|3[01])$") | 
    ...
    min_filter <- svDialogs::dlg_input(message= paste0("[INCORRECT] Re-enter the start date (format: YYYY-MM-DD). Can't be less than: ", min_date),min_date)$res
}
...
while(!stringr::str_detect(max_filter,"^\\d{4}\\-(0?[1-9]|1[012])\\-(0?[1-9]|[12][0-9]|3[01])$")| 
    ...
    max_filter <- svDialogs::dlg_input(message= paste0("[INCORRECT] Re-enter the end date (format: YYYY-MM-DD). Can't be more than: ", max_date),max_date)$res
}
...
if(!file.exists("inputs/environment.Rdata")) {
  ...
  df_mortality_long <- df_mortality_long %>% dplyr::mutate(death_cause_smart = "")
  ...
  df_mortality_long <- df_mortality_long %>% dplyr::mutate(death_location_smart = "")
  ...
  if(length(setdiff(location_codes, ideal_codes))==0) {
    ...
    during_migration <- tcltk::tk_select.list(cause_codes, title = "During Migration Options  [LOCATION OF DEATH]", multiple = T)
    last_place <- tcltk::tk_select.list(cause_codes, title = "Last Place Residence Options [LOCATION OF DEATH]", multiple = T)
    other_place <- tcltk::tk_select.list(cause_codes, title = "Other Place Options [LOCATION OF DEATH]", multiple = T)
  }
}
...
df_mortality_long <- df_mortality_long  %>% 
  dplyr::mutate(sex = ifelse(is.na(sex),NA,
    ...
    dplyr::mutate(date_recall_date = lubridate::parse_date_time(date_recall, orders = date_recall_reformat)) %>%
    ...
    dplyr::mutate(date_join_date = lubridate::parse_date_time(date_join, orders = date_join_reformat)) %>%
    ...
    dplyr::mutate(date_left_date = lubridate::parse_date_time(date_left, orders = date_left_reformat)) %>%
    ...
    dplyr::mutate(date_birth_date = lubridate::parse_date_time(date_birth, orders = date_birth_reformat)) %>%
    ...
    dplyr::mutate(date_death_date = lubridate::parse_date_time(date_death, orders = date_death_reformat)) %>%
  ...
  dplyr::mutate(death_cause_smart = ifelse(is.na(death_cause),NA,
                                           ifelse(death_cause %in% unknown, 1,
                                                  ifelse(death_cause %in% injury_trauma, 2, 
                                                         ifelse(death_cause %in% illness, 3, death_cause_smart))))) %>% 
   dplyr::mutate(death_location_smart = ifelse(is.na(death_location), NA,
...
list_of_var <- c("date_dc","date_recall_event","enumerator","admin1","admin2","cluster",
                 ...
                 "df_mortality_long","flag_mortality", "label_colname","path.sheet.with.main",
...
impactR4PHU::plot_age_distribution(raw.hh_roster %>% 
                                     dplyr::left_join(select(raw.main, c(uuid_main, enumerator))),
                                   year_or_month = "month",
                                   by_group = "enumerator",
                                   breaks = 1)
```
