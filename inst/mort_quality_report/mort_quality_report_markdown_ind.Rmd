...
if(!file.exists("inputs/environment.Rdata")) {
  ...
  if(collected_df_left){
    ...
    birth_died <- names(raw.died_member)[grepl("born|dob|birth",names(raw.died_member))]
...
df_mortality_long <- impactR4PHU::create_mortality_long_df(df_main = raw.main,
  ...
  if(collected_df_left){
    list_of_var <- c("date_dc","date_recall_event","enumerator","admin1","admin2","cluster",
                     ...
                     "current_location","during_migration","last_place","other_place","smart")
  } else {
    list_of_var <- c("date_dc","date_recall_event","enumerator","admin1","admin2","cluster",
                     "uuid_main","uuid_roster","sex_roster","age_roster","joined_roster","joined_date_roster",
                     ...
                     "exp_sex_ratio","exp_ratio_0_4","exp_ratio_2_5","exp_ratio_5_10","exp_hh_size",
                     "label_colname","path.sheet.with.main","collect_num_join_left","num_join","num_left",
                     "path.sheet.with.roster","path.sheet.with.death", "dates_collected","age_roster_month",
                     "male","female","date_dc_reformat","date_recall_reformat","date_join_reformat",
                     "date_birth_reformat","date_left_reformat","date_death_reformat","yes_no_team","team",
...
if(yes_no_team == "yes"){
  for (i in list_teams) {
    ...
    for (j in list_enum) {
      cat(paste0(paste0(rep("\n",2), collapse=""), paste0(rep("#",6), collapse=""), " ", "<strong>",j,"</strong>"))
        if(collect_num_join_left == "yes") {
          results_mort_enum <- impactR4PHU::create_mortality_plaus(df_mortality_long %>%
                                                                     dplyr::filter(enumerator == j,
                                                                                   !!rlang::sym(team) == i),
                                                                   df_main = raw.main%>% 
                                                                     dplyr::filter(!!rlang::sym(enumerator) == j,
                                                                                   !!rlang::sym(team) == i),
      ...
      overall_mort_plaus_enum <- overall_mort_plaus %>%
        dplyr::left_join(result_mort_pivot_enum, by = c("flag_name"="Criteria")) %>%
        dplyr::mutate_all(., ~ifelse(is.na(.),"NA",.)) %>%
        dplyr::select(-group)
 # %>% filter(group == j)
...
if(yes_no_team == "yes"){
  ...
  for(flag in flags_mort_columns) {
    ...
    flag_by_enum <- flag_mortality %>%
      dplyr::select(enumerator,!!rlang::sym(team), !!rlang::sym(flag)) %>%
      filter(!is.na(!!rlang::sym(flag))) %>%
      dplyr::mutate(flag := flag) %>%
      dplyr::group_by(flag,enumerator, !!rlang::sym(team)) %>%
      dplyr::summarise(num_samples = n(),
                flagged = sum(!!rlang::sym(flag))) %>%
