...
if(!file.exists("inputs/environment.Rdata")) {
  if ("HHS" %in% FSL_indicators){
    ...
    if(!all(c(hhs_check_columns,hhs_check_columns_freq) %in% names(raw.main))) {
      ...
      rarely_answer <- tcltk::tk_select.list(unique(unlist(raw.main[,hhs_check_columns_freq])) %>% unique, title = "Rarely Value")
      sometimes_answer <- tcltk::tk_select.list(unique(unlist(raw.main[,hhs_check_columns_freq])) %>% unique, title = "Sometimes Value")
      often_answer <- tcltk::tk_select.list(unique(unlist(raw.main[,hhs_check_columns_freq])) %>% unique, title = "Often Value")
      raw.main <- raw.main %>%
...
if(!file.exists("inputs/environment.Rdata")) {
  ...
  income_types <- names(raw.main)[grepl("income",names(raw.main))]
    ...
    income_types <- tcltk::tk_select.list(income_types, title = "Income Types column", multiple = TRUE)
    if(income_types == ""){
      income_types <- svDialogs::dlg_input(message= "Enter the name of the income_types","income_types")$res
    }
