...
language <- tcltk::tk_select.list(grepl("label"),names(tool.survey), title = "Label column to choose", multiple = F)
...
for(flag in flags_fsl_columns) {
  ...
  flag_overall <- raw.flag.fcs %>%
    dplyr::select(group, !!rlang::sym(flag)) %>%
