...
if (length(ids) > 0) {
  cat("Duplicate uuids detected: ", length(ids))
}
# add to deletion log
deletion.log.duplicate <- create.deletion.log(
  raw.main %>% filter(uuid %in% ids),
  enum_colname,
  "Duplicate"
) # a brand new deletion log
rm(ids)
...
raw.hh_roster <- raw.hh_roster[
  !(raw.hh_roster$uuid %in% deletion.log.fast$uuid),
]
raw.ind_health <- raw.ind_health[
  !(raw.ind_health$uuid %in% deletion.log.fast$uuid),
]
if (!is.null(raw.water_count_loop)) {
  raw.water_count_loop <- raw.water_count_loop[
    !(raw.water_count_loop$uuid %in% deletion.log.fast$uuid),
  ]
}
raw.child_nutrition <- raw.child_nutrition[
  !(raw.child_nutrition$uuid %in% deletion.log.fast$uuid),
]
...
if (!is.null(raw.died_member)) {
  raw.died_member <- raw.died_member[
    !(raw.died_member$uuid %in% deletion.log.fast$uuid),
  ]
}
...
data.audit <- raw.main %>%
  mutate(
    duration_mins = abs(difftime(
      as.POSIXct(lubridate::ymd_hms(end)),
      as.POSIXct(lubridate::ymd_hms(start)),
      units = 'mins'
    )),
    num_NA_cols = rowSums(is.na(raw.main)),
    num_dk_cols = rowSums(raw.main == "dont_know", na.rm = T),
    num_other_cols = rowSums(
      !is.na(raw.main[stringr::str_ends(colnames(raw.main), "_other")]),
      na.rm = T
    )
  ) # %>%
...
audits.summary <- data.audit %>%
  left_join(audits.summary, by = "uuid") %>%
  select(-contains("/")) %>%
  relocate(
    uuid,
    duration_mins,
...
if (nrow(audits) == 0) {
  survey_durations_check <- audits.summary %>%
    filter(duration_mins < 5 | duration_mins > 60)
} else {
  survey_durations_check <- audits.summary %>% filter(tot.rt < 5 | tot.rt > 60)
}
if (nrow(survey_durations_check) > 0) {
  openxlsx::write.xlsx(
    survey_durations_check,
    paste0(
      "output/checking/audit/",
      dataset.name.short,
      "_survey_durations_",
      strings['out_date'],
      ".xlsx"
  ...
  if (nrow(audits) == 0) {
    survey_durations_check <- survey_durations_check %>%
      ...
      mutate(
        reason = ifelse(
          duration_mins < 5,
          "Submission time less than 10 mins",
          "Submission time more than 60 mins"
        )
      )
  } else {
    survey_durations_check <- survey_durations_check %>%
...
res.soft_duplicates <- find.similar.surveys(
  raw.main %>% filter(respondent_consent != "no"),
  tool.survey,
  uuid = "uuid"
) %>%
...
if (nrow(res.soft_duplicates) > 0) {
  ...
  res.soft_duplicates <- res.soft_duplicates %>%
    ...
    mutate(
      reason = "Soft duplicates to check - num different columns less than 12."
    )
}
...
if (!is.null(raw.hh_roster)) {
  ...
  loop_counts_main <- raw.main %>%
    select(uuid, !!sym(enum_colname), num_hh) %>%
    left_join(counts_loop1) %>%
    mutate(
      main_count = ifelse(num_hh == "999", NA, as.numeric(num_hh)),
...
if (!is.null(raw.water_count_loop)) {
  ...
  loop_counts_main <- raw.main %>%
    select(uuid, !!sym(enum_colname), wash_num_containers) %>%
    left_join(counts_loop3) %>%
    mutate(
      main_count = ifelse(
        wash_num_containers == "999",
        NA,
        as.numeric(wash_num_containers)
      ),
...
if (!is.null(raw.child_nutrition)) {
  ...
  loop_counts_main <- raw.main %>%
    select(uuid, !!sym(enum_colname), num_hh, num_children) %>%
    filter(as.numeric(num_children) > 0) %>%
    left_join(counts_loop4) %>%
    mutate(
      main_count = ifelse(num_hh == "999", NA, as.numeric(num_hh)),
...
if (!is.null(raw.died_member)) {
  ...
  loop_counts_main <- raw.main %>%
    select(uuid, !!sym(enum_colname), num_died) %>%
    left_join(counts_loop6) %>%
    mutate(
      main_count = ifelse(num_died == "999", NA, as.numeric(num_died)),
...
res_to_check <- res_to_check %>%
  mutate(remove_or_change = NA, loops_to_remove = NA, explanation = NA) %>%
  relocate(reason, .before = "remove_or_change")


save.deletion.requests(
  res_to_check,
  make.short.name("deletion_requests"),
  use_template = T
)
if (
  !is.null(raw.died_member) &
    !is.null(raw.water_count_loop) &
    !is.null(raw.women)
) {
  sheets <- list(
    "main" = raw.main,
...
writexl::write_xlsx(
  sheets,
  paste0(
    "output/data_log/data/",
    make.short.name("_data_with_loop_indexes"),
    ".xlsx"
  )
)
