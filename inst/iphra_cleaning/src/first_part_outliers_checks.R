...
cols.integer_raw.main <- cols.integer_main[
  cols.integer_main$name %in% colnames(raw.main),
] %>%
  pull(name)
cols.integer_raw.main <- cols.integer_raw.main[
  !stringr::str_detect(cols.integer_raw.main, "fsl_fcs_|fsl_rcsi_")
]
cols.integer_raw.water_count_loop <- cols.integer_main[
  cols.integer_main$name %in% colnames(raw.water_count_loop),
] %>%
  pull(name)
cols.integer_raw.child_nutrition <- cols.integer_main[
  cols.integer_main$name %in% colnames(raw.child_nutrition),
] %>%
  pull(name)
if (!is.null(raw.women)) {
  cols.integer_raw.women <- cols.integer_main[
    cols.integer_main$name %in% colnames(raw.women),
  ] %>%
    pull(name)
  ...
  if (length(cols.integer_raw.women) > 0) {
    for (col in cols.integer_raw.women) {
      values <- raw.women.outliers %>%
        ...
        rename(value = col) %>%
        select(uuid, loop_index, value) %>%
        mutate(value.log = log10(value)) %>%
        mutate(variable = col) %>%
        mutate(
          is.outlier.lin = (value > mean(value) + n.sd * sd(value)) |
            (value < mean(value) - n.sd * sd(value)),
          is.outlier.log = (value.log >
            mean(value.log) + n.sd * sd(value.log)) |
            (value.log < mean(value.log) - n.sd * sd(value.log))
        )
      values <- filter(values, is.outlier.log) %>%
        select(uuid, loop_index, variable, value)
      if (nrow(values) > 0) {
        print(paste0(col, ": ", nrow(values), " outliers detected"))
      }
    ...
    df <- raw.women.outliers %>%
      ...
      mutate(value.log = log10(value)) %>%
      left_join(
        select(res.outliers_raw.women, -value) %>% mutate(is.outlier = T),
        by = c("uuid", "variable")
      ) %>%
    ...
    openxlsx::write.xlsx(
      df.all,
      paste0(
        "output/checking/outliers/women_outlier_prices_analysis_",
        n.sd,
        "sd.xlsx"
      ),
      overwrite = T
    ...
    g.outliers_women <- ggplot2::ggplot(df.all) +
      ggplot2::geom_boxplot(
        ggplot2::aes(x = measure, y = value.log),
        width = 0.2
      ) +
      ggplot2::ylab("Values (log10)") +
      ggplot2::geom_point(
        ggplot2::aes(x = measure, y = value.log, group = measure),
        alpha = f.alpha(df.all$is.outlier),
        colour = "red"
      ) +
    ...
    if (nrow(res.outliers_raw.women) > 0) {
      ...
      cleaning.log.outliers <- rbind(
        cleaning.log.outliers,
        res.outliers_raw.women
      )
    }
  }
}
...
if (!is.null(raw.died_member)) {
  cols.integer_raw.died_member <- cols.integer_main[
    cols.integer_main$name %in% colnames(raw.died_member),
  ] %>%
    pull(name)
  ...
  if (length(cols.integer_raw.died_member) > 0) {
    ...
    df <- raw.died_member.outliers %>%
      ...
      mutate(value.log = log10(value)) %>%
      left_join(
        select(res.outliers_died_member, -value) %>% mutate(is.outlier = T),
        by = c("uuid", "variable")
      ) %>%
      mutate(is.outlier = ifelse(is.na(is.outlier), F, is.outlier)) %>%
      filter(!is.na(value) & value > 0)
    ...
    openxlsx::write.xlsx(
      df.all,
      paste0(
        "output/checking/outliers/died_member_outlier_prices_analysis_",
        n.sd,
        "sd.xlsx"
      ),
      overwrite = T
...
if (length(cols.integer_raw.main) > 0) {
  ...
  df <- raw.main.outliers %>%
    ...
    mutate(value.log = log10(value)) %>%
    left_join(
      select(res.outliers_main, -value) %>% mutate(is.outlier = T),
      by = c("uuid", "variable")
    ) %>%
    mutate(is.outlier = ifelse(is.na(is.outlier), F, is.outlier)) %>%
    filter(!is.na(value) & value > 0)
  ...
  openxlsx::write.xlsx(
    df.all,
    paste0(
      "output/checking/outliers/main_outlier_prices_analysis_",
      n.sd,
      "sd.xlsx"
    ),
    overwrite = T
  ...
  if (length(cols.integer_raw.water_count_loop) > 0) {
    for (col in cols.integer_raw.water_count_loop) {
      values <- raw.water_count_loop.outliers %>%
        ...
        mutate(variable = col) %>%
        mutate(
          is.outlier.lin = (value > mean(value) + 3 * sd(value)) |
      ...
      res.outliers_water_count_loop <- rbind(
        res.outliers_water_count_loop,
        values
      )
    }
    ...
    df <- raw.water_count_loop.outliers %>%
      ...
      mutate(value.log = log10(value)) %>%
      left_join(
        select(res.outliers_water_count_loop, -value) %>%
          mutate(is.outlier = T),
        by = c("uuid", "variable")
      ) %>%
      mutate(is.outlier = ifelse(is.na(is.outlier), F, is.outlier)) %>%
    ...
    openxlsx::write.xlsx(
      df.all,
      paste0(
        "output/checking/outliers/water_count_loop_outlier_prices_analysis_",
        n.sd,
        "sd.xlsx"
      ),
      overwrite = T
...
if (length(cols.integer_raw.child_nutrition) > 0) {
  ...
  df <- raw.child_nutrition.outliers %>%
    ...
    mutate(value.log = log10(value)) %>%
    left_join(
      select(res.outliers_child_nutrition, -value) %>% mutate(is.outlier = T),
      by = c("uuid", "variable")
    ) %>%
    mutate(is.outlier = ifelse(is.na(is.outlier), F, is.outlier)) %>%
    filter(!is.na(value) & value > 0)
  ...
  openxlsx::write.xlsx(
    df.all,
    paste0(
      "output/checking/outliers/child_nutrition_outlier_prices_analysis_",
      n.sd,
      "sd.xlsx"
    ),
    overwrite = T
...
if (nrow(cleaning.log.outliers) > 0) {
  ...
  if (language_assessment == "English") {
    ...
    cat(
      "The outliers check are done. Please go to output/checking/requests/ and check the file with \nthe name outliers_requests and follow the instructions in the read me tab.\n"
    )
    cat(
      "#############################################################################################\n"
    )
  ...
  if (language_assessment == "English") {
    ...
    cat(
      "There were no outliers detected. You can rerun the same file with a \nlower number of SD to check if any outliers will be detected.\n"
    )
    cat(
      "#############################################################################################\n"
    )
