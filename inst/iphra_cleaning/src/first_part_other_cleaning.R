...
if (!is.null(raw.died_member)) {
  other.db.died_member <- other.db[
    other.db$name %in% colnames(raw.died_member),
  ]
}
...
other.responses <- rbind(
  find.responses(raw.main, other.db.main, values_to = "response"),
  find.responses(
    raw.hh_roster,
    other.db.hh_roster,
    values_to = "response",
...
if (strings['api'] == "No Api") {
  ...
  save.other.requests(
    create.translate.requests(other.db, other.responses.j, is.loop = T),
    paste0(dataset.name.short, "_other_requests_", strings["out_date"]),
    use_template = T
  )
} else {
  other.responses.j <- other.responses %>%
    translate.responses_iphra(
      api_key = as.character(strings['api_key']),
      api = as.character(strings['api']),
...
if (!is.null(raw.died_member)) {
  ...
  trans.responses <- find.responses(
    raw.died_member,
    trans.db,
    values_to = "response.",
    is.loop = T
  ...
  if (strings['api'] == "No Api") {
    trans.responses.j <- trans.responses %>%
      mutate(response_translate = NA)
    save.trans.requests(
      create.translate.requests(trans.db, trans.responses.j, is.loop = T),
      paste0(dataset.name.short, "_translate_requests_", strings["out_date"]),
      use_template = T
    )
  } else {
    trans.responses.j <- trans.responses %>%
