...
or.request <- readxl::read_excel(
  paste0(
    "output/checking/requests/",
    list.files("output/checking/requests/", "deletion_requests")
  ),
  sheet = "Sheet2",
  col_types = "text"
)
or.response <- readxl::read_excel(
  paste0(
    "output/checking/responses/",
    list.files("output/checking/responses/", "deletion_requests_edited")
  ),
  sheet = "Sheet2",
  col_types = "text"
)
if (nrow(or.response) > 0) {
  check <- or.response %>%
    dplyr::mutate(
      check = rowSums(is.na(select(
        or.response,
        c(remove_or_change, loops_to_remove)
      )))
    )
  ...
  if (nrow(check %>% filter(check <= 1)) == nrow(or.response)) {
      ...
      if ("loop_count" %in% names(delete_entries)) {
        if (nrow(delete_entries %>% filter(!is.na(loop_count))) > 0) {
          loop_delete <- delete_entries %>%
            ...
            mutate(
              loops_to_remove = stringr::str_remove(loops_to_remove, " ")
            ) %>%
            separate_rows(loops_to_remove, sep = ";")

          raw.hh_roster <- raw.hh_roster[
            !(raw.hh_roster$loop_index %in% loop_delete$loops_to_remove),
          ]
          raw.ind_health <- raw.ind_health[
            !(raw.ind_health$loop_index %in% loop_delete$loops_to_remove),
          ]
          if (!is.null(raw.water_count_loop)) {
            raw.water_count_loop <- raw.water_count_loop[
              !(raw.water_count_loop$loop_index %in%
                loop_delete$loops_to_remove),
            ]
          }
          raw.child_nutrition <- raw.child_nutrition[
            !(raw.child_nutrition$loop_index %in% loop_delete$loops_to_remove),
          ]
          if (!is.null(raw.women)) {
            raw.women <- raw.women[
              !(raw.women$loop_index %in% loop_delete$loops_to_remove),
            ]
          }
          if (!is.null(raw.died_member)) {
            raw.died_member <- raw.died_member[
              !(raw.died_member$loop_index %in% loop_delete$loops_to_remove),
            ]
          }
    ...
    if (nrow(change_entries) > 0) {
      ...
      cleaning.log.deletion <- rbind(
        cleaning.log.deletion,
        cleaning.log.loop_inconsitency
      )
    }
    ...
    if (nrow(deletion.change) > 0) {
      ...
      raw.hh_roster <- raw.hh_roster[
        !(raw.hh_roster$uuid %in% deletion.change$uuid),
      ]
      raw.ind_health <- raw.ind_health[
        !(raw.ind_health$uuid %in% deletion.change$uuid),
      ]
      if (!is.null(raw.water_count_loop)) {
        raw.water_count_loop <- raw.water_count_loop[
          !(raw.water_count_loop$uuid %in% deletion.change$uuid),
        ]
      }
      raw.child_nutrition <- raw.child_nutrition[
        !(raw.child_nutrition$uuid %in% deletion.change$uuid),
      ]
      if (!is.null(raw.women)) {
      ...
      if (!is.null(raw.died_member)) {
        raw.died_member <- raw.died_member[
          !(raw.died_member$uuid %in% deletion.change$uuid),
        ]
      }
