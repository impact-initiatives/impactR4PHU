...
# styles
style.col.blue <- openxlsx::createStyle(
  fgFill = "#CCE5FF",
  valign = "top",
...
style.col.green <- openxlsx::createStyle(
  fgFill = "#E5FFCC",
  border = "TopBottomLeftRight",
  borderColour = "#000000",
  valign = "top",
...
style.col.green.bold <- openxlsx::createStyle(
  textDecoration = "bold",
  fgFill = "#E5FFCC",
  valign = "top",
  border = "TopBottomLeftRight",
...
save.responses <- function(df, wb_name, or.submission = "") {
  ...
  style.col.green.first <- openxlsx::createStyle(
    textDecoration = "bold",
    fgFill = "#E5FFCC",
    valign = "top",
    border = "TopBottomLeftRight",
  ...
  style.col.green.first2 <- openxlsx::createStyle(
    textDecoration = "bold",
    fgFill = "#CCE5FF",
    valign = "top",
    border = "TopBottomLeftRight",
  ...
  openxlsx::addStyle(
    wb,
    "Sheet1",
    style = openxlsx::createStyle(wrapText = T, valign = "top"),
    rows = 1:(nrow(df) + 1),
    cols = 5
  )
  openxlsx::addStyle(
...
save.other.requests <- function(df, wb_name, use_template = F) {
  if (use_template) {
    wb <- openxlsx::loadWorkbook("resources/other_requests_template.xlsm")
  } else {
    wb <- openxlsx::createWorkbook()
  }
  ...
  openxlsx::addStyle(
    ...
    rows = 1:nrow(df) + 1,
    cols = which(colnames(df) == "choices.label")
  )
  openxlsx::addStyle(
    wb,
    "Sheet2",
    ...
    rows = 1:nrow(df) + 1,
    cols = which(colnames(df) == "full.label")
  )
  ...
  openxlsx::setColWidths(
    wb,
    "Sheet2",
    cols = 2:which(colnames(df) == "choices.label") - 1,
    widths = "auto"
  )
  openxlsx::setColWidths(
    wb,
...
save.deletion.requests <- function(df, wb_name, use_template = F) {
  if (use_template) {
    wb <- openxlsx::loadWorkbook("resources/deletion_requests_template.xlsm")
  } else {
    wb <- openxlsx::createWorkbook()
  }
  openxlsx::addWorksheet(wb, "Sheet2", zoom = 90)
  ...
  if ("variable" %in% names(df)) {
    dl_uuids_roster <- df %>%
      filter(
        variable == "num_hh" & stringr::str_starts(reason, "hh_roster")
      ) %>%
      pull(uuid)
    ...
    dl_uuids_health <- df %>%
      filter(
        variable == "num_hh" & stringr::str_starts(reason, "ind_health")
      ) %>%
      pull(uuid)
    ...
    for (i in 1:nrow(df)) {
      ...
      if (
        !is.na(df$variable[i]) &
          df$variable[i] == "num_hh" &
          stringr::str_starts(df$reason[i], "child")
      ) {
        uuid <- df$uuid[i]
      ...
      if (
        !is.na(df$variable[i]) &
          df$variable[i] == "num_hh" &
          stringr::str_starts(df$reason[i], "women")
      ) {
        uuid <- df$uuid[i]
      ...
      if (
        !is.na(df$variable[i]) &
          df$variable[i] == "num_hh" &
          stringr::str_starts(df$reason[i], "ind_health")
      ) {
        uuid <- df$uuid[i]
      ...
      if (
        !is.na(df$variable[i]) &
          df$variable[i] == "num_hh" &
          stringr::str_starts(df$reason[i], "hh_roster")
      ) {
        uuid <- df$uuid[i]
...
save.trans.requests <- function(
  ...
  if (use_template) {
    wb <- openxlsx::loadWorkbook("resources/trans_requests_template.xlsx")
  } else {
    wb <- openxlsx::createWorkbook()
  }
  ...
  openxlsx::addStyle(
    wb,
    "Sheet2",
    style = openxlsx::createStyle(textDecoration = "bold", valign = "bottom"),
    rows = 1,
    cols = 1:ncol(df),
    stack = T
  )
  ...
  openxlsx::addStyle(
    ...
    style = style.col.green,
    rows = 1:(nrow(df) + 1),
    cols = which(stringr::str_starts(colnames(df), "TRUE")),
    stack = T
  )
  openxlsx::addStyle(
    wb,
  ...
  openxlsx::addStyle(
    ...
    style = style.col.green,
    rows = 1:(nrow(df) + 1),
    cols = which(stringr::str_starts(colnames(df), "INVALID")),
    stack = T
  )
  openxlsx::addStyle(
    wb,
...
save.outlier.responses <- function(df, wb_name, use_template = F) {
  if (use_template) {
    wb <- openxlsx::loadWorkbook("resources/outliers_requests_template.xlsx")
  } else {
    wb <- openxlsx::createWorkbook()
  }
...
save.follow.up.requests <- function(cleaning.log, data) {
  ...
  col.style <- openxlsx::createStyle(
    textDecoration = "bold",
    fgFill = "#CECECE",
    halign = "center",
    border = "TopBottomLeftRight",
  ...
  cl <- cl %>%
    dplyr::arrange(match(
      check.id,
      stringr::str_sort(unique(cl$check.id), numeric = T)
    ))
  for (i in country_list) {
    ...
    openxlsx::addStyle(
      wb,
      "Follow-up",
      style = openxlsx::createStyle(wrapText = T),
      rows = 1:(nrow(cl1) + 1),
      cols = 7
    )
    openxlsx::addStyle(
      ...
      for (r in 2:nrow(cl1)) {
        if (
          (!use.color(as.character(cl1[r, "check.id"])) &
            ...
            as.character(cl1[r, "check.id"]) ==
              as.character(cl1[r - 1, "check.id"])) |
            (use.color(as.character(cl1[r, "check.id"])) &
              as.character(cl1[r, "country"]) ==
                as.character(cl1[r - 1, "country"]) &
              as.character(cl1[r, "check.id"]) ==
                as.character(cl1[r - 1, "check.id"]))
        ) {
          if (random.color == "") {
    ...
    filename <- paste0(
      "output/checking/requests/",
      stringr::str_to_lower(i),
      "_follow_up_requests.xlsx"
    )
    openxlsx::saveWorkbook(wb, filename, overwrite = TRUE)
    rm(cl1)
  }
...
create.follow.up.requests <- function(
  checks.df,
  loop_data = NULL,
  wb_name,
  use_template = F
  ...
  # save follow-up requests
  if (use_template) {
    wb <- openxlsx::loadWorkbook("resources/logical_requests_template.xlsm")
  } else {
    wb <- openxlsx::createWorkbook()
  }
  ...
  openxlsx::addStyle(
    ...
    style = style.col.green,
    rows = 1:(nrow(cl) + 1),
    cols = which(colnames(cl) == "explanation")
  )
  openxlsx::addStyle(
    wb,
    "Follow-up",
    style = style.col.green,
    rows = 1:(nrow(cl) + 1),
    cols = which(colnames(cl) == "loops_to_remove")
  )
  openxlsx::addStyle(
    wb,
    "Follow-up",
    style = style.col.green,
    rows = 1:(nrow(cl) + 1),
    cols = which(colnames(cl) == "invalid")
  )
  openxlsx::addStyle(
    wb,
    "Follow-up",
    style = style.col.green,
    rows = 1:(nrow(cl) + 1),
    cols = which(colnames(cl) == "new.value")
  )
  openxlsx::addStyle(
    wb,
    "Follow-up",
    ...
    for (r in 2:nrow(cl)) {
      if (
        (!use.color(as.character(cl[r, "check.id"])) &
          ...
          as.character(cl[r, "check.id"]) ==
            as.character(cl[r - 1, "check.id"])) |
          (use.color(as.character(cl[r, "check.id"])) &
            as.character(cl[r, "check.id"]) ==
              as.character(cl[r - 1, "check.id"]))
      ) {
  ...
  ##Adding data validation
  for (i in 1:nrow(cl)) {
    if (stringr::str_detect(cl$variable[i], "/")) {
      next
    }
...
create.follow.up.requests.data.quality <- function(
  checks.df,
  loop_data = NULL,
  wb_name,
  use_template = F
  ...
  # save follow-up requests
  if (use_template) {
    wb <- openxlsx::loadWorkbook("./../resources/daily_requests.xlsx")
  } else {
    wb <- openxlsx::createWorkbook()
  }
...
create.follow.up.requests.daily <- function(
  checks.df,
  loop_data = NULL,
  wb_name,
  use_template = F
...
load.requests <- function(
  ...
  #'
  file.type = stringr::str_squish(stringr::str_replace_all(
    filename.pattern,
    "[^a-zA-Z]+",
    " "
  ))
  ...
  if (length(filenames) == 0) {
    ...
    cat(
      paste("Loading", length(filenames), file.type, "requests files:\n"),
    ...
    for (c in c_tei_cols) {
      colnames(res)[stringr::str_starts(
        colnames(res),
        stringr::str_to_upper(c)
      )] <- paste0(c, '.v')
    }
    ...
    if (validate) {
      ...
      if (all(c_tei_cols %in% colnames(res))) {
        res <- res %>%
          mutate(check = rowSums(is.na(select(res, all_of(c_tei_cols)))))
        check.res <- res %>%
          select(c(
...
load.edited <- function(dir.edited, file.type) {
  ...
  filenames <- list.files(
    ...
    pattern = paste0(
      ".*",
      file.type,
      "((responses)|(requests))?(_edited)?.*\\.xlsx$"
    )
  )
  if (length(filenames) == 0) {
    ...
    cat(
      paste("Loading", length(filenames), file.type, "logs:\n"),
      paste(filenames, collapse = "\n "),
      "\n"
    )
    res <- data.frame()
    for (filename in filenames) {
      # load file
      other <- readxl::read_xlsx(filename, col_types = "text") %>%
...
load.logic.request <- function(dir.requests) {
  logic.filenames <- list.files(
    dir.requests,
    pattern = "follow_up_requests",
    recursive = FALSE,
    full.names = TRUE
  )
  cat(
...
load.outlier.edited <- function(dir.outlier.edited) {
  logic.filenames <- list.files(
    dir.outlier.edited,
    pattern = "outliers_responses.xlsx",
    recursive = TRUE,
    full.names = TRUE
  )
  cat(
    paste("Loading", length(logic.filenames), "outlier logs:\n"),
    paste(logic.filenames, collapse = "\n "),
    "\n"
  )
  res <- data.frame()
...
recode.set.NA.if <- function(data, variables, code, issue, ignore_case = T) {
  ...
  for (variable in variables) {
    if (ignore_case) {
      data1 <- data %>%
        filter(
          stringr::str_to_lower(!!sym(variable)) %in%
            stringr::str_to_lower(code)
        )
    } else {
...
recode.set.NA.regex <- function(data, variables, pattern, issue) {
  ...
  for (variable in variables) {
    data1 <- data %>%
      filter(stringr::str_detect(!!sym(variable), pattern = pattern))
    cl <- data1 %>%
      mutate(
        variable = variable,
        old.value = !!sym(variable),
...
recode.multiple.set.NA <- function(data, variable, issue) {
  ...
  ccols <- colnames(data)[stringr::str_starts(
    colnames(data),
    paste0(variable, "/")
  )]
  ...
  if (nrow(data) > 0) {
    cl_cummulative <- data %>%
      select(any_of(c("uuid", "loop_index", variable))) %>%
      mutate(
        variable = variable,
        old.value = !!sym(variable),
        new.value = NA,
    ...
    for (col in ccols) {
      ...
      if (nrow(df) > 0) {
        ...
        if (stringr::str_ends(col, "/other")) {
          cl_other_text <- df %>%
            filter(!is.na(!!sym(paste0(variable, "_other")))) %>%
            mutate(
              variable = paste0(variable, "_other"),
              old.value = !!sym(paste0(variable, "_other")),
              new.value = NA,
              issue = issue
            ) %>%
            select(any_of(CL_COLS))
...
recode.multiple.set.choice <- function(data, variable, choice, issue) {
  ...
  if (!choice_column %in% colnames(data)) {
    stop(paste("Column", choice_column, "not present in data!"))
  }
  ...
  if (nrow(data) > 0) {
    ...
    cl_choices <- data %>%
      mutate(
        variable = choice_column,
        old.value = !!sym(choice_column),
        new.value = "1",
        issue = issue
      ) %>%
      select(any_of(CL_COLS))
    ...
    cols <- colnames(data)[
      stringr::str_starts(colnames(data), paste0(variable, "/")) &
...
recode.multiple.add.choices <- function(data, variable, choices, issue) {
  ...
  choices_len <- stringr::str_length(paste0(choices, collapse = "")) +
    length(choices)
  # filter out cases that already have all choices selected
  data <- data %>%
    select(
      any_of(c("uuid", "loop_index", variable)),
      all_of(choice_columns)
    ) %>%
    filter(!is.na(!!sym(variable))) %>%
    mutate(
      variable2 = stringr::str_squish(stringr::str_remove_all(
        !!sym(variable),
        choices_pattern
      ))
    ) %>%
    mutate(
      len_diff = stringr::str_length(!!sym(variable)) -
        stringr::str_length(variable2)
    ) %>%
    filter(
      stringr::str_length(!!sym(variable)) - stringr::str_length(variable2) !=
        choices_len
    )
  if (nrow(data) > 0) {
    cl_cummulative <- data %>%
      select(any_of(c("uuid", "loop_index", variable, "variable2"))) %>%
      rename(old.value = !!sym(variable)) %>%
      mutate(
        variable = variable,
    ...
    for (choice in choices) {
      choice_column <- paste0(variable, "/", choice)
      data1 <- data %>%
        filter(stringr::str_detect(!!sym(variable), choice, negate = T))
      if (nrow(data1) > 0) {
        cl_choice <- data1 %>%
          select(any_of(c("uuid", "loop_index"))) %>%
          mutate(
            variable = choice_column,
            old.value = "0",
            new.value = "1",
...
recode.multiple.add.choice <- function(data, variable, choice, issue) {
  ...
  if (nrow(data) > 0) {
    cl_cummulative <- data %>%
      ...
      mutate(
        variable = variable,
        new.value = stringr::str_squish(paste(old.value, choice)),
        issue = issue
      )
...
recode.multiple.remove.choice <- function(data, variable, choice, issue) {
  ...
  if (nrow(data) > 0) {
    cl_cummulative <- data %>%
      ...
      mutate(
        variable = variable,
        new.value = stringr::str_squish(stringr::str_remove(old.value, choice)),
        issue = issue
      )
  ...
  if (nrow(clog) == 0) {
    ...
    for (r in 1:nrow(clog)) {
      ...
      if (is.loop) {
        ...
        if (
          !suppress.diff.warnings &&
            data[data$loop_index == loop_index, variable] %!=na%
              clog$old.value[r]
        ) {
          warning(paste0(
            ...
            clog$old.value[r],
            "\t found: ",
            data[data$loop_index == loop_index, variable],
            "\tReplacing with: ",
            clog$new.value[r]
          ))
        }
        ...
        if (
          !suppress.diff.warnings &&
            data[data$uuid == uuid, variable] %!=na% clog$old.value[r]
        ) {
          warning(paste0(
            "Value in data is different than old.value in Cleaning log!\nUUID: ",
            uuid,
...
make.logical.check.entry <- function(
  check,
  id,
  question.names,
  cols_to_keep = c(),
...
add.to.cleaning.log.other.recode <- function(data, x) {
  ...
  if (x$ref.type[1] == "select_one") {
    res <- add.to.cleaning.log.other.recode.one(data, x)
  }
  if (x$ref.type[1] == "select_multiple") {
    res <- add.to.cleaning.log.other.recode.multiple(data, x)
  }
  if (res == "err") cat("Errors while recoding other. Check the warnings!\t")
}

add.to.cleaning.log <- function(
  checks,
  check.id,
  question.names = c(),
  issue = "",
  enumerator.code.col = "Staff_Name"
) {
  #' [obsolete]
  for (q.n in question.names) {
    new.entries <- checks %>%
    ...
    cleaning.log.checks <<- dplyr::arrange(
      rbind(cleaning.log.checks, new.entries),
      country,
      uuid
    )
  }
...
add.to.cleaning.log.other.remove <- function(data, x) {
  ...
  if (x$ref.type[1] == "select_multiple") {
    ...
    if (is.na(new.value)) {
      ...
      cols <- colnames(data)[stringr::str_starts(
        colnames(data),
        paste0(x$ref.name, "/")
      )]
      oldvalues <- data %>%
      ...
      df <- data.frame(
        uuid = x$uuid,
        variable = paste0(x$ref.name, "/other"),
        issue = issue,
        old.value = "1",
        new.value = "0"
      )
...
add.to.cleaning.log.other.recode.one <- function(data, x) {
  ...
  new.code <- filter(
    tool.choices,
    list_name == list.name & !!sym(label_colname) == choice
  )
  if (nrow(new.code) != 1) {
    warning(paste0(
      "Choice is not in the list. UUID: ",
      x$uuid,
      "; recode.into: ",
      choice
    ))
...
find.responses <- function(
  ...
  if (is.loop) {
    responses.j <- responses %>%
      left_join(questions.db, by = c("question.name" = "name")) %>%
      dplyr::rename(name = "question.name") %>%
...
translate.responses <- function(
  ...
  responses <- responses %>%
    mutate(resp_lower = stringr::str_to_lower(!!sym(values_from)))
  ...
  if (char_counter > threshold) {
    yes_no <- svDialogs::dlgInput(
      paste0(
        "The number of characters exceeds ",
        threshold,
        ". Please enter [YES] if you would like to proceed or [NO] to kill:"
      ),
      "YES or NO"
    )$res
  } else {
  ...
  batching <- svDialogs::dlgInput(
    paste0(
      "How many batches would you like to split your translation (",
      char_counter,
      " characters)? (please only integer)"
    ),
    0
  )$res
  ...
  if (yes_no == "YES") {
    if (length(input_vec) > 0) {
      for (code in language_codes) {
        ...
        ## create batches
        temp_resp_batches <- split(
          temp_resp,
          factor(sort(rank(row.names(temp_resp)) %% batching))
        )
        ...
        for (temp_resp_batch in temp_resp_batches) {
          tcltk::setTkProgressBar(
            ...
            paste0(
              ...
              batching,
              "\n",
              length(temp_resp_batch$input_vec),
              " responses will be translated from ",
              code,
              " to ",
              target_lang,
              "\nThis means ",
              sum(stringr::str_length(temp_resp_batch$input_vec)),
              " utf-8 characters."
            )
          )
          prog <- prog + 1
          ...
          # actual translation:
          result_vec <- NULL
          result_vec <- try(translateR::translate(
            content.vec = temp_resp_batch$input_vec,
            microsoft.api.key = source(
              "resources/microsoft.api.key_publichealth.R"
            )$value,
            microsoft.api.region = "switzerlandnorth",
            source.lang = code,
            target.lang = target_lang
          ))
          if (inherits(result_vec, "try-error")) {
            break
          }
          ...
          info_df <- rbind(
            ...
            data.frame(
              ...
              "translated_characters_num" = sum(stringr::str_length(
                temp_resp_batch$input_vec
              )),
              "language_from" = code,
        ...
        responses <- responses %>%
          left_join(temp_resp_whole, by = c("resp_lower" = "input_vec"))
      }
    } else {
...
create.translate.requests <- function(questions.db, responses.j, is.loop = F) {
  ...
  response_cols <- colnames(responses.j)[stringr::str_starts(
    colnames(responses.j),
    "response"
  )]
...
pull.raw <- function(uuids = NA, loop_indexes = NA) {
  ...
  if (all(is.na(loop_indexes))) {
    if (all(is.na(uuids))) {
      stop("Need to provide either uuids, or loop_indexes!")
    }
    uuids <- stringr::str_squish(uuids)
    return(raw.main %>% filter(uuid %in% uuids))
  } else {
    loop_indexes <- stringr::str_squish(loop_indexes)
    if (all(stringr::str_starts(loop_indexes, "loop1"))) {
      return(raw.loop1 %>% filter(loop_index %in% loop_indexes))
    } else if (all(stringr::str_starts(loop_indexes, "loop2"))) {
      return(raw.loop2 %>% filter(loop_index %in% loop_indexes))
    } else {
      # TODO: add additional loops if necessary
      stop("Referenced loop indexes belong to different loops!")
    }
  }
}
...
add.to.fu.requests <- function(checks, check.id) {
  new.entries <- filter(checks, flag) %>%
    mutate(check.id = check.id) %>%
    select(uuid, check.id)
...
name2label_question <- function(col) {
  if (stringr::str_detect(col, "/")) {
    ...
    c.name <- paste0(
      tail(stringr::str_split(col, "/")[[1]], -1),
      collapse = "/"
    )
  } else {
    q.name <- col
  ...
  if (q.name %in% tool.survey$name) {
    ...
    if (is.na(q.label) | q$q.type %in% c("note")) {
      q.label <- q.name
    }
    if (!is.na(c.name)) {
      q.list_name = ifelse(q$list_name == "NA", NA, q$list_name)
      c.label <- tool.choices[
        tool.choices$list_name == q.list_name & tool.choices$name == c.name,
        label_colname
      ]
    } else {
      c.label <- NA
