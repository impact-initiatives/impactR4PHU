...
generate.boxplot <- function(outliers) {
  ...
  g <- ggplot(data.boxplot) +
    geom_boxplot(aes(x = q_k7, y = log.value)) +
    ylab("Values (log10)") +
    geom_point(
...
load_entry <- function(daf_row) {
  ...
  entry$disaggregate.variables <- c(str_split(
  ...
  entry$omit_na <- is.na(entry$calculation) ||
    str_detect(entry$calculation, "include[_-]na", negate = T)
  # add_total is False by default
  entry$add_total <- !is.na(entry$calculation) &&
    str_detect(entry$calculation, "add_total")
