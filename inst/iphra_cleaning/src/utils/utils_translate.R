translate.responses_iphra <- function(
  responses,
  api_key = NULL,
  api = NULL,
  ...
  responses <- responses %>%
    mutate(resp_lower = str_to_lower(!!sym(values_from)))
        ...
        for (temp_resp_batch in temp_resp_batches) {
          tcltk::setTkProgressBar(
            ...
            paste0(
              ...
              target_lang,
              "\nThis means ",
              sum(str_length(temp_resp_batch$input_vec)),
              " utf-8 characters."
            )
          )
          prog <- prog + 1
          ...
          if (api == "Microsoft") {
            ...
            result_vec <- try(deeplr::translate2(
              text = temp_resp_batch$input_vec,
          ...
          info_df <- rbind(
            ...
            data.frame(
              ...
              "translated_characters_num" = sum(str_length(
