  ...
  for (filename in audit.filenames) {
    ...
    if (is.null(uuids) | uuid %in% uuids) {
      ...
      if (track.changes & "old-value" %in% colnames(audit)) {
        audit <- audit %>%
          rename("old.value" = `old-value`, "new.value" = `new-value`) %>%
...
process.uuid <- function(df) {
  ...
  for (r in 2:nrow(df)) {
    if (status == "filling" & df$event[r] == "form.exit") {
      ...
      e <- append(
        e,
        nrow(filter(questions, !is.na(`old.value`) & `old.value` != ""))
      )
      status <- "waiting"
      ...
      if ("uuid2" %in% colnames(df)) {
        ...
        warning("status=waiting while form.exit!")
      }
    }
  }
  res <- data.frame()
  ...
  res[1, "tot.rt"] <- round(
    sum(filter(df, event %in% c("question", "group.questions"))$duration) / 60,
    1
  )
...
find.similar.surveys <- function(
  data.main,
  tool.survey,
  uuid = "_uuid",
  staff_name_col = "Staff_Name",
  idnk_value = "idnk"
) {
...
check.soft.duplicates <- function(
  data.main,
  ids,
  uuid = "_uuid",
  only_differences = F
...
calculate.enumerator.similarity <- function(
  data,
  tool.survey,
  col_enum,
  col_admin = "adm"
  ...
  res <- data %>%
    ...
    lapply(function(gov) {
      ...
      if (length(unique(df$enum.id)) > 1) {
        ...
        gower_dist <- daisy(
          select(df, -c(!!sym(col_enum), enum.id)),
          metric = "gower",
          warnBin = F,
          warnAsym = F,
          warnConst = F
        ...
        # create output
        r <- data.frame(
          enum.id = as.numeric(names(res.si$clus.avg.widths)),
          si = res.si$clus.avg.widths
        ) %>%
          left_join(
            distinct(select(df, !!sym(col_admin), !!sym(col_enum), enum.id)),
            by = "enum.id"
          ) %>%
          left_join(
            group_by(df, enum.id) %>%
              summarise(num.surveys = n(), .groups = "drop_last"),
            by = "enum.id"
          ) %>%
