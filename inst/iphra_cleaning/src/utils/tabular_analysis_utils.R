...
make_table.select_one <- function(
  ...
  res.long <- srvyr.design.grouped %>%
    ...
    summarise(num_samples = n(), prop = survey_prop(na.rm = T, vartype = "var"))
  ...
  res.wide <- res.long %>%
    ...
    rename_with(~ str_remove(., "prop_"), starts_with("prop_"))
...
make_table.select_multiple <- function(
  ...
  s_props <- srvyr.design.grouped %>%
    summarise(across(
      .fns = list(prop = ~ survey_mean(., na.rm = T, vartype = "var"))
    ))
