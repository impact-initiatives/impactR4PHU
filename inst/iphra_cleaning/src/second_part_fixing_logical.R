...
if (is.data.frame(fu.edited)) {
  fu.edited <- fu.edited %>%
    mutate(
      modified = !is.na(invalid) | (!is.na(new.value) & new.value != old.value),
      missed = rowSums(is.na(across(c(new.value, invalid, loops_to_remove)))) ==
        3 &
        !is.na(old.value),
  ...
  fu.edited <- fu.edited %>%
    filter(modified) %>%
    mutate(
      new.value = ifelse(!is.na(invalid) & invalid == "yes", NA, new.value),
      issue = ifelse(
        !is.na(explanation),
        paste0("Information followed up with field team: ", explanation),
        issue
      )
    )
  ...
  if (any(is.na(fu.multiple$invalid))) {
    stop("Select multiples cannot be invalid. Set them to 0 instead!")
  }
  cleaning.log.followups_multiple <- tibble()

  if (nrow(fu.multiple) > 0) {
    main_row <- pull.raw(ifelse(
      is.na(fu.multiple[1, ]$loop_index),
      fu.multiple[1, ]$uuid,
      fu.multiple[1, ]$loop_index
    ))
    for (r in 1:nrow(fu.multiple)) {
      ...
      if (x$new.value == "0") {
        cl <- recode.multiple.remove.choices(
          main_row,
          cummulative_variable,
          cchoice,
          issue = x$issue
        )
        main_row <- main_row %>% apply.changes(cl)
        cleaning.log.followups_multiple <- rbind(
          cleaning.log.followups_multiple,
          cl
        )
      } else {
        cl <- recode.multiple.add.choices(
          main_row,
          cummulative_variable,
          cchoice,
          issue = x$issue
  ...
  cleaning.log.followups <- fu.edited %>%
    filter(get.type(variable) != "select_multiple") %>%
    select(any_of(CL_COLS))
  cleaning.log.followups <- rbind(
  ...
  raw.main <- raw.main %>%
    apply.changes(
      cleaning.log.followups %>% filter(variable %in% names(raw.main))
    )
  if (!is.null(raw.water_count_loop)) {
    raw.water_count_loop <- raw.water_count_loop %>%
      apply.changes(
        cleaning.log.followups %>%
          filter(variable %in% names(raw.water_count_loop)),
        is.loop = T
      )
  }
  raw.child_nutrition <- raw.child_nutrition %>%
    apply.changes(
      cleaning.log.followups %>%
        filter(variable %in% names(raw.child_nutrition)),
      is.loop = T
    )
  if (!is.null(raw.died_member)) {
    raw.died_member <- raw.died_member %>%
      apply.changes(
        cleaning.log.followups %>% filter(variable %in% names(raw.died_member)),
        is.loop = T
      )
  }
  ...
  if (!is.null(raw.child_nutrition)) {
    if (
      all(
        c("nut_edema_confirm", "nut_edema", "nut_muac_cm") %in%
          names(raw.child_nutrition)
      )
    ) {
      ...
      check <- raw.child_nutrition %>%
        filter(is.na(nut_edema_confirm)) %>%
        dplyr::mutate(
          flag = ifelse(nut_muac_cm >= 12.5 & !is.na(nut_cmam_enrollment), 1, 0)
        ) %>%
        filter(flag == 1)
  ...
  if (!is.null(raw.died_member)) {
    ...
    columns_loop_died <- c(
      ...
      "age_months_died",
      "age_days_died",
      "calc_final_age_years_died",
      "days_since_recall_died",
      "days_since_born_died",
      "days_recall_born_died",
      "ind_born_died",
      "died_present",
      "date_death_yn",
      "date_death_exact",
      ...
      "cause_death_other",
      "location_death",
      "location_death_other",
      "died_healthcare_yn",
      "died_healthcare_location",
      "died_healthcare_other",
      "died_no_healthcare",
      "died_no_healthcare_other",
      "death_details"
    )
    if (nrow(check) > 0) {
  ...
  if (language_assessment == "English") {
    ...
    cat(
      "Direct logical checks are cleaned. Please go ahead and run the outlier checks.\n"
    )
    cat(
      "#############################################################################################\n"
    )
    ...
    cat(
      "Les contrôles logiques directs sont nettoyés. Veuillez poursuivre et exécuter \nles contrôles des valeurs aberrantes.\n"
    )
    cat(
      "#############################################################################################\n"
    )
