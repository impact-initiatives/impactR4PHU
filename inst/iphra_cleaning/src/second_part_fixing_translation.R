...
if (nrow(or.edited %>% filter(check == 1 & ref.type == "select_one")) > 0) {
  if (language_assessment == "English") {
    ...
    stop(
      "Veuillez revérifier le fichier que vous avez rempli, il est clair que certaines lignes select_one ont été remplies plusieurs fois."
    )
    cat(
      "\n\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n"
    )
...
if (nrow(or.true.and.recode) > 0) {
  cat(
    paste0(
      "Multiple columns selected in ",
      nrow(or.true.and.recode),
      " or.edited entries:\n",
      paste0(or.true.and.recode %>% pull(uuid), collapse = "\n")
    ),
    sep = "\n"
  )
  if (any(or.true.and.recode$ref.type != "select_multiple")) {
    stop("One of those is not a select_multiple!!!")
  }
  ...
  for (r in 1:nrow(or.true.and.recode)) {
    ...
    if (is.na(x$loop_index)) {
      old.value <- as.character(raw.main[
        raw.main$uuid == x$uuid[1],
        x$ref.name[1]
      ])
    } else if (str_starts(x$loop_index, "loop_hh_roster")) {
      old.value <- as.character(raw.hh_roster[
        raw.hh_roster$loop_index == x$loop_index[1],
        x$ref.name[1]
      ])
    } else if (str_starts(x$loop_index, "loop_ind_health")) {
      old.value <- as.character(raw.ind_health[
        raw.ind_health$loop_index == x$loop_index[1],
        x$ref.name[1]
      ])
    } else if (str_starts(x$loop_index, "loop_child_nutrition")) {
      old.value <- as.character(raw.child_nutrition[
        raw.child_nutrition$loop_index == x$loop_index[1],
        x$ref.name[1]
      ])
    } else {
      if (!is.null(raw.died_member)) {
        old.value <- as.character(raw.died_member[
          raw.died_member$loop_index == x$loop_index[1],
          x$ref.name[1]
        ])
      }
    }
    ...
    for (choice in choices) {
      ...
      if (is.na(x$loop_index)) {
        ...
        if (variable.name %in% colnames(raw.hh_roster)) {
          old.boolean <- raw.hh_roster[[variable.name]][
            raw.hh_roster$loop_index == x$loop_index[1]
          ]
        } else {
          warning("Column not found")
        ...
        if (variable.name %in% colnames(raw.ind_health)) {
          old.boolean <- raw.ind_health[[variable.name]][
            raw.ind_health$loop_index == x$loop_index[1]
          ]
        } else {
          warning("Column not found")
        ...
        if (variable.name %in% colnames(raw.child_nutrition)) {
          old.boolean <- raw.child_nutrition[[variable.name]][
            raw.child_nutrition$loop_index == x$loop_index[1]
          ]
        } else {
          warning("Column not found")
        ...
        if (!is.null(raw.died_member)) {
          if (variable.name %in% colnames(raw.died_member)) {
            old.boolean <- raw.died_member[[variable.name]][
              raw.died_member$loop_index == x$loop_index[1]
            ]
          } else {
            warning("Column not found")
...
response.col <- colnames(or.true)[str_detect(
  colnames(or.true),
  "response.\\S{2}."
)]
...
if (!is.null(raw.died_member)) {
  ...
  if (nrow(t.trans) > 0) {
    stop("Missing entries or multiple columns selected")
  }
  ...
  response.col <- colnames(trans.true)[str_detect(
    colnames(trans.true),
    "response.\\S{2}."
  )]
  t.trans <- trans.true %>%
    ...
    rename(
      variable = name,
      old.value = response.col,
      new.value = true.trans
    ) %>%
    select(uuid, loop_index, variable, issue, old.value, new.value)
...
if (language_assessment == "English") {
  ...
  cat(
    "Fixing the recoding of the others and the translations is done. Next step is running logical checks.\n"
  )
  cat(
    "#####################################################################################################\n"
  )
