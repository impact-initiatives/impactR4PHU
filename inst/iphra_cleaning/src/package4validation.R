...
if (length(filename_kobo_raw) == 0) {
  stop("Error: couldn't find filename_kobo_raw")
}
filename_data_final <- list.files(
  "output/final/",
  full.names = T,
  pattern = anon_pattern
)
if (length(filename_data_final) == 0) {
  stop("Error: couldn't find filename_data_log")
}

filename_checking_audit <- list.files("output/checking/audit/", full.names = T)
filename_checking_outliers <- list.files(
  "output/checking/outliers/",
  full.names = T
)
filename_checking_requests <- list.files(
  "output/checking/requests/",
  full.names = T
)
filename_checking_responses <- list.files(
...
filenames_enum_performance <- list.files(
  "output/enum_performance/",
  full.names = T
)

# resources:
filenames_resources <- list.files(
  "resources",
...
# source:
filenames_R <- list.files(
  pattern = "*.R$|*.Rmd$",
  recursive = T,
  full.names = T
)
filenames_R <- filenames_R[stringr::str_detect(
  filenames_R,
  "(api\\.key)|(validation)|(renv)",
  T
)]
