...
int_cols_main <- tool.survey %>%
  ...
  pull(name)
int_cols_hh_roster <- tool.survey %>%
  filter(type == "integer" & datasheet == "hh_roster") %>%
  pull(name)
if (!is.null(raw.water_count_loop)) {
  int_cols_water_count_loop <- tool.survey %>%
    filter(type == "integer" & datasheet == "water_count_loop") %>%
    pull(name)
}
if (!is.null(raw.died_member)) {
  int_cols_died_member <- tool.survey %>%
    filter(type == "integer" & datasheet == "died_member") %>%
    pull(name)
}
...
cl_999s <- bind_rows(
  recode.set.NA.if(raw.main, int_cols_main, "999", "replacing 999 with NA"),
  recode.set.NA.if(
    raw.hh_roster,
    int_cols_hh_roster,
    "999",
...
if (!is.null(raw.water_count_loop)) {
  cl_999s <- bind_rows(
    cl_999s,
    recode.set.NA.if(
      raw.water_count_loop,
      int_cols_water_count_loop,
      "999",
      "replacing 999 with NA"
    )
  )
...
if (
  all(fcm_check_1_columns %in% names(raw.flag)) |
...
if (!is.null(raw.child_nutrition)) {
  if (
    all(c("nut_muac_cm", "nut_edema_confirm") %in% names(raw.child_nutrition))
  ) {
...
fcs_columns <- names(raw.flag.fcs)[which(stringr::str_detect(
  names(raw.flag.fcs),
  "fsl_fcs_"
))]
if ("fsl_fcs_score" %in% names(raw.flag.fcs)) {
  ...
  for (i in 1:nrow(check)) {
    cl <- recode.set.NA.if(
      ...
      fcs_columns,
      check[i, fcs_columns],
      "replacing fcs columns with NA because all fcs are 0",
      ignore_case = F
    ) %>%
  ...
  cleaning.log.checks.direct <- bind_rows(
...
fcs_columns <- names(raw.flag.fcs)[which(stringr::str_starts(
  names(raw.flag.fcs),
  "fsl_fcs_"
))]
if ("fsl_fcs_score" %in% names(raw.flag.fcs)) {
  ...
  for (i in 1:nrow(check)) {
    cl <- recode.set.NA.if(
      ...
      fcs_columns,
      check[i, fcs_columns],
      "replacing fcs columns with NA because all fcs are 7",
      ignore_case = F
    ) %>%
      filter(!is.na(old.value)) %>%
      mutate(
...
### All LCSI NA
lcsi_columns <- names(raw.flag.fcs)[which(stringr::str_starts(
  names(raw.flag.fcs),
  "fsl_lcsi_"
))]
if ("flag_lcsi_na" %in% names(raw.flag.fcs)) {
  ...
  for (i in 1:nrow(check)) {
    cl <- recode.set.NA.if(
      check[i, ],
      lcsi_columns,
      check[i, lcsi_columns],
      "replacing lcsi columns with NA because all lcsi are na",
      ignore_case = F
    ) %>%
      filter(!is.na(old.value)) %>%
      mutate(
...
if ("flag_lcsi_displ" %in% names(raw.flag.fcs)) {
  ...
  for (i in 1:nrow(check)) {
    cl <- recode.set.NA.if(
      ...
      displ,
      check[i, displ],
      "replacing lcsi displacement strategy columns with NA because HH not IDP",
      ignore_case = F
    ) %>%
      filter(!is.na(old.value)) %>%
      mutate(
...
if ("flag_lcsi_liv_agriculture" %in% names(raw.flag.fcs)) {
  ...
  for (i in 1:nrow(check)) {
    cl <- recode.set.NA.if(
      ...
      agric,
      check[i, agric],
      "replacing lcsi agricultural strategy columns with NA because HH do not have income from agriculture",
      ignore_case = F
    ) %>%
      filter(!is.na(old.value)) %>%
      mutate(
...
if ("flag_lcsi_liv_livestock" %in% names(raw.flag.fcs)) {
  ...
  for (i in 1:nrow(check)) {
    cl <- recode.set.NA.if(
      ...
      livest,
      check[i, livest],
      "replacing lcsi livestock strategy columns with NA because HH do not have income from livestock",
      ignore_case = F
    ) %>%
      filter(!is.na(old.value)) %>%
      mutate(
...
if (!is.null(raw.water_count_loop)) {
  ...
  if ("flag_sd_litre" %in% names(raw.flag.wash)) {
    ...
    columns <- c(
      "wash_container_type",
      "wash_container_type_other",
      "wash_container_litre_other"
    )
    ...
    if (nrow(check_loop) > 0) {
      for (i in 1:nrow(check_loop)) {
        cl <- recode.set.NA.if(
          check_loop[i, ],
          columns,
          check_loop[i, columns],
          "replacing container info to NA if the sd of lpd is -/+ 3 from the mean of total lpd",
          ignore_case = F
        ) %>%
          filter(!is.na(old.value)) %>%
          mutate(
    ...
    if (nrow(check_loop) > 0) {
      for (i in 1:nrow(check_loop)) {
        cl <- recode.set.NA.if(
          ...
          columns,
          check_loop[i, columns],
          "replacing container info to NA if lpd is lower than 1",
          ignore_case = F
        ) %>%
          filter(!is.na(old.value)) %>%
          mutate(
    ...
    if (nrow(check_loop) > 0) {
      for (i in 1:nrow(check_loop)) {
        cl <- recode.set.NA.if(
          ...
          columns,
          check_loop[i, columns],
          "replacing container info to NA if lpd is higher than 50",
          ignore_case = F
        ) %>%
          filter(!is.na(old.value)) %>%
          mutate(
  ...
  if ("flag_high_container" %in% names(raw.flag.wash)) {
    check_uuid <- raw.flag.wash %>%
      filter(flag_high_container == 1) %>%
      pull(uuid)
    check_loop <- raw.water_count_loop %>%
      filter(uuid %in% check_uuid)
    columns <- c(
      ...
      "wash_container_type_other",
      "wash_container_litre_other",
      "wash_container_journey_info",
      "wash_container_journey_collection",
      "container_position"
    )
    ...
    if (nrow(check_loop) > 0) {
      for (i in 1:nrow(check_loop)) {
        cl <- recode.set.NA.if(
          ...
          columns,
          check_loop[i, columns],
          "replacing container info to NA if num of containers higher than 20",
          ignore_case = F
        ) %>%
          filter(!is.na(old.value)) %>%
          mutate(
...
if ("flag_not_immediate" %in% names(raw.flag.wash)) {
  ...
  for (i in 1:nrow(check)) {
    cl <- recode.set.value.regex(
      check[i, ],
      "wash_water_collect_time",
      check[i, "wash_water_collect_time"],
      "dont_know",
      "replacing water collection time info to dont know because water source is on premise"
    ) %>%
      filter(!is.na(old.value)) %>%
      mutate(
        old.value = as.character(old.value),
    ...
    if (nrow(check) > 0) {
      for (i in 1:nrow(check)) {
        cl <- recode.set.NA.if(
          ...
          columns,
          check[i, columns],
          "replacing extreme muacs with NA",
          ignore_case = F
        ) %>%
          filter(!is.na(old.value)) %>%
          mutate(
...
raw.child_nutrition <- raw.child_nutrition %>%
  apply.changes(cleaning.log.checks.direct, is.loop = T)
if (!is.null(raw.water_count_loop)) {
...
if ("flag_protein_rcsi" %in% names(raw.flag.fcs)) {
  check_protein_rcsi <- raw.flag.fcs %>%
    ...
    left_join(
      raw.flag %>% select(uuid, fsl_rcsi_score, fsl_fcs_meat, fsl_fcs_dairy)
    )

  if (nrow(check_protein_rcsi) > 0) {
    checks_followups <- rbind(
      checks_followups,
      make.logical.check.entry(
        check_protein_rcsi,
        1,
        c("fsl_rcsi_score", "fsl_fcs_meat", "fsl_fcs_dairy"),
        cols_to_keep = c(enum_colname),
        "rCSI Score is high while protein consumption is also reported as frequent",
        F
      )
    )
  }
    ...
    checks_followups <- rbind(
      ...
      make.logical.check.entry(
        ...
        c(
          ...
          "fsl_lcsi_emergency2",
          "fsl_lcsi_emergency3"
        ), # to provide all teh strategies
        cols_to_keep = c(enum_colname),
        "HHs report using crisis or emergency strategies but not stress strategies or Emergency and no crisis.",
        F
      )
    )
  }
  ...
  if (nrow(check_fcsrcsi_box) > 0) {
    checks_followups <- rbind(
      ...
      make.logical.check.entry(
        check_fcsrcsi_box,
        3,
        c(fcs_flag_columns, rcsi_flag_columns),
        cols_to_keep = c(enum_colname),
        "HH that would have an acceptable FCS score and a high rCSI score",
        F
      )
    )
  }
      ...
      left_join(
        raw.main %>%
          select(
            ...
            wash_num_containers,
            wash_water_source,
            `wash_different_water_sources/piped_dwelling`,
            `wash_different_water_sources/piped_compound`,
    ...
    if (nrow(check_no_container) > 0) {
      checks_followups <- rbind(
        ...
        make.logical.check.entry(
          ...
          cols_to_keep = c(enum_colname),
          "HH reported no containers but also reports that water sources are not on premises",
          F
        )
      )
    }
    ...
    if (nrow(check_eodema) > 0) {
      checks_followups <- bind_rows(
        ...
        make.logical.check.entry(
          ...
          c("nut_edema_confirm"),
          cols_to_keep = c(enum_colname),
          "Respondent reported children have oedema.",
          T
        )
      ) %>%
...
if (!is.null(raw.died_member)) {
  ...
  if (nrow(check_mortality) > 0) {
    checks_followups <- bind_rows(
      ...
      make.logical.check.entry(
        ...
        c("num_died"),
        cols_to_keep = c(enum_colname),
        "Respondent reported more than 2 death in the HH",
        F
      )
    )
  }
  ...
  check_cause_death <- raw.died_member %>%
    ...
    filter(
      sex_died == "m" &
        cause_death %in% c("post_partum", "during_pregnancy", "during_delivery")
    ) %>%
    left_join(raw.main %>% select(uuid, enum_colname))
  ...
  if (nrow(check_cause_death) > 0) {
    checks_followups <- bind_rows(
      ...
      make.logical.check.entry(
        check_cause_death,
        7,
        c("sex_died", "cause_death"),
        cols_to_keep = c(enum_colname),
        "Respondent reported sex of dead person male and a cause of death related to female only.",
        T
      )
    ) %>%
      relocate(loop_index, .before = 2)
...
if (nrow(checks_followups) > 0) {
  ...
  create.follow.up.requests(
    checks_followups,
    loop_data = raw.died_member,
    paste0(make.short.name("followup_requests"), ".xlsm"),
    use_template = T
  )
}
...
if (language_assessment == "English") {
  if (nrow(checks_followups) > 0) {
    ...
    cat(
      "Direct logical checks are flagged and no follow_up is needed.\nContinue running the next step."
    )
    cat(
      "#############################################################################################\n"
    )
  ...
  if (nrow(checks_followups) > 0) {
    ...
    cat(
      "Les contrôles logiques directs sont signalés et.\nContinuer avec la prochaine etape."
    )
    cat(
      "#############################################################################################\n"
    )
