...
label_colname <- tcltk::tk_select.list(
  names(readxl::read_excel(strings['filename.tool'], "survey"))[grepl(
    "label",
    names(readxl::read_excel(strings['filename.tool'], "survey"))
  )],
...
uuid_name <- names(data.list[["main"]])[grepl(
  "uuid",
  names(data.list[["main"]])
)]
index_name <- names(data.list[["main"]])[grepl(
  "_index",
  names(data.list[["main"]])
)]

data.list[["main"]] <- data.list[["main"]] %>%
  rename(uuid = uuid_name, index = index_name)
...
for (sheet in sheet_names[-1]) {
  ...
  uuid_sheet_name <- names(data.list[[sheet]])[grepl(
    "uuid",
    names(data.list[[sheet]])
  )]
