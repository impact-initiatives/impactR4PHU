xlsform_fill_loop <- function(tool.path = "", language = "English", n = 100) {
  ...
  label_colname <- tool_colnames[agrep(
    paste0("label::", language),
    tool_colnames
  )]

  if (is_empty(label_colname)) {
    stop(
      "Please provide the correct language used in the tool (e.g.language = 'English' if label::English)"
    )
  }
  ...
  for (i in 1:nrow(tool.survey)) {
    ...
    if (str_detect(toolrow$type, "begin[ _]repeat")) {
      sheet_name <- toolrow$name
    } else if (str_detect(toolrow$type, "end[ _]repeat")) {
      # watch out for nested repeats (Why would you even want to do that?)
      sheet_name <- "main"
  ...
  for (i in 1:nrow(tool.survey)) {
    toolrow <- tool.survey %>% slice(i)
    if (str_detect(toolrow$type, "begin[ _]repeat")) {
      repeat_c <- toolrow$repeat_count
    } else if (str_detect(toolrow$type, "end[ _]repeat")) {
      # watch out for nested repeats (Why would you even want to do that?)
      repeat_c <- NA
    } else if (str_detect(toolrow$type, "((end)|(begin))[ _]group", T)) {
      tool.survey[i, "count_repeat"] <- repeat_c
    }
  }

  choices <- read_xlsx(tool.path, sheet = "choices", col_types = "text") %>%
    filter(!is.na(list_name)) %>%
    select(all_of(c("list_name", "name")), !!sym(label_colname)) %>%
    distinct()
  ...
  for (sheet in sheets) {
    ...
    count <- as.numeric(data.list$main[, gsub(
      "[${]|[}]",
      "",
      unique(questions$count_repeat)
    )])
            ...
            if (get_raw_type(varname) == "integer") {
              if (!is.na(constraint) & varname %in% repeat_count) {
                ...
                if (bounds$upper_bound == 100) {
                  ...
                  filling <- fill_integer_constraint(
                    varname,
                    n_loop,
                    constraint5
                  )
...
skipped_values <- function(data, questionnaire) {
  lapply(names(data), function(x) {
    tryCatch(
      {
        questionnaire$question_is_skipped(data, x)
      },
      error = function(e) {
        warning(paste0(
          "failed applying skiplogic for '",
          x,
          "' with error: ",
          e$message
        ))
...
add_form_calculations <- function(data, questions) {
  get_raw_type <- function(varname) {
    questions$type[match(varname, questions$name)][1]
  }
  calculate_varnames <- questions$name[which(
    sapply(questions$name, get_raw_type) == "calculate"
  )]
  get_calculation <- function(varname) {
    questions$calculation[match(varname, questions$name)][1]
  }

  for (varname in calculate_varnames) {
    data[[varname]] <- fill_calculate(
      varname,
...
get_questionnaire_functions <- function(questions, choices) {
  empty_data <- as.data.frame(lapply(questions$name, function(x) {
    NA
  }))
...
choices_of_question_funfact <- function(questions, choices) {
  ...
  # }
  choices_per_data_column <- questions$type %>%
    as.character %>%
    strsplit(" ") %>%
    lapply(unlist) %>%
    lapply(function(x) {
...
fill_decimal_constraint <- function(varname, n, constraint_message) {
  filling <- tibble::tibble(generate_random_numbers(
    n,
    constraint_message,
    round = F
  ))
...
fill_text <- function(varname, n) {
  filling <- tibble::tibble(stringi::stri_rand_strings(
    n,
    length = round(runif(n, max = 100)),
    pattern = "[A-Za-z\\ ]"
  ))
  colnames(filling) <- varname
  filling
...
fill_select_one <- function(varname, n, options) {
  if (length(options) == 0) {
    options <- NA
    warning(paste0(
      "no choices found for variable: '",
      varname,
      "' - filling this select_one with NA"
    ))
...
fill_select_multiple <- function(varname, n, options) {
  if (length(options) == 0) {
    warning(paste0(
      "no choices found for variable: '",
      varname,
      "' - ignoring this select_multiple"
    ))
    return(tibble::tibble(1:n)[, 0])
  }
  options <- as.character(options)
  options <- unique(options)
  filling_logical <- matrix(
    sample(c(1, 0), n * length(options), T, prob = c(0.3, 0.7)),
    nrow = n
  )
  colnames(filling_logical) <- paste0(varname, "/", options)

  filling_concatenated_choices <- tibble::tibble(apply(
    filling_logical,
    1,
    function(x) {
      paste0(options[which(x == 1)], collapse = " ")
    }
  ))
  colnames(filling_concatenated_choices) <- varname

  tibble::as_tibble(cbind(
    filling_concatenated_choices,
    filling_logical,
    stringsAsFactors = F
  ))
}
...
fill_datetime <- function(varname, n) {
  ...
  filling[, 1] <- format.Date(
    as.data.frame(filling)[, 1],
    "%Y-%m-%dT%H:%M:%S%z"
  ) %>%
    as.character
  colnames(filling) <- varname
...
fill_calculate <- function(varname, kobo_calculation, other_data) {
  ...
  if (calculation_as_rcode == "") {
    calculation_as_rcode <- "rep(NA,nrow(other_data))"
  }
  calc_result <- tryCatch(
    {
      purrr::pmap(.l = other_data, .f = function(...) {
        dots <- list(...)
        with(dots, {
          ...
          if (length(calc_result) != 1) {
            stop(paste0(
              "filling '",
              varname,
              "' with NA. Calculation did not produce one value per data row."
            ))
          }
          calc_result
        })
    ...
    error = function(e) {
      warning(paste0(
        "\nfilling '",
        varname,
        "' with NA. Calculation failed with error: \n",
        e$message
      ))
...
skip_data <- function(tool, df) {
  ...
  tool <- tool |>
    mutate(
      ...
      transformation3 = gsub(
        "\\bor\\b",
        "|",
        transformation2,
        ignore.case = TRUE
      ),
      transformation4 = gsub(
        "\\band\\b",
        "&",
        transformation3,
        ignore.case = TRUE
      ),
      transformation5 = gsub(
        "(?<!\\!|>|<)=(?![>=<])",
        "==",
        transformation4,
        perl = TRUE
  ...
  get_relevancy <- function(name) {
    if (!is.na(tool[min(which(tool$name == name)), "relevant"])) {
      return(sprintf(
        "(%s)",
        as.character(tool[min(which(tool$name == name)), "transformation5"])
      ))
    }
    return("")
  }
  ...
  for (col_ind in 2:ncol(df_out)) {
    ...
    df_result[
      which(df_out[[col_ind]] == FALSE),
      which(names(df_result) == names(df_out)[col_ind])
    ] = NA
    df_result[
      which(df_out[[col_ind]] == FALSE),
      which(grepl(sm_col_ind, names(df_result)))
    ] = NA
  }
  return(df_result)
}
...
extract_constraints <- function(
  ...
  adjust_bound <- function(constraint) {
    if (grepl("\\.>=", constraint)) {
      ...
      return(
        as.numeric(gsub(".*\\.>(\\d+).*", "\\1", constraint)) +
          .Machine$double.eps
      )
    } else if (grepl("\\.<=", constraint)) {
      return(as.numeric(gsub(".*\\.<=(\\d+).*", "\\1", constraint)))
...
generate_random_numbers <- function(n, constraint_message, round = T) {
  ...
  if (round) {
    ...
    random_numbers <- round(
      runif(n, min = constraints$lower_bound, max = constraints$upper_bound),
      0
    )
  } else {
    # Generate n random numbers within the specified range
