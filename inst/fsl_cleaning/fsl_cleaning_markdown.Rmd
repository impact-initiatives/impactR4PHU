---
title: "FSL Data Cleaning and Follow up Report"
subtitle: "`r Sys.Date()`"
...
path.raw.main <- svDialogs::dlg_open(multiple = F, title = "Please select your raw data excel file.")$res
...
```{r, results='asis'}
  ...
  cat(paste0(paste0(rep("\n",2), collapse=""), paste0(rep("#",4), collapse=""), " ", "Direct Checks"))
...
if(!file.exists("inputs/environment.Rdata")) {
  if ("HHS" %in% FSL_indicators){
    ...
    if(!all(c(hhs_check_columns,hhs_check_columns_freq) %in% names(raw.main))) {
      ...
      yes_answer <- tcltk::tk_select.list(unique(unlist(raw.main[,hhs_check_columns])), title = "Yes Value")
      no_answer <- tcltk::tk_select.list(unique(unlist(raw.main[,hhs_check_columns])), title = "No Value")
      rarely_answer <- tcltk::tk_select.list(unique(unlist(raw.main[,hhs_check_columns_freq])), title = "Rarely Value")
      sometimes_answer <- tcltk::tk_select.list(unique(unlist(raw.main[,hhs_check_columns_freq])), title = "Sometimes Value")
      often_answer <- tcltk::tk_select.list(unique(unlist(raw.main[,hhs_check_columns_freq])), title = "Often Value")
      raw.main <- raw.main %>%
...
if(!file.exists("inputs/environment.Rdata")) {
  if ("LCSI" %in% FSL_indicators){
    ...
    if(!all(lcsi_check_columns %in% names(raw.main))) {
      ...
      yes_val <- tcltk::tk_select.list(unique(unlist(raw.main[,lcsi_check_columns])), title = "Yes Value")
      no_val <- tcltk::tk_select.list(unique(unlist(raw.main[,lcsi_check_columns])), title = "No Value")
      exhausted_val <- tcltk::tk_select.list(unique(unlist(raw.main[,lcsi_check_columns])), title = "Exhausted Value")
      not_applicable_val <- tcltk::tk_select.list(unique(unlist(raw.main[,lcsi_check_columns])), title = "Not Applicable Value")
      raw.main <- raw.main %>%
...
if(!file.exists("inputs/environment.Rdata")) {
  if ("HDDS" %in% FSL_indicators){
    ...
    if(!all(hdds_check_columns %in% names(raw.main))) {
      ...
      yes_val_hdds <- tcltk::tk_select.list(unique(unlist(raw.main[,hdds_check_columns])), title = "Yes Value")
      no_val_hdds <- tcltk::tk_select.list(unique(unlist(raw.main[,hdds_check_columns])), title = "No Value")
      raw.main <- raw.main %>%
        dplyr::mutate_at(vars(lcsi_check_columns),~case_when(. == yes_val ~ yes_val,
                                                             . == no_val ~ no_val,
                                                             TRUE ~ NA)) %>%
    ...
    fsl_hdds_sugar <- "fsl_hdds_sugar"
    fsl_hdds_condiments <- "fsl_hdds_condiments"
    yes_val_hdds <- "yes_val_hdds"
    no_val_hdds <- "no_val_hdds"
    hdds_check_columns <- c(fsl_hdds_cereals,fsl_hdds_tubers,fsl_hdds_veg,fsl_hdds_fruit,fsl_hdds_meat,
                            fsl_hdds_eggs,fsl_hdds_fish,fsl_hdds_legumes,fsl_hdds_dairy,fsl_hdds_oil,
                            fsl_hdds_sugar,fsl_hdds_condiments)
  }
...
if(!file.exists("inputs/environment.Rdata")) {
  ...
  residency_status <- names(raw.main)[grepl("residency|status",names(raw.main))]
    ...
    residency_status <- tcltk::tk_select.list(residency_status, title = "Residency Status column")
    if(residency_status == ""){
      residency_status <- svDialogs::dlg_input(message= "Enter the name of the residency_status","residency_status")$res
    }
...
list_of_var <- c("FSL_indicators","num_children","income_types", "fsl_fcs_cereal","fsl_fcs_legumes",
                 "fsl_fcs_veg","fsl_fcs_fruit","fsl_fcs_meat","fsl_fcs_dairy","fsl_fcs_sugar","uuid_main",
                 ...
                 "fclcm_check_columns","fcm_check_1_columns","fcm_check_2_columns","fcm_check_3_columns","yes_val_hdds","no_val_hdds",
...
### All LCSI NA
lcsi_columns <- lcsi_check_columns
...
lcs_variables <- c(fsl_lcsi_stress1,
                   fsl_lcsi_stress2,
                   fsl_lcsi_stress3,
                   fsl_lcsi_stress4,
                   fsl_lcsi_crisis1,
...
<h5>Number of detected entries with all FCS component are 0:<strong> `r if(nrow(cl_fcs_all_0)>0) {nrow(cl_fcs_all_0 %>%  distinct(uuid))} else {0}`</strong></h5>
<h5>Number of detected entries with all FCS component are 7:<strong>`r if(nrow(cl_fcs_all_7)>0) {nrow(cl_fcs_all_7 %>%  distinct(uuid))} else {0}`</strong></h5>
<h5>Number of detected entries with all LCSI are Not Applicable:<strong> `r if(nrow(cl_lcsi_all_na)>0) {nrow(cl_lcsi_all_na %>%  distinct(uuid))} else {0}`</strong></h5>
<h5>Number of detected entries with LCSI strategy related to displacement but HH not IDPs:<strong> `r if(nrow(cl_lcsi_displ)>0) {nrow(cl_lcsi_displ %>%  distinct(uuid))} else {0}`</strong></h5>
<h5>Number of detected entries with LCSI strategy related to agriculture but HH do not have income from agriculture:<strong> `r if(nrow(cl_lcsi_agric)>0) {nrow(cl_lcsi_agric %>%  distinct(uuid))} else {0}`</strong></h5>
<h5>Number of detected entries with LCSI strategy related to livestock but HH do not have income from livestock::<strong>`r if(nrow(cl_lcsi_livest)>0) {nrow(cl_lcsi_livest %>%  distinct(uuid))} else {0}` </strong></h5>
...
if("flag_protein_rcsi" %in% names(raw.flag.fcs)){
  check_protein_rcsi <- raw.flag.fcs %>% 
    select(uuid_main,enumerator, flag_protein_rcsi)%>% 
    filter(flag_protein_rcsi == 1) %>% 
    left_join(raw.flag.fcs %>% select(uuid_main, fsl_rcsi_score, fsl_fcs_meat, fsl_fcs_dairy)) %>% 
    rename(uuid = uuid_main)
  ...
  if(nrow(check_protein_rcsi)>0){
    checks_followups <- rbind(checks_followups,
                              make.logical.check.entry(check_protein_rcsi, 1,  c("fsl_rcsi_score",fsl_fcs_meat, fsl_fcs_dairy), 
                                                       cols_to_keep = c(enumerator),"rCSI Score is high while protein consumption is also reported as frequent", F))
  }
}
...
if("flag_lcsi_coherence" %in% names(raw.flag.fcs)){
  ...
  check_lcsi_coherence <- raw.flag.fcs %>% 
    select(uuid_main,enumerator, flag_lcsi_coherence)%>% 
    filter(flag_lcsi_coherence == 1)%>% 
    left_join(raw.flag.fcs %>% select(uuid_main, 
  ...
  if(nrow(check_lcsi_coherence)>0){
    checks_followups <- rbind(checks_followups,
                              make.logical.check.entry(check_lcsi_coherence, 2,
                                                       ...
                                                       cols_to_keep = c(enumerator),"HHs report using crisis or emergency strategies but not stress strategies or Emergency and no crisis.", F))
  }
}
#Check number 3
...
if("flag_fcsrcsi_box" %in% names(raw.flag.fcs)) {
  check_fcsrcsi_box <- raw.flag.fcs %>% 
    select(uuid_main,enumerator, flag_fcsrcsi_box)%>% 
    filter(flag_fcsrcsi_box == 1)%>% 
    left_join(raw.flag.fcs %>% select(uuid_main, fcs_flag_columns, rcsi_flag_columns))%>% 
    rename(uuid = uuid_main)
  
  if(nrow(check_fcsrcsi_box)>0){
    checks_followups <- rbind(checks_followups,
                              make.logical.check.entry(check_fcsrcsi_box, 3,  c(fcs_flag_columns, rcsi_flag_columns), 
                                                       cols_to_keep = c(enumerator),"HH that would have an acceptable FCS score and a high rCSI score", F))
  }
}
source("src/create_file.R")
```

<h5>Number of detected entries with rCSI Score is high while protein consumption is also reported as frequent:<strong> `r if(nrow(checks_followups)>0) {nrow(checks_followups %>% filter(check.id == 1) %>% distinct(uuid))} else {0}`</strong></h5>
<h5>Number of detected entries with HHs report using crisis or emergency strategies but not stress strategies or Emergency and no crisis:<strong> `r if(nrow(checks_followups)>0) {nrow(checks_followups %>% filter(check.id == 2) %>% distinct(uuid) )} else {0}`</strong></h5>
<h5>Number of detected entries with HH that would have an acceptable FCS score and a high rCSI score:<strong> `r if(nrow(checks_followups)>0) {nrow(checks_followups %>% filter(check.id == 3) %>% distinct(uuid))} else {0}`</strong></h5>
