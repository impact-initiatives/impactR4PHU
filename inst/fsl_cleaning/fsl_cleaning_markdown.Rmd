...
if("flag_protein_rcsi" %in% names(raw.flag.fcs)){
  check_protein_rcsi <- raw.flag.fcs %>% 
    select(uuid,enumerator, flag_protein_rcsi)%>% 
...
if("flag_lcsi_coherence" %in% names(raw.flag.fcs)){
  ...
  check_lcsi_coherence <- raw.flag.fcs %>% 
    select(uuid,enumerator, flag_lcsi_coherence)%>% 
...
if("flag_fcsrcsi_box" %in% names(raw.flag.fcs)) {
  check_fcsrcsi_box <- raw.flag.fcs %>% 
    select(uuid,enumerator, flag_fcsrcsi_box)%>% 
...
<h5>Number of detected entries with rCSI Score is high while protein consumption is also reported as frequent:<strong> `r if(nrow(checks_followups)>0) {nrow(checks_followups %>% filter(check == 1) %>% distinct(uuid))} else {0}`</strong></h5>
<h5>Number of detected entries with HHs report using crisis or emergency strategies but not stress strategies or Emergency and no crisis:<strong> `r if(nrow(checks_followups)>0) {nrow(checks_followups %>% filter(check == 2) %>% distinct(uuid) )} else {0}`</strong></h5>
<h5>Number of detected entries with HH that would have an acceptable FCS score and a high rCSI score:<strong> `r if(nrow(checks_followups)>0) {nrow(checks_followups %>% filter(check == 3) %>% distinct(uuid))} else {0}`</strong></h5>
