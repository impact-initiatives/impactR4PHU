...
if(!file.exists("inputs/environment.Rdata")) {
  if ("HDDS" %in% FSL_indicators){
    ...
    if(!all(hdds_check_columns %in% names(raw.main))) {
      ...
      yes_val_hdds <- tcltk::tk_select.list(dplyr::pull(raw.main[,hdds_check_columns]) %>% unique, title = "Yes Value")
      no_val_hdds <- tcltk::tk_select.list(dplyr::pull(raw.main[,hdds_check_columns]) %>% unique, title = "No Value")
      raw.main <- raw.main %>%
        dplyr::mutate_at(vars(lcsi_check_columns),~case_when(. == yes_val ~ yes_val,
                                                             . == no_val ~ no_val,
                                                             TRUE ~ NA)) %>%
