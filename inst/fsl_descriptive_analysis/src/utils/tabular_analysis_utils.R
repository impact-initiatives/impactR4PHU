...
sm_ccols_to_choices <- function(ccols) {
  ...
  stringr::str_split(
    ccols %>% stringr::str_remove_all("_prop$"),
    "___",
    simplify = T
  )[, 2]
}

make_table.select_one <- function(
  srvyr.design.grouped,
  entry,
  add_total = FALSE
) {
  disagg_vars <- c(entry$admin, entry$disaggregate.variables)
  disagg_vars <- disagg_vars[
    disagg_vars %in% (srvyr.design.grouped %>% variable.names)
  ]
  ...
  res.wide <- res.long %>%
    tidyr::pivot_wider(
      id_cols = any_of(disagg_vars),
      names_from = !!sym(entry$variable),
      values_from = c(num_samples, prop),
      values_fill = 0
    ) %>%
    # calculate total num_samples per group, instead of number of responses for each option
    mutate(
      num_samples = rowSums(across(starts_with("num_samples_")), na.rm = T),
      .before = starts_with("prop_")
    ) %>%
    # remove and rename columns
    select(-starts_with("num_samples_")) %>%
    rename_with(~ stringr::str_remove(., "prop_"), starts_with("prop_"))

  if (add_total) {
    # calculate total and bind it to res
    res.total <- make_table.select_one(
      srvyr.design.grouped %>%
        ungroup %>%
...
make_table.select_multiple <- function(
  srvyr.design.grouped,
  entry,
  add_total = FALSE
) {
  disagg_vars <- c(entry$admin, entry$disaggregate.variables)
  if (any(isna(disagg_vars))) {
    disagg_vars <- NULL
  }
  disagg_vars <- disagg_vars[
    disagg_vars %in% (srvyr.design.grouped %>% variable.names)
  ...
  s_props <- srvyr.design.grouped %>%
    summarise(across(
      .fns = list(prop = ~ srvyr::survey_mean(., na.rm = T, vartype = "var"))
    ))
  ...
  if (add_total) {
    ...
    total <- srvyr.design.grouped %>%
      ...
      summarise(
        across(
          .fns = list(
            prop = ~ srvyr::survey_mean(., na.rm = T, vartype = "var")
          )
        ),
        .groups = "drop"
      )

    total_samples <- s_samples %>%
      group_by(!!sym(entry$admin)) %>%
      summarise(num_samples = sum(num_samples))

    res <- res %>%
      bind_rows(total_samples %>% left_join(total, by = entry$admin))
  }
  # convert choice names to labels (all except the NA column)
  res <- res %>%
    rename_with(
      ~ get.choice.label(sm_ccols_to_choices(.), entry$list_name, simplify = T),
      ends_with("_prop") & !contains("___NA")
    )
  # also convert the NA column:
  if (!entry$omit_na) {
    res <- res %>% rename("NA" = !!paste0(entry$variable, "___NA_prop"))
  }
...
make_table <- function(srvyr.design, entry, disagg.var) {
  ...
  res <- switch(
    ...
    select_one = {
      srvyr.design.grouped %>%
        make_table.select_one(
          entry,
          add_total = entry$add_total & disagg.var != entry$admin
        )
    },
    select_multiple = {
      srvyr.design.grouped %>%
        make_table.select_multiple(
          entry,
          add_total = entry$add_total & disagg.var != entry$admin
        )
    }
  ...
  if ("num_samples" %in% names(res)) {
    res <- res %>% filter(num_samples > 0)
  }
  ...
  res <- switch(
    ...
    # default (select_one or multiple): convert everything except num_samples to percent
    {
      res %>%
        mutate(across(
          where(is.numeric) & !matches("^num_samples$"),
          as_perc
        )) %>%
        # add label for the TOTAL row
        mutate(
          !!disagg.var := tidyr::replace_na(
            !!sym(disagg.var) %>% as.character,
            "<TOTAL>"
          )
        )
    }
