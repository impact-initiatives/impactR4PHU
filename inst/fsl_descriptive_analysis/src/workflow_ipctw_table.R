...
ipc_loa <- data.frame(analysis_var = c(), analysis_type = c(), group_var = c())

fcs_cat_bit <- data.frame(
  analysis_var = c(
    "fsl_fcs_cat",
    fsl_fcs_cereal,
    fsl_fcs_legumes,
...
hdds_cat_bit <- data.frame(
  ...
  group_var = c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA)
)
...
ipc_table_results_analysis <- analysistools::create_analysis(
  design = ipc_table_design,
  loa = ipc_loa,
  sm_separator = "/"
)
...
no_na_rows <- ipc_table_results_analysis$results_table %>%
  dplyr::filter(
    !(analysis_type == "prop_select_one" & is.na(analysis_var_value))
  )
example_ipc <- presentresults::create_ipc_table(
  ...
  lcsi_value_set = c(yes_val, no_val, exhausted_val, not_applicable_val),
  hhs_cat_values = c(
    "None",
    "No or Little",
    "Moderate",
