...
# ------------------------------------------------------------------------------
label_colname <- tcltk::tk_select.list(
  names(readxl::read_excel(strings['path.tool'], "survey"))[grepl(
    "label",
    names(readxl::read_excel(strings['path.tool'], "survey"))
  )],
  title = "Label column to choose",
  multiple = F
)
...
if (nrow(labels_groups) > 0) {
  warning(
    paste0(
      "You have some duplicated labels in your tool.choices!\nThey are: ",
...
tool.survey <- tool.survey %>%
  filter(
    !name %in% c("hhs_cat", "hhs_score", "rcsi_cat", "lcsi_cat", "fcs_cat")
  )
...
tool_datasheets <- tool.survey %>%
  distinct(datasheet) %>%
  filter(!is.na(.)) %>%
  pull
if (length(sheet_names) != length(tool_datasheets)) {
  warning(paste0(
    "Mismatch in the number of datasheets found in tool.survey (",
    length(tool_datasheets),
    ")\tand in the data (",
    length(sheet_names),
    ")"
  ))
...
if (any(sort(tool_datasheets) != sort(sheet_names))) {
  if (
    params["fix_sheet_names_to_match"] %in% c("data", "data.list", "datalist")
  ) {
    # the 'datasheet' columns in tool.survey will be fixed to match the data sheet names
    cat(paste0(
      "Mismatch in datasheet names will be fixed by changing tool.survey to match the sheet names from the data.\n"
    ))
    for (sheet in sheet_names[-1]) {
      # take the first column from this sheet and find it in tool.survey
      i <- 1
      data_cnames <- data.list[[sheet]] %>%
        select(-contains("/"), -starts_with("_")) %>%
        names
      ...
      old_sheetname <- tool.survey %>%
        filter(name == first_col) %>%
        pull(datasheet)
      # change all occurences of `old_sheetname` to `sheet`
      tool.survey <- tool.survey %>%
        mutate(
          datasheet = ifelse(datasheet %==na% old_sheetname, sheet, datasheet)
        )
    }
    rm(i, first_col, old_sheetname, data_cnames)
  } else if (
    params["fix_sheet_names_to_match"] %in%
      c("tool", "tool.survey", "toolsurvey")
  ) {
    # the names for data.list will be fixed to match tool.survey
    cat(paste0(
      "Mismatch in the names of datasheets will be fixed to match the `datasheet` column from tool.survey.\n"
    ))
    ...
    warning(paste0(
      "Mismatch in datasheet names found in tool.survey (",
      paste(tool_datasheets, collapse = ", "),
      ") and in the data (",
      paste(sheet_names, collapse = ", "),
...
if (
  max(col_counts$num_cols) != col_counts[col_counts$sheet == "main", ]$num_cols
) {
  warning(
    "Unexpectedly, the 'main' sheet is not the one with the most columns! Check if the data looks good, please!"
  )
}
...
mort_daf_variable <- tcltk::tk_select.list(
  tool.survey %>%
    filter(
      q.type %in% c("decimal", "integer", "select_one", "select_multiple")
    ) %>%
    pull(name),
...
daf <- data.frame(variable = mort_daf_variable) %>%
  dplyr::mutate(section = "Mortality [Part 1]", label = get.label(variable))
...
if (!all(necessary.dap.cols %in% colnames(daf))) {
  stop(paste(
    "Your DAF is missing some necessary columns:",
    paste0(
      necessary.dap.cols[!necessary.dap.cols %in% colnames(daf)],
      collapse = ", "
    )
  ))
}
...
if ("func" %in% names(daf)) {
  ...
  if (any(!daf$func %in% valid.func.params)) {
    stop(
      "There are unexpected func parameters in your DAF: ",
      paste(daf$func[!daf$func %in% valid.func.params], collapse = ", ")
    )
  }
  type_mismatches <- daf %>%
    filter(variable %in% tool.survey$name) %>%
    left_join(
      tool.survey %>% select(name, q.type),
      by = c("variable" = "name")
    ) %>%
    filter(
      func != "count" &
        (q.type != func &
          !(q.type %in%
            c("decimal", "integer", "calculate") &
            func %in% c("mean", "median", "integer", "numeric")))
    )
  if (nrow(type_mismatches) > 0) {
    stop(
      "Invalid func parameter for variable(s): ",
      paste0(
        type_mismatches$variable,
        " (",
        type_mismatches$func,
        ")",
        collapse = ", "
      )
    )
...
daf <- suppressWarnings(
  daf %>%
    mutate(
      var_type = ifelse(
        !variable %in% tool.survey$name,
        list_variables_not_in_tool[variable],
        get.type(variable)
      )
    ) %>%
...
for (column in additional.daf.cols) {
  if (!column %in% names(daf)) {
    daf[[column]] <- NA
    cat(paste0(
      "\n> Your DAF is missing the '",
      column,
      "' column - by default it was filled with NA"
    ))
  }
}
...
if (!"func" %in% names(daf)) {
  ...
  daf <- daf %>%
    mutate(
      func = ifelse(
        func %in% c("integer", "numeric", "mean", "median"),
        "numeric",
        func
      )
    )
...
admin_default <- ifelse(
  "strata" %in% names(data.list$main),
  "strata",
  "overall"
)
if (!"admin" %in% names(daf)) {
  cat(paste0(
    "\n> Your DAF is missing the 'admin' column - by default it was set to '",
    admin_default,
    "'\n"
  ))
