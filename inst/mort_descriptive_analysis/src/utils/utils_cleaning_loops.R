...
add.to.cleaning.log.other.recode.LOOP <- function(data, x) {
  ...
  if (x$ref.type[1] == "select_one") {
    res <- add.to.cleaning.log.other.recode.one.LOOP(x)
  }
  if (x$ref.type[1] == "select_multiple") {
    res <- add.to.cleaning.log.other.recode.multiple.LOOP(data, x)
  }
  if (res == "err") {
    cat("Errors encountered while recoding other. Check the warnings!")
  }
}
...
add.to.cleaning.log.other.remove.LOOP <- function(data, x) {
  ...
  df <- data.frame(
    uuid = x$uuid,
    loop_index = x$loop_index,
    variable = x$name,
    issue = issue,
    old.value = x$response.fr,
    new.value = NA
  )
  ...
  if (x$ref.type[1] == "select_multiple") {
    if (!is.na(x$loop_index) & str_starts(x$loop_index[1], "loop_hh_roster")) {
      old.value <- as.character(data[
        data$loop_index == x$loop_index[1],
        x$ref.name
      ])
    } else if (
      !is.na(x$loop_index) & str_starts(x$loop_index[1], "loop_ind_health")
    ) {
      old.value <- as.character(data[
        data$loop_index == x$loop_index[1],
        x$ref.name
      ...
      !is.na(x$loop_index) & str_starts(x$loop_index[1], "loop_child_nutrition")
    ) {
      old.value <- as.character(data[
        data$loop_index == x$loop_index[1],
        x$ref.name
      ...
      !is.na(x$loop_index) & str_starts(x$loop_index[1], "loop_died_member")
    ) {
      old.value <- as.character(data[
        data$loop_index == x$loop_index[1],
        x$ref.name
...
add.to.cleaning.log.other.recode.multiple.LOOP <- function(data, x) {
  ...
  for (choice in choices) {
    ...
    if (variable.name %in% colnames(data)) {
      if (
        !is.na(x$loop_index) & str_starts(x$loop_index[1], "loop_hh_roster")
      ) {
        ...
        !is.na(x$loop_index) &
          str_starts(x$loop_index[1], "loop_child_nutrition")
      ) {
        old.boolean <- data[[variable.name]][data$loop_index == x$loop_index[1]]
      } else if (
        !is.na(x$loop_index) & str_starts(x$loop_index[1], "loop_died_member")
...
add.to.cleaning.log.LOOP <- function(
  checks,
  check.id,
  question.names = c(),
  issue = "",
...
save.follow.up.requests.LOOP <- function(cleaning.log, data) {
  ...
  c <- cl %>%
    arrange(match(check.id, str_sort(unique(cl$check.id), numeric = T)))
    ...
    for (r in 2:nrow(cl)) {
      if (
        ...
        addStyle(
          wb,
          "Follow-up",
          style = createStyle(fgFill = random.color, wrapText = T),
          rows = r:(r + 1),
          cols = col.id
        )
      } else {
...
recode.multiple.add.choice.LOOP <- function(
  data,
  variable,
  choice,
  issue,
  ...
  if (!isLoop) {
    data <- data %>%
      filter(str_detect(!!sym(variable), choice, negate = T)) %>%
  ...
  if (nrow(data) > 0) {
    cl_cummulative <- select(data, uuid, loop_index, variable) %>%
      ...
      mutate(
        variable = variable,
        new.value = paste(old.value, choice),
        issue = issue
      )
...
recode.multiple.remove.choice.LOOP <- function(
  data,
  variable,
  choice,
  issue,
  ...
  if (!isLoop) {
    data <- data %>%
      filter(str_detect(!!sym(variable), choice)) %>%
      mutate(loop_index = NA)
  }
  if (nrow(data) > 0) {
    cl_cummulative <- select(data, uuid, loop_index, variable) %>%
      ...
      mutate(
        variable = variable,
        new.value = str_squish(str_remove(old.value, choice)),
        issue = issue
      )
  ...
  if (nrow(clog) == 0) {
    ...
    for (r in 1:nrow(clog)) {
      ...
      if (!isLoop) {
        ...
        if (
          data[data$loop_index == loop_index, variable] %!=na% clog$old.value[r]
        ) {
          warning(paste0(
            "Value in data is different than old.value in Cleaning log!\nUUID: ",
            loop_index,
    ...
    if (length(missinguuids > 0) && !isLoop) {
      warning(paste0(
        "uuids from cleaning log not found in data:\n",
        paste0(missinguuids, collapse = "\n")
      ))
    }
    if (length(missingloop_indexs) > 0 && isLoop) {
      warning(paste0(
        "loop_indexs from cleaning log not found in data:\n",
