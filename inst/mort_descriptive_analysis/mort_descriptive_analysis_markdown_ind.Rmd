...
srvey_mortality <- srvyr::as_survey_design(df_mortality_long %>%
                                                             dplyr::mutate(death = ifelse(is.na(death),0,as.numeric(death)),
                                                                           birth = ifelse(is.na(birth),0,as.numeric(birth)),
...
```{r, include=FALSE}
crude_age_group <- srvey_mortality %>%
...
plot_results.summary_age_group <- ggplot2::ggplot(by_age_group %>%
  ...
  ggplot2::labs(caption = c("Black Dotted Line: Crude Threshold (WHO)", "Red Dotted Line: Under 5 Threshold (WHO)")) +
  ggplot2::theme(plot.caption = ggplot2::element_text(hjust = c(0.2,0.8),color = c("black","red")))+
  ggplot2::facet_wrap(~age_group_2)
```
...
```{r, include=FALSE}
crude_pop_group <- srvey_mortality %>%
  group_by(!!rlang::sym(population_group)) %>% 
  srvyr::summarise(x = srvyr::survey_ratio(numerator = death * 10000,
                                           denominator = person_time,
                                           vartype = "ci")) %>%
  dplyr::mutate_at(vars(c(x,x_low,x_upp)),~round(.,2)) %>%
...
under5_pop_group <-  srvey_mortality%>%
  group_by(!!rlang::sym(population_group)) %>% 
    srvyr::summarise(x = srvyr::survey_ratio(numerator = death_under5 * 10000,
                                             denominator = under_5_pt,
                                             vartype = "ci"))%>%
...
by_pop_group <- rbind(crude_pop_group,under5_pop_group) %>%
  rename(`point.est` = "x",
          ...
          `95%uci` = "x_upp") %>%
  dplyr::relocate(variable, .before = 1) %>% 
  dplyr::rename("group" = population_group) %>% 
  filter(!is.na(`95%lci`))
```
...
```{r, include=FALSE}
crude_admin1_pop_group <- srvey_mortality %>%
    dplyr::group_by(admin1, !!rlang::sym(population_group)) %>%
    srvyr::summarise(x = srvyr::survey_ratio(numerator = death * 10000,
                                             denominator = person_time,
                                             vartype = "ci",
                                             deff = "replace")) %>%
...
under5_admin1_pop_group <- srvey_mortality %>%
    dplyr::group_by(admin1, !!rlang::sym(population_group)) %>%
    srvyr::summarise(x = srvyr::survey_ratio(numerator = death_under5 * 10000,
                                             denominator = under_5_pt,
                                             vartype = "ci",
...
by_admin1_pop_group <- rbind(crude_admin1_pop_group,under5_admin1_pop_group) %>%
  dplyr::relocate(variable, .before = 1)%>% 
   ...
   dplyr::relocate(variable, .before = 1)%>%
  filter(!is.na(`95%lci`)) %>% 
  mutate(admin1_group = paste0(admin1, "_", !!rlang::sym(population_group))) %>% 
  select(-c(admin1,population_group)) %>% 
 dplyr::relocate(admin1_group, .before = 2)
```
...
plot_results.summary_admin1_pop_group <- ggplot2::ggplot(by_admin1_pop_group %>%
  ...
  ggplot2::labs(caption = c("Black Dotted Line: Crude Threshold (WHO)", "Red Dotted Line: Under 5 Threshold (WHO)")) +
  ggplot2::theme(plot.caption = ggplot2::element_text(hjust = c(0.2,0.8),color = c("black","red")))+
  ggplot2::facet_wrap(~admin1_group)
```
...
```{r, include=FALSE}
crude_hoh_gender <- srvey_mortality %>%
    dplyr::group_by(!!rlang::sym(hoh_gender)) %>%
    srvyr::summarise(x = srvyr::survey_ratio(numerator = death * 10000,
                                             denominator = person_time,
                                             vartype = "ci",
                                             deff = "replace")) %>%
...
under5_hoh_gender <- srvey_mortality %>%
    dplyr::group_by(!!rlang::sym(hoh_gender)) %>%
    srvyr::summarise(x = srvyr::survey_ratio(numerator = death_under5 * 10000,
                                             denominator = under_5_pt,
                                             vartype = "ci",
...
```{r, include=FALSE}
crude_income_source <- srvey_mortality %>%
    dplyr::group_by(income_source_category) %>%
    srvyr::summarise(x = srvyr::survey_ratio(numerator = death * 10000,
                                             denominator = person_time,
                                             vartype = "ci",
                                             deff = "replace")) %>%
...
under5_income_source <- srvey_mortality %>%
    dplyr::group_by(income_source_category) %>%
    srvyr::summarise(x = srvyr::survey_ratio(numerator = death_under5 * 10000,
                                             denominator = under_5_pt,
                                             vartype = "ci",
...
plot_results.summary_income_source <- ggplot2::ggplot(by_income_source %>%
                                               mutate(variable = case_when(variable == "cmr"~"Crude",
                                                      ...
                                                      income_source_category = factor(income_source_category, c("Below 20%", "20% - 40%","40% - 60%","60% - 80%","Above 80%"))), ggplot2::aes(x = variable)) +
  ggplot2::geom_hline(yintercept=1, linetype='dotted', color = 'black', size =1)+
  ggplot2::geom_hline(yintercept=2, linetype='dotted', color = 'red', size =1)+
  ggplot2::geom_errorbar(ggplot2::aes(ymin =`95%lci`, ymax = `95%uci`, width = 0.2, color = "red"))+
  ggplot2::geom_text(ggplot2::aes(x = variable, y = `95%lci`, label = `95%lci`, vjust = 1.5, color = "red", fontface = "bold"), size = 3)+
  ...
  ggplot2::labs(caption = c("Black Dotted Line: Crude Threshold (WHO)", "Red Dotted Line: Under 5 Threshold (WHO)")) +
  ggplot2::theme(plot.caption = ggplot2::element_text(hjust = c(0.2,0.8),color = c("black","red")))+
  ggplot2::facet_wrap(~income_source_category, ncol = 5)
```
...
birth <-  srvey_mortality %>%
    srvyr::summarise(x = srvyr::survey_ratio(numerator = birth * 1000,
                                             denominator = person_time/365,
                                             vartype = "ci",
                                             deff = "replace")) %>%
    dplyr::mutate_at(vars(c(x,x_low,x_upp,x_deff)),~round(.,2)) %>% 
   dplyr::rename(`point.est` = "x",
...
```{r, include=FALSE}
birth_pop_group <- srvey_mortality %>%
  group_by(!!rlang::sym(population_group)) %>% 
    srvyr::summarise(x = srvyr::survey_ratio(numerator = birth * 1000,
                                             denominator = person_time/365,
                                             vartype = "ci",
  ...
  dplyr::select(population_group,`point.est`,`95%lci`,`95%uci`) %>%
  dplyr::mutate(variable = "Birth") %>% 
  dplyr::rename("group" =population_group)
```
