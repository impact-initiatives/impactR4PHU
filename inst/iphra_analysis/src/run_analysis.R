...
strings <- c(
  ...
  population_estimation = svDialogs::dlgInput(
    if (language['language_assessment'] == "English") {
    ...
    "ex: 1500"
  )$res, # this one is appended to the end of filenames
  filename.data = choose.files(
    caption = if (language['language_assessment'] == "English") {
...
## TABULAR  -------------------------------------------------------------------
rmarkdown::render(
  'src/analysis_tabular.Rmd',
  output_file = paste0(
    "./../output/",
    strings['dataset.name.short'],
    "_Tabular_Analysis_",
    strings['out_date'],
    ".html"
  )
