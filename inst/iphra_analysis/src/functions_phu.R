...
check_fs_flags <- function(
  ...
  results <- .dataset %>%
    ...
    mutate(
      date_dc_date = lubridate::as_date(
        as.numeric(date_dc_date),
        origin = "1899-12-30"
      )
    )
  ...
  if (all(fcs_flag_columns %in% colnames(.dataset))) {
    ...
    results2 <- .dataset %>%
      ...
      dplyr::mutate(
        flag_meat_cereal_ratio = ifelse(
          is.na(!!rlang::sym(fcs_cereal)),
          NA,
          ifelse(!!rlang::sym(fcs_cereal) < fcs_meat, 1, 0)
        ),
        flag_low_cereal = ifelse(
          is.na(!!rlang::sym(fcs_cereal)),
          NA,
          ifelse(!!rlang::sym(fcs_cereal) < 5, 1, 0)
        ),
        flag_low_fcs = ifelse(
          is.na(!!rlang::sym(fcs_score)),
          NA,
          ifelse(!!rlang::sym(fcs_score) <= 10, 1, 0)
        ),
        flag_high_fcs = ifelse(
          is.na(!!rlang::sym(fcs_score)),
          NA,
          ifelse(!!rlang::sym(fcs_score) >= 56, 1, 0)
        ),
        flag_low_oil = ifelse(
          is.na(!!rlang::sym(fcs_cereal)),
          NA,
          ifelse(!!rlang::sym(fcs_oil) < 5, 1, 0)
        )
      ) %>%
      dplyr::rowwise() %>%
      dplyr::mutate(
        sd_foods = sd(
          c(
            !!rlang::sym(fcs_cereal),
            !!rlang::sym(fcs_legumes),
            !!rlang::sym(fcs_dairy),
            !!rlang::sym(fcs_meat),
            !!rlang::sym(fcs_veg),
            !!rlang::sym(fcs_fruit),
        ...
        flag_sd_foodgroup = dplyr::case_when(
          sd_foods < 0.8 ~ 1,
          .default = 0,
          TRUE ~ NA
        )
  ...
  if (
    all(rcsi_flag_columns %in% names(.dataset)) &
      num_children %in% names(.dataset)
  ) {
    results2 <- .dataset %>%
      dplyr::mutate_at(vars(rcsi_flag_columns), as.numeric) %>%
      dplyr::mutate(
        flag_protein_rcsi = ifelse(
          is.na(!!rlang::sym(rcsi_score)),
          NA,
          ifelse(
            is.na(!!rlang::sym(fcs_cereal)),
            NA,
            ifelse(
              !!rlang::sym(rcsi_score) >= 19 &
                (!!rlang::sym(fcs_dairy) >= 5 | !!rlang::sym(fcs_meat) >= 5),
              1,
              0
            )
          )
        ),
        flag_fcs_rcsi = ifelse(
          is.na(!!rlang::sym(rcsi_score)),
          NA,
          ifelse(
            is.na(!!rlang::sym(fcs_score)),
            NA,
            ifelse(
              !!rlang::sym(fcs_score) < 35 & !!rlang::sym(rcsi_score) <= 4,
              1,
              0
            )
          )
        ),
        flag_high_rcsi = ifelse(
          is.na(!!rlang::sym(rcsi_score)),
          NA,
          ifelse(!!rlang::sym(rcsi_score) >= 43, 1, 0)
        ),
        flag_rcsi_children = ifelse(
          is.na(!!rlang::sym(rcsi_mealadult)),
          NA,
          ifelse(
            !is.na(!!rlang::sym(rcsi_mealadult)) &
              as.numeric(num_children) == 0,
            1,
            0
          )
        ),
        flag_fcsrcsi_box = dplyr::case_when(
          as.numeric(!!rlang::sym(rcsi_score)) > 18 &
            as.numeric(!!rlang::sym(fcs_score)) > 56 ~
            1,
          .default = 0,
          TRUE ~ NA
        )
      ...
      dplyr::mutate(
        sd_rcsicoping = sd(
          c(
            !!rlang::sym(rcsi_lessquality),
            !!rlang::sym(rcsi_borrow),
            !!rlang::sym(rcsi_mealsize),
            !!rlang::sym(rcsi_mealadult),
            !!rlang::sym(rcsi_mealnb)
          ),
          na.rm = TRUE
        )
      ) %>%
      dplyr::ungroup() %>%
      dplyr::mutate(
        flag_sd_rcsicoping = dplyr::case_when(
          sd_rcsicoping < 0.8 & !!rlang::sym(rcsi_score) < 4 ~ 1,
          .default = 0,
          TRUE ~ NA
        )
      ) %>%
  ...
  if (all(hhs_flag_columns %in% names(.dataset))) {
    results2 <- .dataset %>%
      dplyr::mutate(
        flag_severe_hhs = ifelse(
          is.na(!!rlang::sym(hhs_score)),
          NA,
          ifelse(!!rlang::sym(hhs_score) >= 5, 1, 0)
        )
      ) %>%
  ...
  if (all(lcs_flag_columns %in% names(.dataset))) {
    results2 <- .dataset %>%
      dplyr::mutate(
        flag_lcsi_coherence = ifelse(
          is.na(!!rlang::sym(lcsi_emergency)),
          NA,
          ifelse(
            !!rlang::sym(lcsi_emergency) == 1 &
              !!rlang::sym(lcsi_stress) == 0 |
              !!rlang::sym(lcsi_emergency) == 1 &
                !!rlang::sym(lcsi_crisis) == 0 |
              !!rlang::sym(lcsi_crisis) == 1 & !!rlang::sym(lcsi_stress) == 0,
            1,
            0
          )
        ),
        flag_lcsi_severity = dplyr::case_when(
          !!rlang::sym(lcsi_emergency) == 1 ~ 1,
          .default = 0,
          TRUE ~ NA
        )
      )
    ...
    results2$lcsi.count.na <- apply(results2[c(lcs_variables)], 1, function(x) {
      sum(x == "not_applicable")
    })
    ...
    suppressWarnings(
      agric <- lcs_variables[which(grepl(
        "agriculture|crop|crops|farm",
        get.label(lcs_variables)
      ))]
    )

    suppressWarnings(
      livest <- lcs_variables[which(grepl(
        "livestock|livestocks|animal",
        get.label(lcs_variables)
      ))]
    )

    suppressWarnings(
      displ <- lcs_variables[which(grepl(
        "displaced|migration|migrated",
        get.label(lcs_variables)
      ))]
    )

    if (length(agric) > 0) {
      results2$flag_lcsi_liv_agriculture <- dplyr::case_when(
        rowSums(sapply(results2[agric], function(i) grepl("yes", i))) > 0 &
          any(results2[income_types] == "sell_agri_prod") > 0 ~
          1,
        .default = 0,
        TRUE ~ NA
      ) ## Fix second part to take only select_one from three columns
    }

    if (length(livest) > 0) {
      results2$flag_lcsi_liv_livestock <- dplyr::case_when(
        rowSums(sapply(results2[livest], function(i) grepl("yes", i))) > 0 &
          any(results2[income_types] == "sell_anim_prod") > 0 ~
          1,
        .default = 0,
        TRUE ~ NA
      ) ## Fix second part to take only select_one from three columns
    ...
    if (length(displ) > 0) {
      results2$flag_lcsi_displ <- dplyr::case_when(
        rowSums(sapply(results2[displ], function(i) grepl("yes", i))) > 0 &
          results2["residency_status"] == "idp" ~
          1,
        .default = 0,
        TRUE ~ NA
      ) ## Fix second part to take only select_one from three columns
  ...
  if (all(hdds_flag_columns %in% names(.dataset))) {
    results2 <- .dataset %>%
      dplyr::mutate(
        flag_low_sugar_cond_hdds = ifelse(
          is.na(!!rlang::sym(hdds_score)),
          NA,
          ifelse(
            (!!rlang::sym(hdds_score) <= 2 &
              !!rlang::sym(hdds_sugar) == "yes" &
              !!rlang::sym(hdds_condiments) == "yes") |
              (!!rlang::sym(hdds_score) <= 1 &
                !!rlang::sym(hdds_sugar) == "yes") |
              (!!rlang::sym(hdds_score) <= 1 &
                !!rlang::sym(hdds_condiments) == "yes"),
            1,
  ...
  if (is.loop) {
    ...
    results2 <- results2 %>%
      dplyr::mutate(
        ...
        flag_no_container = case_when(
          !!rlang::sym(water_source) == "1" &
            is.na(!!rlang::sym(num_containers)) ~
            1,
          .default = 0,
          TRUE ~ NA
        ),
        flag_not_immediate = case_when(
          !!rlang::sym(water_source) == "1" &
            !!rlang::sym(water_collect_time) != "inside_compound" ~
            1,
          .default = 0,
          TRUE ~ NA
        )
...
check_nut_flags <- function(
  ...
  results2 <- results2 %>%
    dplyr::mutate(
      severe_mfaz = ifelse(is.na(mfaz), NA, ifelse(mfaz < -3, 1, 0)),
      moderate_mfaz = ifelse(
        is.na(mfaz),
        NA,
        ifelse(mfaz >= -3 & mfaz < -2, 1, 0)
      ),
      global_mfaz = ifelse(is.na(mfaz), NA, ifelse(mfaz < -2, 1, 0)),
      severe_mfaz = ifelse(
        is.na(!!rlang::sym(edema_confirm)),
        severe_mfaz,
        ifelse(!!rlang::sym(edema_confirm) == "y", 1, severe_mfaz)
      ),
      global_mfaz = ifelse(
        is.na(!!rlang::sym(edema_confirm)),
        global_mfaz,
        ifelse(!!rlang::sym(edema_confirm) == "y", 1, global_mfaz)
      ),
      severe_mfaz = ifelse(age_months < 6 | age_months >= 60, NA, severe_mfaz),
      moderate_mfaz = ifelse(
        age_months < 6 | age_months >= 60,
        NA,
        moderate_mfaz
      ),
      ...
      sam_muac = ifelse(
        is.na(nut_muac_cm),
        NA,
        ifelse(nut_muac_cm < 11.5, 1, 0)
      ),
      mam_muac = ifelse(
        is.na(nut_muac_cm),
        NA,
        ifelse(nut_muac_cm >= 11.5 & nut_muac_cm < 12.5, 1, 0)
      ),
      gam_muac = ifelse(
        is.na(nut_muac_cm),
        NA,
        ifelse(nut_muac_cm < 12.5, 1, 0)
      ),
      sam_muac = ifelse(
        is.na(!!rlang::sym(edema_confirm)),
        sam_muac,
        ifelse(!!rlang::sym(edema_confirm) == "yes", 1, sam_muac)
      ),
      gam_muac = ifelse(
        is.na(!!rlang::sym(edema_confirm)),
        gam_muac,
        ifelse(!!rlang::sym(edema_confirm) == "yes", 1, gam_muac)
      ),
      ...
      muac_noflag = ifelse(
        is.na(nut_muac_cm),
        NA,
        ifelse(flag_extreme_muac == 1, NA, nut_muac_cm)
      ),
...
check_mortality_flags <- function(
  .dataset,
  dataset_roster,
  dataset_died,
  ...
  results_crude <- .dataset %>%
    ...
    dplyr::mutate(
      date_recall_event = lubridate::as_date(recall_date) + 1,
  ...
  num_birth <- dataset_roster %>%
    ...
    dplyr::mutate(
      birth = ifelse(lubridate::year(date_birth_date) == "2023", 1, 0),
      birth_m = ifelse(
        lubridate::year(date_birth_date) == "2023" & ind_sex == "m",
        1,
        0
      ),
      birth_f = ifelse(
        lubridate::year(date_birth_date) == "2023" & ind_sex == "f",
        1,
        0
      ),
  ...
  num_died <- dataset_died %>%
    dplyr::mutate(
      ...
      num_died_under5_m = ifelse(
        calc_final_age_years_died < 6 & sex_died == "m",
        1,
        0
      ),
      num_died_under5_f = ifelse(
        calc_final_age_years_died < 6 & sex_died == "f",
        1,
        0
      ),
      pt_died = num_days,
    ...
    group_by(uuid) %>%
    dplyr::summarise(
      num_died_under5 = sum(num_died_under5, na.rm = T),
  ...
  results2_crude <- results_crude %>%
    ...
    mutate(
      pt_total = num_hh * as.numeric(date_dc_date - date_recall_event),
      pt_total_m = num_m * as.numeric(date_dc_date - date_recall_event),
      pt_total_f = num_f * as.numeric(date_dc_date - date_recall_event),
      pt_total_under5 = num_under5 *
        as.numeric(date_dc_date - date_recall_event),
      pt_total_under5_m = num_under5_m *
        as.numeric(date_dc_date - date_recall_event),
      pt_total_under5_f = num_under5_f *
        as.numeric(date_dc_date - date_recall_event),
  ...
  sex_age_ratios <- dataset_died %>%
    ...
    dplyr::mutate(
      age_0to5 = ifelse(calc_final_age_years_died < 5, 1, 0),
      age_5plus = ifelse(calc_final_age_years_died >= 5, 1, 0),
      age_0to2 = ifelse(calc_final_age_years_died < 2, 1, 0),
      age_2to5 = ifelse(
        calc_final_age_years_died >= 2 & calc_final_age_years_died < 5,
        1,
        0
      ),
      age_5to10 = ifelse(
        calc_final_age_years_died >= 5 & calc_final_age_years_died < 10,
        1,
        0
      )
    ) %>%
    mutate(
      flag_cause_death = ifelse(
        sex_died == "m" &
          cause_death %in%
            c("post_partum", "during_pregnancy", "during_delivery"),
        1,
        0
      )
    )

  result <- list("crude" = results2_crude, "ratios" = sex_age_ratios)
...
create_fsl_quality_report_phu <- function(
  df,
  grouping = NULL,
  short_report = NULL,
  file_path = NULL
  ...
  if (c("fsl_fcs_score") %in% colnames(df)) {
    results2 <- df %>%
      ...
      dplyr::summarise(
        mean_fcs = round(mean(fsl_fcs_score, na.rm = TRUE), 2),
  ...
  if (c("fsl_rcsi_score") %in% colnames(df)) {
    results2 <- df %>%
      dplyr::group_by(!!rlang::sym(grouping)) %>%
      dplyr::summarise(
        mean_rcsi = round(mean(fsl_rcsi_score, na.rm = TRUE), 2),
        sd_rcsi = round(stats::sd(fsl_rcsi_score, na.rm = TRUE), 2),
        mean_rcsi_lessquality = round(
          mean(fsl_rcsi_lessquality, na.rm = TRUE),
          2
        ),
        sd_rcsi_lessquality = round(
          stats::sd(fsl_rcsi_lessquality, na.rm = TRUE),
          2
        ),
        ...
        sd_rcsi_mealadult = round(
          stats::sd(fsl_rcsi_mealadult, na.rm = TRUE),
          2
        ),
  ...
  if (c("fsl_hhs_score") %in% colnames(df)) {
    results2 <- df %>%
      dplyr::group_by(!!rlang::sym(grouping)) %>%
      dplyr::summarise(
        mean_hhs = round(mean(fsl_hhs_score, na.rm = TRUE), 2),
  ...
  if (c("fsl_hdds_score") %in% colnames(df)) {
    results2 <- df %>%
      dplyr::group_by(!!rlang::sym(grouping)) %>%
      dplyr::summarise(
        mean_hdds = round(mean(fsl_hdds_score, na.rm = TRUE), 2),
  ...
  if (
    length(setdiff(c("fsl_fcs_score", "fsl_rcsi_score"), colnames(df))) == 0
  ) {
    tryCatch(
      {
        results2 <- df %>%
          ...
          dplyr::summarise(
            corr.fcs_rcsi = round(
              as.numeric(stats::cor.test(fsl_fcs_score, fsl_rcsi_score)[4]),
              2
            ),
            corr.fcs_rcsi.pvalue = as.numeric(stats::cor.test(
              fsl_fcs_score,
              fsl_rcsi_score
            )[3])
          )
      ...
      error = function(e) {
        results2 <- df %>%
          dplyr::group_by(!!rlang::sym(grouping)) %>%
          dplyr::summarise(corr.fcs_rcsi = NA, corr.fcs_rcsi.pvalue = NA)
        if (!exists("results")) {
          results <- results2
        } else {
          results <- merge(results, results2)
    ...
    tryCatch(
      {
        results2 <- df %>%
          ...
          dplyr::summarise(
            corr.fcs_hhs = round(
              as.numeric(stats::cor.test(fsl_fcs_score, fsl_hhs_score)[4]),
              2
            ),
            corr.fcs_hhs.pvalue = round(
              as.numeric(stats::cor.test(fsl_fcs_score, fsl_hhs_score)[3]),
              6
            )
          )
        if (!exists("results")) {
      ...
      error = function(e) {
        results2 <- df %>%
          dplyr::group_by(!!rlang::sym(grouping)) %>%
          dplyr::summarise(corr.fcs_hhs = NA, corr.fcs_hhs.pvalue = NA)
        if (!exists("results")) {
          results <- results2
        } else {
          results <- merge(results, results2)
  ...
  if (
    length(setdiff(c("fsl_fcs_score", "fsl_hdds_score"), colnames(df))) == 0
  ) {
    tryCatch(
      {
        results2 <- df %>%
          ...
          dplyr::summarise(
            corr.fcs_hdds = round(
              as.numeric(stats::cor.test(fsl_fcs_score, fsl_hdds_score)[4]),
              2
            ),
            corr.fcs_hdds.pvalue = round(
              as.numeric(stats::cor.test(fsl_fcs_score, fsl_hdds_score)[3]),
              3
            )
          )
        if (!exists("results")) {
      ...
      error = function(e) {
        results2 <- df %>%
          dplyr::group_by(!!rlang::sym(grouping)) %>%
          dplyr::summarise(corr.fcs_hdds = NA, corr.fcs_hdds.pvalue = NA)
        if (!exists("results")) {
          results <- results2
        } else {
          results <- merge(results, results2)
  ...
  if (
    length(setdiff(c("fsl_hdds_score", "fsl_rcsi_score"), colnames(df))) == 0
  ) {
    tryCatch(
      {
        results2 <- df %>%
          dplyr::group_by(!!rlang::sym(grouping)) %>%
          dplyr::summarise(
            corr.hdds_rcsi = round(
              as.numeric(stats::cor.test(fsl_hdds_score, fsl_rcsi_score)[4]),
              2
            ),
            corr.hdds_rcsi.pvalue = round(
              as.numeric(stats::cor.test(fsl_hdds_score, fsl_rcsi_score)[3]),
              3
            )
          )
        if (!exists("results")) {
      ...
      error = function(e) {
        results2 <- df %>%
          dplyr::group_by(!!rlang::sym(grouping)) %>%
          dplyr::summarise(corr.hdds_rcsi = NA, corr.hdds_rcsi.pvalue = NA)
        if (!exists("results")) {
          results <- results2
        } else {
          results <- merge(results, results2)
  ...
  if (
    length(setdiff(c("fsl_hhs_score", "fsl_rcsi_score"), colnames(df))) == 0
  ) {
    tryCatch(
      {
        results2 <- df %>%
          ...
          dplyr::summarise(
            corr.hhs_rcsi = round(
              as.numeric(stats::cor.test(fsl_hhs_score, fsl_rcsi_score)[4]),
              2
            ),
            corr.hhs_rcsi.pvalue = round(
              as.numeric(stats::cor.test(fsl_hhs_score, fsl_rcsi_score)[3]),
              3
            )
          )
        if (!exists("results")) {
      ...
      error = function(e) {
        results2 <- df %>%
          dplyr::group_by(!!rlang::sym(grouping)) %>%
          dplyr::summarise(corr.hhs_rcsi = NA, corr.hhs_rcsi.pvalue = NA)
        if (!exists("results")) {
          results <- results2
        } else {
          results <- merge(results, results2)
  ...
  if (c("flag_fc_cell") %in% names(df)) {
    results2 <- df %>%
      dplyr::mutate(
        ...
        p5 = ifelse(is.na(fc_phase), NA, ifelse(fc_phase == "Phase 5 FC", 1, 0))
      ) %>%
      dplyr::group_by(!!rlang::sym(grouping)) %>%
      dplyr::summarise(
        prop_fc_flags = sum(flag_fc_cell, na.rm = TRUE) /
          sum(!is.na(fc_cell), na.rm = TRUE),
    ...
    results2 <- df %>%
      ...
      dplyr::summarise(
        prop_fc_flags = sum(flag_fc_cell, na.rm = TRUE) /
          sum(!is.na(fc_cell), na.rm = TRUE),
        fes_1 = round(
          sum(food_exp_share == "1", na.rm = TRUE) /
            sum(!is.na(food_exp_share)),
          2
        ),
        fes_2 = round(
          sum(food_exp_share == "2", na.rm = TRUE) /
            sum(!is.na(food_exp_share)),
          2
        ),
        fes_3 = round(
          sum(food_exp_share == "3", na.rm = TRUE) /
            sum(!is.na(food_exp_share)),
          2
        ),
        fes_4 = round(
          sum(food_exp_share == "4", na.rm = TRUE) /
            sum(!is.na(food_exp_share)),
          2
        )
      )
...
create_anthro_quality_report_phu <- function(
  df,
  grouping = NULL,
  file_path = NULL,
  short_report = NULL
  ...
  if (!is.null(file_path)) {
    writexl::write_xlsx(results.table, file_path)
  }
  options(warn = 0)
  return(results.table)
}

create_mortality_quality_report_phu <- function(
  df,
  data_died,
  grouping = NULL,
  file_path = NULL,
  ...
  df2 <- df %>%
    dplyr::group_by(!!rlang::sym(grouping)) %>%
    dplyr::summarize(
      total_people = sum(num_hh, na.rm = TRUE),
    ...
    dplyr::mutate(
      ...
      prop_hh_multiple_death = round(
        (flag_hh_multiple_death / total_people),
        2
      ) *
        100
    ) %>%
    ...
    dplyr::mutate(
      ...
      plaus_hh_multiple_death = case_when(
        prop_hh_multiple_death < 0.5 ~ 0,
  ...
  df3 <- data_died %>%
    dplyr::group_by(!!rlang::sym(grouping)) %>%
    dplyr::mutate(
      sex = case_when(sex_died == "m" ~ 1, sex_died == "f" ~ 2, TRUE ~ NA)
    ) %>%
    dplyr::summarise(
      sex_ratio = round(
        as.numeric(nipnTK::sexRatioTest(
      ...
      age_ratio_0_5.pvalue = stats::chisq.test(
        x = c(sum(!is.na(age_0to5)), sum(!is.na(age_5plus))),
        p = age_under5_ratio
      )[3],
      age_ratio_2_5 = sum(!is.na(age_0to2)) / sum(!is.na(age_2to5)),
      age_ratio_2_5.pvalue = stats::chisq.test(
        x = c(sum(!is.na(age_0to2)), sum(!is.na(age_2to5))),
        p = age_under2to5_ratio
      )[3],
      age_ratio_5_10 = sum(!is.na(age_0to5)) / sum(!is.na(age_5to10)),
      age_ratio_5_10.pvalue = stats::chisq.test(
        x = c(sum(!is.na(age_0to5)), sum(!is.na(age_5to10))),
        p = age_under5to10_ratio
      )[3]
    ) %>%
    dplyr::mutate(
      plaus_sex_ratio = case_when(
        sex_ratio.pvalue > 0.1 ~ 0,
        sex_ratio.pvalue > 0.005 ~ 1,
        sex_ratio.pvalue > 0.001 ~ 3,
        sex_ratio.pvalue <= 0.001 ~ 5,
        TRUE ~ NA
      ),
      plaus_age0to4_5plus_ratio = case_when(
        age_ratio_0_5.pvalue > 0.1 ~ 0,
        age_ratio_0_5.pvalue > 0.005 ~ 1,
        age_ratio_0_5.pvalue > 0.001 ~ 3,
        age_ratio_0_5.pvalue <= 0.001 ~ 5,
        TRUE ~ NA
      ),
      plaus_age0to1_2to4_ratio = case_when(
        age_ratio_0_5.pvalue > 0.1 ~ 0,
        age_ratio_0_5.pvalue > 0.005 ~ 1,
        age_ratio_0_5.pvalue > 0.001 ~ 3,
        age_ratio_0_5.pvalue <= 0.001 ~ 5,
      ...
      plaus_age0to4_5to10_ratio = case_when(
        age_ratio_0_5.pvalue > 0.1 ~ 0,
        age_ratio_0_5.pvalue > 0.005 ~ 1,
        age_ratio_0_5.pvalue > 0.001 ~ 3,
        age_ratio_0_5.pvalue <= 0.001 ~ 5,
  ...
  df4 <- merge(df2, df3) %>%
    dplyr::mutate(
      plaus_mort_score = rowSums(
        across(
          c(
            plaus_overall_cdr,
            plaus_hh_multiple_death,
            plaus_sex_ratio,
            plaus_age0to4_5plus_ratio,
            plaus_age0to1_2to4_ratio,
            plaus_age0to4_5to10_ratio
          ),
          .fns = as.numeric
        ),
        na.rm = T
      ),
      plaus_mort_cat = case_when(
        plaus_mort_score < 10 ~ "Excellent",
...
calculate_plausibility_report_phu <- function(df) {
  ...
  if (c("age_ratio.pvalue") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_ageratio = ifelse(
          age_ratio.pvalue > 0.1,
          0,
          ifelse(
            age_ratio.pvalue > 0.05,
            1,
            ifelse(
              age_ratio.pvalue > 0.001,
              3,
  ...
  if (
    (c("sex_ratio.pvalue") %in% names(df)) &
      !(c("cdr") %in%
        names(df))
  ) {
    df <- df %>%
      dplyr::mutate(
        plaus_sexratio = ifelse(
          sex_ratio.pvalue > 0.1,
          0,
          ifelse(
            sex_ratio.pvalue > 0.05,
            1,
            ifelse(
              sex_ratio.pvalue > 0.001,
              3,
  ...
  if (c("dps_muac") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_dps_muac = ifelse(
          dps_muac >= 0 & dps_muac < 8,
          0,
          ifelse(
            dps_muac >= 8 & dps_muac < 13,
            1,
            ifelse(
              dps_muac >= 13 & dps_muac < 3,
              4,
  ...
  if (c("flag_perc_mfaz_children") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_perc_mfaz_children = ifelse(
          flag_perc_mfaz_children >= 0 & flag_perc_mfaz_children < 8,
          0,
          ifelse(
            flag_perc_mfaz_children >= 8 & flag_perc_mfaz_children < 13,
            5,
            ifelse(
              flag_perc_mfaz_children >= 13 & flag_perc_mfaz_children < 20,
              10,
  ...
  if (c("n_children_muac") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_n_children_muac = ifelse(
          n_children_muac > 100,
          0,
          ifelse(
            n_children_muac > 80 & n_children_muac <= 100,
            2,
            ifelse(
              n_children_muac > 50 & n_children_muac <= 80,
              4,
  ...
  if (c("sd_muac_mm") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_sd_muac_mm = ifelse(
          sd_muac_mm < 12,
          0,
          ifelse(
            sd_muac_mm < 14,
            5,
            ifelse(sd_muac_mm < 15, 10, ifelse(sd_muac_mm >= 15, 20, NA))
          )
        )
      )
  }
  if (length(setdiff(anthro_plaus_vars, names(df))) == 0) {
    ...
    df <- df %>%
      dplyr::mutate(
        ...
        plaus_anthro_cat = ifelse(
          plaus_anthro_score >= 0 & plaus_anthro_score <= 9,
          "Excellent",
          ifelse(
            plaus_anthro_score > 9 & plaus_anthro_score <= 19,
            "Good",
            ifelse(
              plaus_anthro_score > 19 & plaus_anthro_score < 25,
              "Acceptable",
    ...
    print(paste0(
      "Not all necessary variables for anthropometric plausibility score and classification. Skipping this step. The dataframe is missing "
    ))
    print(setdiff(anthro_plaus_vars, names(df)))
  }
  mort_plaus_vars <- c(
    "plaus_cdr",
    "plaus_prop_hh_flag_deaths",
    "plaus_sex_ratio.pvalue",
    "plaus_age_ratio_0_5.pvalue",
    "plaus_age_ratio_2_5.pvalue",
    "plaus_age_ratio_5_10.pvalue",
    "plaus_mean_hh_size.pvalue",
    "plaus_prop_joiners",
    "plaus_prop_leavers",
    ...
    df <- df %>%
      dplyr::mutate(
        plaus_cdr = ifelse(
          cdr < 1,
          0,
          ifelse(cdr < 2, 5, ifelse(cdr < 3.5, 10, ifelse(cdr >= 3.5, 20, 0)))
        )
      )
  }
  if (c("prop_hh_flag_deaths") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_prop_hh_flag_deaths = ifelse(
          prop_hh_flag_deaths < 0.005,
          0,
          ifelse(
            prop_hh_flag_deaths < 0.01,
            2,
            ifelse(
              prop_hh_flag_deaths < 0.015,
              5,
  ...
  if (c("age_ratio_0_5.pvalue") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_age_ratio_0_5.pvalue = ifelse(
          age_ratio_0_5.pvalue > 0.1,
          0,
          ifelse(
            age_ratio_0_5.pvalue > 0.05,
            2,
            ifelse(
              age_ratio_0_5.pvalue > 0.001,
              5,
  ...
  if (c("age_ratio_2_5.pvalue") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_age_ratio_2_5.pvalue = ifelse(
          age_ratio_2_5.pvalue > 0.1,
          0,
          ifelse(
            age_ratio_2_5.pvalue > 0.05,
            2,
            ifelse(
              age_ratio_2_5.pvalue > 0.001,
              5,
  ...
  if (c("age_ratio_5_10.pvalue") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_age_ratio_5_10.pvalue = ifelse(
          age_ratio_5_10.pvalue > 0.1,
          0,
          ifelse(
            age_ratio_5_10.pvalue > 0.05,
            2,
            ifelse(
              age_ratio_5_10.pvalue > 0.001,
              5,
  ...
  if (c("mean_hh_size.pvalue") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_mean_hh_size.pvalue = ifelse(
          mean_hh_size.pvalue > 0.05,
          0,
          ifelse(
            mean_hh_size.pvalue > 0.001,
            2,
            ifelse(
              mean_hh_size.pvalue > 1e-04,
              5,
  ...
  if (length(setdiff(c("poisson_pvalues.deaths"), names(df))) == 0) {
    df <- df %>%
      dplyr::mutate(
        plaus_poisson_pvalues.deaths = ifelse(
          poisson_pvalues.deaths > 0.1,
          0,
          ifelse(
            poisson_pvalues.deaths > 0.05,
            2,
            ifelse(
              poisson_pvalues.deaths > 0.001,
              5,
  ...
  if (length(setdiff(mort_plaus_vars, names(df))) == 0) {
    df <- df %>%
      dplyr::mutate(
        mort_plaus_score = plaus_cdr +
          ...
          plaus_prop_leavers +
          plaus_poisson_pvalues.deaths,
        mort_plaus_cat = ifelse(
          mort_plaus_score >= 0 & mort_plaus_score < 10,
          "Excellent (0-<10)",
          ifelse(
            mort_plaus_score >= 10 & mort_plaus_score < 20,
            "Good (10-<20)",
            ifelse(
              mort_plaus_score >= 20 & mort_plaus_score < 25,
              "Acceptable (20 - <25)",
    ...
    print(paste0(
      "Not all necessary variables for mortality plausibility score and classification. Skipping this step. The dataframe is missing "
    ))
  ...
  if (c("flag_low_fcs") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_low_fcs = dplyr::case_when(
          flag_low_fcs < 2 ~ 0,
  ...
  if (c("flag_high_fcs") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_high_fcs = dplyr::case_when(
          flag_high_fcs < 2 ~ 0,
  ...
  if (c("flag_sd_foodgroup") %in% names(df)) {
    ...
    df <- df %>%
      dplyr::mutate(
        plaus_flag_sd_foodgroup = dplyr::case_when(
          flag_sd_foodgroup < 2 ~ 0,
  ...
  if (c("flag_meat_cereal_ratio") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_meat_cereal_ratio = dplyr::case_when(
          flag_meat_cereal_ratio < 2 ~ 0,
  ...
  if (c("flag_low_sugar_cond_hdds") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_low_sugar_cond_hdds = dplyr::case_when(
          flag_low_sugar_cond_hdds < 2 ~ 0,
  ...
  if (length(setdiff(c(fcs_plaus_vars), names(df))) < 6) {
    ...
    df <- df %>%
      dplyr::rowwise() %>%
      dplyr::mutate(
        plaus_fcs = sum(!!!rlang::syms(plaus_nms), na.rm = TRUE)
      ) %>%
      dplyr::ungroup()
  }
  if (c("flag_high_rcsi") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_high_rcsi = dplyr::case_when(
          flag_high_rcsi < 2 ~ 0,
  ...
  if (c("flag_sd_rcsicoping") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_sd_rcsicoping = dplyr::case_when(
          flag_sd_rcsicoping < 2 ~ 0,
  ...
  if (c("flag_protein_rcsi") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_protein_rcsi = dplyr::case_when(
          flag_protein_rcsi < 2 ~ 0,
  ...
  if (c("flag_rcsi_children") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_rcsi_children = dplyr::case_when(
          flag_rcsi_children < 2 ~ 0,
          flag_protein_rcsi >= 2 & flag_protein_rcsi < 10 ~ 2,
          flag_protein_rcsi >= 10 ~ 3,
          TRUE ~ 0
        )
  ...
  rcsi_plaus_vars <- c(
    "plaus_flag_protein_rcsi",
    "plaus_flag_sd_rcsicoping",
    "plaus_flag_high_rcsi",
    "plaus_sd_rcsi",
    "plaus_flag_rcsi_children"
  )
  if (length(setdiff(c(rcsi_plaus_vars), names(df))) < 5) {
    ...
    df <- df %>%
      dplyr::rowwise() %>%
      dplyr::mutate(
        plaus_rcsi = sum(!!!rlang::syms(plaus_nms), na.rm = TRUE)
      ) %>%
      dplyr::ungroup()
  }
  if (c("flag_severe_hhs") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_severe_hhs = dplyr::case_when(
          flag_severe_hhs < 1 ~ 0,
  ...
  if (length(setdiff(c(hhs_plaus_vars), names(df))) < 2) {
    ...
    df <- df %>%
      dplyr::rowwise() %>%
      dplyr::mutate(
        plaus_hhs = sum(!!!rlang::syms(plaus_nms), na.rm = TRUE)
      ) %>%
      dplyr::ungroup()
  }
  if (c("flag_lcsi_liv_livestock") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_lcsi_liv_livestock = dplyr::case_when(
          flag_lcsi_liv_livestock < 2 ~ 0,
  ...
  if (c("flag_lcsi_liv_agriculture") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_lcsi_liv_agriculture = dplyr::case_when(
          flag_lcsi_liv_agriculture < 2 ~ 0,
  ...
  if (
    c("flag_lcsi_liv_agriculture") %in%
      names(df) |
      c("flag_lcsi_liv_livestock") %in% names(df)
  ) {
    if (
      length(setdiff(
        c("flag_lcsi_liv_agriculture", "flag_lcsi_liv_livestock"),
        names(df)
      )) ==
        0
    ) {
      df <- df %>%
        dplyr::mutate(
          plaus_flag_lcsi_agr_livestock = dplyr::case_when(
            plaus_flag_lcsi_liv_livestock >= plaus_flag_lcsi_liv_agriculture ~
              plaus_flag_lcsi_liv_livestock,
            plaus_flag_lcsi_liv_livestock < plaus_flag_lcsi_liv_agriculture ~
              plaus_flag_lcsi_liv_agriculture,
            TRUE ~ 0
          )
        )
    } else if (
      length(setdiff(c("flag_lcsi_liv_agriculture"), names(df))) == 0
    ) {
      df <- df %>%
        dplyr::mutate(
          plaus_flag_lcsi_agr_livestock = plaus_flag_lcsi_liv_agriculture
        )
    } else if (length(setdiff(c("flag_lcsi_liv_livestock"), names(df))) == 0) {
      df <- df %>%
        dplyr::mutate(
          plaus_flag_lcsi_agr_livestock = plaus_flag_lcsi_liv_livestock
        )
    }
  }
  if (c("flag_lcsi_coherence") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_lcsi_coherence = dplyr::case_when(
          flag_lcsi_coherence < 2 ~ 0,
  ...
  if (c("flag_lcsi_na") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_lcsi_na = dplyr::case_when(
          flag_lcsi_na < 2 ~ 0,
  ...
  if (c("flag_lcsi_severity") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_lcsi_severity = dplyr::case_when(
          flag_lcsi_severity < 2 ~ 0,
  ...
  lcs_plaus_vars <- c(
    "plaus_flag_lcsi_agr_livestock",
    "plaus_flag_lcsi_coherence",
    "plaus_flag_lcsi_na",
    "plaus_flag_lcsi_severity"
  )
  if (length(setdiff(lcs_plaus_vars, names(df))) < 4) {
    ...
    df <- df %>%
      dplyr::rowwise() %>%
      dplyr::mutate(
        plaus_lcsi = sum(!!!rlang::syms(plaus_nms), na.rm = TRUE)
      ) %>%
      dplyr::ungroup()
  }
  if (
    length(setdiff(c("corr.fcs_rcsi", "corr.fcs_rcsi.pvalue"), names(df))) == 0
  ) {
    df <- df %>%
      dplyr::mutate(
        plaus_corr.fcs_rcsi = ifelse(
          corr.fcs_rcsi < -0.2 & corr.fcs_rcsi.pvalue < 0.05,
          0,
          ifelse(
            corr.fcs_rcsi < -0.2 & corr.fcs_rcsi.pvalue >= 0.05,
            1,
  ...
  if (
    length(setdiff(c("corr.fcs_hhs", "corr.fcs_hhs.pvalue"), names(df))) == 0
  ) {
    df <- df %>%
      dplyr::mutate(
        plaus_corr.fcs_hhs = ifelse(
          corr.fcs_hhs < -0.2 & corr.fcs_hhs.pvalue < 0.05,
          0,
          ifelse(
            corr.fcs_hhs < -0.2 & corr.fcs_hhs.pvalue >= 0.05,
            1,
  ...
  if (
    length(setdiff(c("corr.hhs_rcsi", "corr.hhs_rcsi.pvalue"), names(df))) == 0
  ) {
    df <- df %>%
      dplyr::mutate(
        plaus_corr.hhs_rcsi = ifelse(
          corr.hhs_rcsi > 0.2 & corr.hhs_rcsi.pvalue < 0.05,
          0,
          ifelse(
            corr.hhs_rcsi > 0.2 & corr.hhs_rcsi.pvalue >= 0.05,
            1,
  ...
  if (c("prop_fc_flags") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_prop_fc_flags = ifelse(
          prop_fc_flags < 0.02,
          0,
          ifelse(prop_fc_flags < 0.1, 2, ifelse(prop_fc_flags >= 0.1, 4, 0))
        )
      )
  }
  if (c("flag_fcsrcsi_box") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_fcsrcsi_box = dplyr::case_when(
          flag_fcsrcsi_box < 2 ~ 0,
  ...
  if (c("flag_fcs_rcsi") %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_flag_fcs_rcsi = dplyr::case_when(
          flag_fcs_rcsi < 2 ~ 0,
  ...
  other_fsl_plaus_vars <- c(
    "plaus_prop_fc_flags",
    "plaus_corr.hhs_rcsi",
    "plaus_corr.fcs_hhs",
    "plaus_corr.fcs_rcsi",
    "plaus_flag_fcsrcsi_box",
    "plaus_flag_fcs_rcsi"
  )
  if (length(setdiff(other_fsl_plaus_vars, names(df))) < 6) {
    ...
    df <- df %>%
      dplyr::rowwise() %>%
      dplyr::mutate(
        plaus_other_fsl = sum(!!!rlang::syms(plaus_nms), na.rm = TRUE)
      ) %>%
      dplyr::ungroup()
  }
  all_fsl_plaus_vars <- c(
    "plaus_lcsi",
    "plaus_fcs",
    "plaus_rcsi",
    "plaus_hhs",
  ...
  if (length(setdiff(all_fsl_plaus_vars, names(df))) < 5) {
    ...
    df <- df %>%
      dplyr::rowwise() %>%
      dplyr::mutate(
        plaus_fsl_score = sum(!!!rlang::syms(plaus_nms), na.rm = TRUE)
      ) %>%
      dplyr::ungroup() %>%
      dplyr::mutate(
        plaus_fsl_cat = dplyr::case_when(
          plaus_fsl_score < 20 ~ "Good",
  ...
  if (c("mad_ratio.pvalue") %in% colnames(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_mad_ratio.pvalue = ifelse(
          mad_ratio.pvalue > 0.05,
          0,
          ifelse(
            mad_ratio.pvalue > 0.001,
            5,
            ifelse(
              mad_ratio.pvalue > 1e-04,
              10,
  ...
  if (c("prop_flag_high_mdd_low_mmf") %in% colnames(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_prop_flag_high_mdd_low_mmf = ifelse(
          prop_flag_high_mdd_low_mmf < 0.01,
          0,
          ifelse(
            prop_flag_high_mdd_low_mmf < 0.05,
            5,
            ifelse(
              prop_flag_high_mdd_low_mmf < 0.1,
              10,
  ...
  if (c("age_ratio_under6m_6to23m.pvalue") %in% colnames(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_age_ratio_under6m_6to23m.pvalue = ifelse(
          age_ratio_under6m_6to23m.pvalue > 0.05,
          0,
          ifelse(
            age_ratio_under6m_6to23m.pvalue > 0.01,
            2,
            ifelse(
              age_ratio_under6m_6to23m.pvalue > 0.001,
              5,
  ...
  if (c("sd_mdd") %in% colnames(df)) {
    df <- df %>%
      dplyr::mutate(
        plaus_sdd_mdd = ifelse(
          sd_mdd > 1 & sd_mdd < 2,
          0,
          ifelse(
            sd_mdd > 0.8 & sd_mdd < 2.2,
            5,
  ...
  if (c("prop_iycf_caregiver" %in% colnames(df))) {
    df <- df %>%
      dplyr::mutate(
        plaus_prop_iycf_caregiver = ifelse(
          prop_iycf_caregiver >= 0.9,
          0,
          ifelse(
            prop_iycf_caregiver >= 0.8,
            2,
            ifelse(
              prop_iycf_caregiver >= 0.7,
              5,
  ...
  iycf_plaus_vars <- c(
    "plaus_sdd_mdd",
    "plaus_age_ratio_under6m_6to23m.pvalue",
    "plaus_sexratio",
    "plaus_prop_flag_high_mdd_low_mmf",
  ...
  if (length(setdiff(iycf_plaus_vars, colnames(df))) == 0) {
    if (!(c("plaus_prop_iycf_caregiver") %in% colnames(df))) {
      ...
      print(
        "No iycf_caregiver variable was available. Plaus penalty of 10 applied assuming this wasn't done during the survey."
      )
    }
    df <- df %>%
      dplyr::mutate(
        iycf_plaus_score = plaus_prop_iycf_caregiver +
          plaus_sdd_mdd +
          plaus_age_ratio_under6m_6to23m.pvalue +
          plaus_sexratio +
          plaus_prop_flag_high_mdd_low_mmf +
          plaus_mad_ratio.pvalue,
        iycf_plaus_cat = ifelse(
          iycf_plaus_score >= 0 & iycf_plaus_score < 10,
          "Excellent (0-<10)",
          ifelse(
            iycf_plaus_score >= 10 & iycf_plaus_score < 15,
            "Good (10-<15)",
            ifelse(
              iycf_plaus_score >= 15 & iycf_plaus_score < 25,
              "Acceptable (15 - <25)",
    ...
    print(paste0(
      "Not all necessary variables for IYCF plausibility score and classification. Skipping this step. The dataframe is missing "
    ))
...
#                             SOME UPDATED PLOTS                               #
################################################################################
plot_anthro_age_distribution_phu <- function(
  df,
  index,
  file_path = NULL,
  wdth = NULL,
    ...
    dplyr::mutate(
      age_group = ifelse(
        ...
        ifelse(
          ...
          "<6 months",
          ifelse(
            age_months > 5 & age_months < 18,
            "6-17 months",
            ifelse(
              age_months > 17 & age_months < 30,
              "18-29 months",
              ifelse(
                age_months > 29 & age_months < 42,
                "30-41 months",
                ifelse(
                  age_months > 41 & age_months < 54,
                  "42-53 months",
                  ifelse(
                    age_months > 53 & age_months < 60,
                    "54-59 months",
                    ifelse(age_months > 59, ">59 months", NA)
                  )
                )
              )
            )
  ...
  if (index == "muac") {
    df <- df %>%
      dplyr::filter(muac_flag != 1) %>%
      dplyr::mutate(
        cat = ifelse(
          is.na(gam_muac_noflag),
          NA,
          ifelse(
            sam_muac_noflag == "1",
            "SAM",
            ifelse(
              mam_muac_noflag == "1",
              "MAM",
              ifelse(gam_muac_noflag == "0", "Normal", NA)
            )
          )
        ),
        cat = as.factor(cat),
        cat = factor(cat, levels = c("SAM", "MAM", "Normal"))
      )
    ...
    df <- df %>%
      dplyr::filter(flag_sd_mfaz != 1) %>%
      dplyr::mutate(
        cat = ifelse(
          is.na(global_mfaz_noflag),
          NA,
          ifelse(
            severe_mfaz_noflag == "1",
            "SAM",
            ifelse(
              moderate_mfaz_noflag == "1",
              "MAM",
              ifelse(global_mfaz_noflag == "0", "Normal", NA)
            )
          )
        ),
        cat = as.factor(cat),
    ...
    df <- df %>%
      dplyr::filter(wfhz_smart_flag != 1) %>%
      dplyr::mutate(
        gam_wfhz_noflag = as.character(gam_wfhz_noflag),
        mam_wfhz_noflag = as.character(mam_wfhz_noflag),
        sam_wfhz_noflag = as.character(sam_wfhz_noflag)
      ) %>%
      dplyr::mutate(
        cat = ifelse(
          is.na(gam_wfhz_noflag),
          NA,
          ifelse(
            sam_wfhz_noflag == "1",
            "SAM",
            ifelse(
              mam_wfhz_noflag == "1",
              "MAM",
              ifelse(gam_wfhz_noflag == "0", "Normal", NA)
            )
          )
        ),
        cat = as.factor(cat),
    ...
    df <- df %>%
      dplyr::filter(hfaz_smart_flag != 1) %>%
      dplyr::mutate(
        cat = ifelse(
          is.na(global_stunting_noflag),
          NA,
          ifelse(
            severe_stunting_noflag == "1",
            "Severe Stunting",
            ifelse(
              moderate_stunting_noflag == "1",
              "Moderate Stunting",
              ifelse(global_stunting_noflag == "0", "Normal", NA)
            )
          )
        ),
        cat = as.factor(cat),
        cat = factor(
          cat,
          levels = c("Severe Stunting", "Moderate Stunting", "Normal")
        )
      )
    title <- "Nutritional Status: Stunting by Age Group"
    color_groups <- c("Severe Stunting", "Moderate Stunting", "Normal")
    color_palette <- c(
      `Severe Stunting` = "indianred",
      `Moderate Stunting` = "khaki",
      Normal = "palegreen4"
    )
  } else if (index == "wfaz") {
    df <- df %>%
      dplyr::filter(wfaz_smart_flag != 1) %>%
      dplyr::mutate(
        cat = ifelse(
          is.na(global_underweight_noflag),
          NA,
          ifelse(
            severe_underweight_noflag == "1",
            "Severe Underweight",
            ifelse(
              moderate_underweight_noflag == "1",
              "Moderate Underweight",
              ifelse(global_underweight_noflag == "0", "Normal", NA)
            )
          )
        ),
        cat = as.factor(cat),
      ...
      dplyr::mutate(
        cat = ifelse(
          ...
          ifelse(
            c_sam == "1",
            "SAM",
            ifelse(c_mam == "1", "MAM", ifelse(c_gam == "0", "Normal", NA))
          )
        ),
        cat = as.factor(cat),
        cat = factor(cat, levels = c("SAM", "MAM", "Normal"))
  ...
  g <- ggplot2::ggplot(
    ...
    ggplot2::geom_text(
      ggplot2::aes(label = paste0(round(pct, 3) * 100, "% - (", n, ")")),
...
plot_age_years_distribution_phu <- function(
  df,
  by_group = NULL,
  min_age = NULL,
  max_age = NULL,
  ...
  if (!(c("age_years") %in% colnames(df))) {
    stop(
      "There is no age_years column in your dataframe. Please check your inputs."
    )
  }
  if (is.null(min_age)) {
    min_age <- 0
  ...
  if (is.null(by_group)) {
    g <- ggplot2::ggplot(data = df, ggplot2::aes(x = get("age_years"))) +
      ggplot2::geom_histogram(binwidth = breaks) +
      ggplot2::scale_x_continuous(
        minor_breaks = seq(min_age, max_age, by = ),
    ...
    g <- ggplot2::ggplot(data = df, ggplot2::aes(x = get("age_years"))) +
      ...
      ggplot2::facet_wrap(~ get(by_group), ncol = 1)
  }
  if (!is.null(title_name)) {
    g <- g + ggplot2::ggtitle(title_name)
  }
...
plot_age_months_distribution_phu <- function(
  df,
  by_group = NULL,
  file_path = NULL,
  wdth = NULL,
  ...
  if (!(c("age_months") %in% colnames(df))) {
    stop(
      "There is no age_months column in your dataframe. Please check your inputs."
    )
  }
  ...
  if (is.null(by_group)) {
    g <- ggplot2::ggplot(data = df, ggplot2::aes(x = age_months)) +
      ...
      ggplot2::scale_x_continuous(
        minor_breaks = seq(age_min, age_max, by = 1),
        breaks = seq(age_min, age_max, by = 12),
...
plot_ridge_distribution_phu <- function(
  df,
  numeric_cols,
  name_groups = NULL,
  name_units = NULL,
  ...
  df <- df %>%
    dplyr::select(grouping, numeric_cols) %>%
  ...
  g <- ggplot2::ggplot(
    ...
    ggplot2::xlab(name_units) +
    ggplot2::ylab(name_groups) +
    ggplot2::theme(
...
plot_agepyramid_phu <- function(
  ...
  if (!is.null(filtering)) {
    print(
      "Please note, if using filtering the variable must be coded numerically by 0s and 1s"
    )
    df <- df %>%
    ...
    g <- apyramid::age_pyramid(
      ...
      ggplot2::ylab(paste0("Proportion of ", filtering))
  } else {
...
plot_cumulative_distribution_phu <- function(
  df,
  index,
  flags,
  grouping = NULL,
  ...
  if (length(setdiff(index, valid_indexes)) != 0) {
    stop(
      "No valid index was specified. Valid inputs include 'wfhz', 'hfaz', 'wfaz', or 'mfaz'."
    )
  }
  if (is.null(flags)) {
    flags <- "yes"
    ...
    stop(
      "Please include a valid input for the flags argument. Use 'yes' to include flagged values in the plot. Use 'no' to exclude flagged values from the plot."
    )
  }
  if (index == "wfhz") {
    if (flags == "yes") {
  ...
  g <- ggplot2::ggplot(df, ggplot2::aes(x = .data$indx, color = "Overall")) +
    ...
    ggplot2::geom_vline(xintercept = severe) +
  ...
  g <- g +
    ggplot2::xlim(c(minval, maxval)) +
    ggplot2::theme_minimal() +
    ggplot2::xlab(index) +
    ggplot2::ylab("% Cumulative Proportion")
  g <- g +
    ggplot2::scale_x_continuous(breaks = seq(minval, maxval, by = brkval))
