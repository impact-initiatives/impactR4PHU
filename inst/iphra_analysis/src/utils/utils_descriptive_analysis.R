...
select_one.analysis <- function(srv.design, entry) {
  ...
  if (length(disaggregations) == 0) {
    ...
    res <- cbind(res.prop, res.ci) %>%
      data.frame() %>%
      mutate(name = rownames(.)) %>%
      mutate(
        pct = round(res.prop * 100, 1),
        ci = paste0(
          format(round(X5.. * 100, 1), scientific = F),
          "%-",
    ...
    res.prop <- svyby(
      make.formula(entry$variable),
      make.formula(disaggregations),
      srv.design,
      svymean,
      drop.empty.groups = F,
      multicore = F,
    ...
    # arrange output formatting
    res.ci$name <- as.character(lapply(rownames(res.ci), function(x) {
      str_split(x, entry$variable)[[1]][2]
    }))
    res.ci <- res.ci %>%
      mutate(
        ci = paste0(
    ...
    res <- select(
      res.prop,
      -colnames(res.prop)[str_detect(colnames(res.prop), "^se\\.")]
    )
    ...
    cols1 <- colnames(res)[
      !(colnames(res) %in% disaggregations) &
        !str_detect(colnames(res), "^ci\\.")
    ]
    cols2 <- colnames(res)[
      !(colnames(res) %in% disaggregations) &
        str_detect(colnames(res), "^ci\\.")
    ...
    res1 <- pivot_longer(
      res %>% select(rev(disaggregations), all_of(cols1)),
      cols = all_of(cols1),
      names_to = entry$variable,
      values_to = "pct"
    )
    res2 <- pivot_longer(
      res %>% select(rev(disaggregations), all_of(cols2)),
      cols = all_of(cols2),
      names_to = entry$variable,
      values_to = "ci"
    )
...
select_one.analysis_overall <- function(srv.design, entry) {
  ...
  if (length(disaggregations) == 1) {
    ...
    res.ci <- confint(res.prop, level = 0.90)
    # arrange output formatting
    res.overall <- cbind(res.prop, res.ci) %>%
      data.frame() %>%
      mutate(name = rownames(.)) %>%
      mutate(
        pct = round(res.prop * 100, 1),
    ...
    res.overall[[entry$variable]] <- str_remove(
      res.overall$name,
      entry$variable
    )
    ...
    cols1 <- colnames(res.overall)[
      !(colnames(res.overall) %in% disaggregations) &
        !str_detect(colnames(res.overall), "^ci\\.")
    ]
    cols2 <- colnames(res.overall)[
      !(colnames(res.overall) %in% disaggregations) &
        str_detect(colnames(res.overall), "^ci\\.")
    ]
    res1 <- pivot_longer(
      res.overall %>% select(rev(disaggregations), all_of(cols1)),
      cols = all_of(cols1),
      names_to = entry$variable,
      values_to = "pct"
    )
    res2 <- pivot_longer(
      res.overall %>% select(rev(disaggregations), all_of(cols2)),
      cols = all_of(cols2),
      names_to = entry$variable,
      values_to = "ci"
    )
...
select_one.to_html_bind_overall <- function(
  res,
  res.overall,
  entry,
  include.CI = T
  ...
  choices <- (tool.choices %>% pull(label_colname))[
    tool.choices$list_name == var_list_name
  ]
  res <- res %>% mutate(pct = ifelse(is.na(pct), NA, paste0(pct, "%")))
  res <- res %>%
    arrange(match(!!sym(entry$variable), choices)) %>%
    pivot_wider(
      names_from = entry$variable[1],
      values_from = c("pct", "ci"),
      names_sep = ".",
      values_fill = list(pct = "0%")
    )
  res.overall <- res.overall %>%
    mutate(pct = ifelse(is.na(pct), NA, paste0(pct, "%")))
  res.overall <- res.overall %>%
    arrange(match(!!sym(entry$variable), choices)) %>%
    pivot_wider(
      names_from = entry$variable[1],
  ...
  colnames(res) <- as.character(lapply(colnames(res), function(x) {
    if (str_starts(x, "pct.")) {
      return(str_sub(x, 5, str_length(x)))
    } else if (str_starts(x, "ci.")) {
      return(paste0(str_sub(x, 4, str_length(x)), " [CI]"))
    } else {
      return(x)
    }
  }))
  colnames(res.overall) <- as.character(lapply(
  ...
  if (!include.CI) {
    res <- select(
      res,
      -all_of(colnames(res)[str_detect(colnames(res), "\\[CI\\]")])
    )
    res.overall <- select(
      res.overall,
      -all_of(colnames(res.overall)[str_detect(
        colnames(res.overall),
        "\\[CI\\]"
      )])
    )
  ...
  if (!is.na(entry$disaggregate.variable)) {
    ...
    res <- res %>%
      left_join(
        t.res,
        by = c(set_names(entry$admin), set_names(entry$disaggregate.variable))
      ) %>%
      relocate("num_samples", .after = 2)
    res.overall <- res.overall %>%
      left_join(
        t.res_over,
        by = c("strata", set_names(entry$disaggregate.variable))
      ) %>%
      relocate("num_samples", .after = 2) %>%
      select(-overall)
    if (nrow(res) != n_rows) stop()
...
select_one.to_html <- function(res, entry, include.CI = T) {
  var.full <- entry$variable
  var_list_name <- get.choice.list.from.name(entry$variable)
  choices <- tool.choices[[label_colname]][
    tool.choices$list_name == var_list_name
  ]
  res <- res %>% mutate(pct = ifelse(is.na(pct), NA, paste0(pct, "%")))
  res <- res %>%
...
select_multiple.analysis <- function(srv.design, entry) {
  ...
  choices <- tool.choices %>%
    ...
    select(name, !!sym(label_colname)) %>%
    rename(label = !!sym(label_colname)) %>%
    mutate(label = ifelse(name %in% c("other", "Other"), "Other", label))
  variables <- colnames(srv.design$variables)[str_starts(
    colnames(srv.design$variables),
  ...
  if (length(disaggregations) == 0) {
    ...
    res <- cbind(res.prop, res.ci) %>%
      ...
      mutate(
        ...
        choice = str_sub(name, 1, str_length(name) - 1)
      ) %>%
      select(pct, ci, choice)
  } else {
    # get proportions and confidence intervals (using svyby + svymean)
    res.prop <- svyby(
      make.formula(variables),
      make.formula(disaggregations),
      srv.design,
      svymean,
      drop.empty.groups = F,
    ...
    cols <- colnames(res)[
      str_ends(colnames(res), "1") & !(colnames(res) %in% disaggregations)
    ]
    res <- res[c(rev(disaggregations), cols)]
    colnames(res) <- c(
      rev(disaggregations),
      str_sub(cols, 1, str_length(cols) - 1)
    )
    cols2 <- colnames(res)[str_starts(colnames(res), "ci.")]
    res1 <- pivot_longer(
      res %>% select(rev(disaggregations), all_of(variables)),
  ...
  res <- mutate(
    res,
    choice = str_sub(choice, str_length(entry$variable) + 4, str_length(choice))
  ) %>%
    left_join(choices, by = c("choice" = "name")) %>%
    select(-choice)
...
select_multiple.analysis_overall <- function(srv.design, entry) {
  ...
  if (length(disaggregations) == 1) {
    ...
    res.overall <- select(
      res.prop,
      -colnames(res.prop)[str_starts(colnames(res.prop), "^se\\.")]
    )
    res.overall <- mutate_if(res.overall, is.numeric, ~ round(. * 100, 1))
    for (lev in unique(res.ci$name)) {
      res.ci.sub <- res.ci[res.ci$name == lev, ] %>% select(-name)
    ...
    cols <- colnames(res.overall)[
      str_ends(colnames(res.overall), "1") &
        !(colnames(res.overall) %in% disaggregations)
    ]
    res.overall <- res.overall[c(rev(disaggregations), cols)]
    colnames(res.overall) <- c(
      rev(disaggregations),
      str_sub(cols, 1, str_length(cols) - 1)
    )
    cols2 <- colnames(res.overall)[str_detect(colnames(res.overall), "^ci\\.")]
    res1 <- pivot_longer(
      res.overall %>% select(rev(disaggregations), all_of(variables)),
      cols = all_of(variables),
      names_to = "choice",
      values_to = "pct"
    )
...
select_multiple.to_html_bind_overall <- function(
  res,
  res.overall,
  entry,
  include.CI = T
  ...
  res2 <- res %>%
    select(-pct) %>%
    pivot_wider(names_from = "label", values_from = ci, values_fill = "NA")
  res2 <- res2[, (ncol(res) - 3 + 1):ncol(res2)]
  colnames(res2) <- as.character(lapply(colnames(res2), function(x) {
    paste0(x, " [CI]")
  }))
  res1.overall <- res.overall %>%
    select(-ci) %>%
    pivot_wider(names_from = "label", values_from = pct, values_fill = "0%")
  res2.overall <- res.overall %>%
    select(-pct) %>%
    pivot_wider(names_from = "label", values_from = ci, values_fill = "NA")
  res2.overall <- res2.overall[, (ncol(res.overall) - 3 + 1):ncol(res2.overall)]
  colnames(res2.overall) <- as.character(lapply(
    colnames(res2.overall),
    function(x) paste0(x, " [CI]")
  ))
  if (include.CI) {
    res <- cbind(res1, res2)
  } else {
  ...
  if (!is.na(entry$disaggregate.variable)) {
    res <- res %>%
      arrange(!!sym(entry$admin), !!sym(entry$disaggregate.variable))
    res.overall <- res.overall %>%
      arrange(overall, !!sym(entry$disaggregate.variable))
    t.res <- data %>%
      filter(!is.na(!!sym(entry$variable))) %>%
      group_by(!!sym(entry$admin), !!sym(entry$disaggregate.variable)) %>%
      summarise(num_samples = n()) %>%
...
mean.analysis <- function(srv.design, entry) {
  ...
  if (!is.na(entry$admin)) {
    srv.design.grouped <- srv.design.grouped %>%
      group_by(!!sym(entry$admin), .add = T, .drop = T)
  }
  if (!is.na(entry$disaggregate.variable)) {
    srv.design.grouped <- srv.design.grouped %>%
      group_by(!!sym(entry$disaggregate.variable), .add = T, .drop = T)
  }
  if (all(is.na(srv.design.grouped$variables[[entry$variable]]))) {
    return(data.frame())
  }
  res <- srv.design.grouped %>%
    filter(!is.na(!!sym(entry$variable))) %>%
    summarise(
      mean = survey_mean(!!sym(entry$variable), vartype = "ci", level = 0.90)
    )
  if (str_starts(entry$variable, "pct.")) {
    res <- res %>%
      mutate(
        mean = round(mean, 1),
        ci = paste0(
          format(round(mean_low, 1), scientific = F),
          "%-",
    ...
    res <- res %>%
      mutate(
        mean = round(mean, 2),
        ci = paste0(
          format(round(mean_low, 2), scientific = F),
          "-",
...
mean.analysis_overall <- function(srv.design, entry) {
  ...
  if (!is.na(entry$disaggregate.variable)) {
    res.overall <- filter(
      res.overall,
      !is.na(!!sym(entry$disaggregate.variable))
    )
  }
  return(res.overall)
}
...
mean.to_html_overall <- function(res, res.overall, entry, include.CI = T) {
  if (str_starts(entry$variable, "pct.")) {
    ...
    res.overall <- res.overall %>%
      mutate(mean = ifelse(is.na(mean), NA, paste0(mean, "%")))
  }
  ...
  if (str_starts(entry$variable, "pct.")) {
    res <- res %>%
      mutate(
        ...
        ci = paste0(
          format(round(median_low, 1), scientific = F),
          "%-",
    ...
    res <- res %>%
      mutate(
        ...
        ci = paste0(
          format(round(median_low, 2), scientific = F),
          "-",
...
median.to_html <- function(res, entry, include.CI = T) {
  if (str_starts(entry$variable, "pct.")) {
    res <- res %>%
      mutate(median = ifelse(is.na(median), NA, paste0(median, "%")))
  }
...
# add section to HTML
add_to_html.section <- function(name) {
...
load.entry <- function(analysis.plan.row) {
  ...
  omit_na <- is.na(analysis.plan.row$calculation) |
    (!is.na(calculation) &
      str_detect(analysis.plan.row$calculation, "omit_na", T))
  if (is.na(disaggregate.variable)) {
    disaggregate.variables <- c(NA)
  } else {
    disaggregate.variables <- c(str_split(
      str_remove_all(disaggregate.variable, " "),
      " ?;+ ?"
    )[[1]])
  }
  return(list(
    ...
    func = func,
    admin = admin,
    disaggregate.variables = disaggregate.variables,
    # data=data,
    xlsx_name = xlsx_name,
    comments = comments,
...
save.dfs <- function(
  ...
  if (use_template) {
    wb <- openxlsx::loadWorkbook("./../resources/analysis_template.xlsx")
  } else {
