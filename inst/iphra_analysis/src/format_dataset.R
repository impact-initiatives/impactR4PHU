...
hh_roster <- hh_roster %>%
  dplyr::mutate(
    age_group = as.character(cut(
      as.numeric(calc_final_age_years),
      breaks = c(-1, 2, 5, 10, 18, 59, Inf),
      labels = c("00-02", "03-05", "06-10", "11-18", "19-59", "59+")
    ))
  )
if (!is.null(ind_health)) {
  ind_health <- ind_health %>%
    dplyr::mutate(
      age_group = as.character(cut(
        as.numeric(health_ind_age_years),
        breaks = c(-1, 2, 5, 10, 18, 59, Inf),
        labels = c("00-02", "03-05", "06-10", "11-18", "19-59", "59+")
      )),
      health_ind_sex = dplyr::case_when(
        health_ind_sex == "m" ~ "Male",
...
if (!is.null(child_nutrition)) {
  child_nutrition_data <- child_nutrition %>%
    dplyr::mutate(
      age_group_nut = as.character(cut(
        as.numeric(child_age_months),
        breaks = c(5, 23, 59),
        labels = c("06-23", "24-59")
      )),
      child_sex = dplyr::case_when(
        child_sex == "m" ~ "Male",
        child_sex == "f" ~ "Female"
      )
    )
...
if (!is.null(women)) {
  women <- women %>%
    mutate(
      woman_bf = dplyr::case_when(
        woman_bf == "bf_child_under_23m" ~ "Breastfeeding child under 23 month",
  ...
  unmet_needs <- ind_health %>%
    mutate(
      unmet_needs = case_when(
        health_ind_received_healthcare == "no" ~ 1,
...
if (all(hhs_check_columns %in% names(main))) {
  ...
  hhs_column <- hhs_check_columns[
    !stringr::str_detect(hhs_check_columns, "_freq")
  ]
  for (i in hhs_column) {
    # make a long table:
    res.long <- hhs_survey %>%
  ...
  hhs_column_Freq <- hhs_check_columns[stringr::str_detect(
    hhs_check_columns,
    "_freq"
  )]
  for (i in hhs_column_Freq) {
    ...
    res.wide <- res.long %>%
      ...
      mutate(HHS = i) %>%
      rename(Often = "often", Rarely = "rarely", Sometimes = "sometimes") %>%
      relocate(HHS, .before = 1)
...
if (!is.null(died_member)) {
  mortality <- check_mortality_flags(main, hh_roster, died_member)
  mortality_data <- mortality$crude %>%
    mutate(
      fpc = nrow(hh_roster) / as.numeric(strings['population_estimation'])
    ) %>%
    mutate(
  ...
  if (as.numeric(strings['population_estimation']) > 10000) {
    ...
    res <- srvyr::as_survey_design(mortality_data, fpc = fpc)
  }

  crude <- res %>%
    srvyr::summarise(
      x = srvyr::survey_ratio(
        numerator = num_died * 10000,
  ...
  under5 <- res %>%
    srvyr::summarise(
      x = srvyr::survey_ratio(
        numerator = num_died_under5 * 10000,
  ...
  results.summary <- rbind(crude, under5) %>%
    select(-x_deff) %>%
    rename(`point.est` = "x", `95%lci` = "x_low", `95%uci` = "x_upp") %>%
    relocate(variable, .before = 1)

  mortality_data_sex <- mortality$crude %>%
    mutate(
      fpc = nrow(data.list$hh_roster) /
        as.numeric(strings['population_estimation'])
    ) %>%
    mutate(
      person_time_m = pt_total_m + pt_died_m - pt_birth_m,
  ...
  if (as.numeric(strings['population_estimation']) > 10000) {
    ...
    res_sex <- srvyr::as_survey_design(mortality_data_sex, fpc = fpc)
  }

  crude_m <- res_sex %>%
    srvyr::summarise(
      x = srvyr::survey_ratio(
        numerator = num_died_m * 10000,
    ...
    mutate_all(., ~ round(., 2)) %>%
    mutate(variable = "cmr", Sex = "Male")

  crude_f <- res_sex %>%
    srvyr::summarise(
      x = srvyr::survey_ratio(
        numerator = num_died_f * 10000,
    ...
    mutate_all(., ~ round(., 2)) %>%
    mutate(variable = "cmr", Sex = "Female")

  under5_m <- res_sex %>%
    srvyr::summarise(
      x = srvyr::survey_ratio(
        numerator = num_died_under5_m * 10000,
    ...
    mutate_all(., ~ round(., 2)) %>%
    mutate(variable = "u5mr", Sex = "Male")

  under5_f <- res_sex %>%
    srvyr::summarise(
      x = srvyr::survey_ratio(
        numerator = num_died_under5_f * 10000,
    ...
    mutate_all(., ~ round(., 2)) %>%
    mutate(variable = "u5mr", Sex = "Female")

  results.summary_bySex <- rbind(crude_m, crude_f, under5_m, under5_f) %>%
    select(-x_deff) %>%
    rename(`point.est` = "x", `95%lci` = "x_low", `95%uci` = "x_upp") %>%
    relocate(variable, .before = 1)
}
...
if (!is.null(child_nutrition_data)) {
  if (
    all(c("nut_muac_cm", "nut_edema_confirm") %in% names(child_nutrition_data))
  ) {
