...
bugged_choices <- tool.choices %>%
  filter(stringr::str_detect(name, bugged_choice_regex)) %>%
  left_join(tool.survey %>% select(q.type, list_name, datasheet)) %>%
  distinct()

bugged_cols <- tool.survey %>%
  filter(list_name %in% bugged_choices$list_name) %>%
...
if (nrow(bugged_cols) > 0) {
  ...
  if (nrow(bugged_s_one) > 0) {
    for (i in 1:nrow(bugged_s_one)) {
      ...
      data.list[[dsheet]][[col]] <- stringr::str_replace_all(
        data.list[[dsheet]][[col]],
        bugged_choice_regex,
        "_"
      )
    }
  ...
  if (nrow(bugged_s_multiple) > 0) {
    for (i in 1:nrow(bugged_s_multiple)) {
      ...
      colnames(data.list[[dsheet]]) <- stringr::str_replace_all(
        colnames(data.list[[dsheet]]),
        choice,
        choice_rep
      )
    }
...
bugged_vars <- tool.survey %>%
  filter(stringr::str_detect(name, bugged_varname_regex)) %>%
  select(name, q.type, datasheet, list_name)

if (nrow(bugged_vars) > 0) {
  for (sheet in names(data.list)) {
    names(data.list[[sheet]]) <- stringr::str_replace_all(
      names(data.list[[sheet]]),
      bugged_varname_regex,
      "_"
    )
  }
  tool.survey$name <- stringr::str_replace_all(
    tool.survey$name,
    bugged_varname_regex,
    "_"
  )
  daf$variable <- stringr::str_replace_all(
    daf$variable,
    bugged_varname_regex,
    "_"
  )
